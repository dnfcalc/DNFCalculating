...
装备增幅名望 = [3,6,8,11,14,16,19,32,36,50,99,158,171,185,198,211,224,237,250,263,276,289,302,315,328,341,354,369,382,395,408]

装备改造名望 = [11,16,32,50,158,185,211,237,263,277,290,303,316,329,342,355,369,382,395,408,408,408,408,408,408,408,408,408,408,408,408]
武器改造名望 = [33,50,99,156,492,574,656,738,820,861,902,943,984,1025,1066,1107,1148,1189,1230,1271,1271,1271,1271,1271,1271,1271,1271,1271,1271,1271,1271]
神话装备强化名望 = [3,6,9,12,15,18,21,36,40,57,111,178,193,208,222,237,252,267,282,296,311,326,341,355,370,385,400,415,429,444,459]
神话装备增幅名望 = [4,7,10,14,17,20,24,40,45,63,123,198,214,231,247,263,280,296,313,329,346,362,378,395,411,428,444,461,477,493,510]
...
武器增幅名望 = [9,17,25,33,41,50,58,99,111,156,307,492,533,574,615,656,697,738,779,820,861,902,943,984,1025,1066,1107,1148,1189,1230,1271]
...
def 符文名望计算(角色属性):
    ...
    return 最终符文名望
def 宠物名望计算(角色属性):
    细节 = 细节收集("宠物附魔", 角色属性.属性设置输入)
    l = sum(_ for _ in 细节)
    最终宠物附魔名望 = 185 if l != 0 else 0  # 填了就是185，没填就是0

    宠物名称 = 角色属性.角色属性B.宠物.名称
    宠物名称 = 宠物名称[7:]
    最终宠物名望 = 宠物名望.get(宠物名称, 0)

    return 最终宠物附魔名望 + 最终宠物名望
def 宠物装备名望计算(角色属性):
    def 宠物装备名望红计算(角色属性):
        细节 = 细节收集("宠物装备红", 角色属性.属性设置输入)
        return 69

    def 宠物装备名望蓝计算(角色属性):
        细节 = 细节收集("宠物装备蓝", 角色属性.属性设置输入)
        return 69

    def 宠物装备名望绿计算(角色属性):
        细节 = 细节收集("宠物装备绿", 角色属性.属性设置输入)
        return 69

    红, 蓝, 绿 = 宠物装备名望红计算(角色属性), 宠物装备名望蓝计算(角色属性), 宠物装备名望绿计算(角色属性)
    return 红+蓝+绿
def 装备基础名望计算(角色属性):
    所有装备 = 角色属性.角色属性B.装备栏
    ...
    改造基础名望 = 632 * 改造数目
    史诗基础名望 = 464 * (11 - 改造数目)
    套装基础名望 = len(角色属性.角色属性B.套装栏) * 265
    return 改造基础名望 + 史诗基础名望 + 套装基础名望
def 装备打造名望计算(角色属性):
    所有装备 = 角色属性.角色属性B.装备栏[-1] # 去掉武器
    打造名望 = []
    for i in range(len(所有装备)):
        ...
        if 装备.品质 != "史诗":
            ...
            if 装备.所属套装 == "智慧产物":
                改造等级 = 角色属性.角色属性B.改造等级[i]
                单件装备打造名望 = 装备改造名望[改造等级]
            else:
                打造等级 = 角色属性.角色属性B.强化等级[i]
                if 角色属性.角色属性B.是否增幅[i]:
...
def 附魔名望计算(角色属性):
    def 附魔部位名望计算(角色属性, 部位):
        属性设置输入, 细节选项输入 = 角色属性.属性设置输入, 角色属性.细节选项输入
        部位列 = 装备列[部位]

    附魔名望 = {}
    for 部位 in 装备列:
        附魔名望[部位] = 附魔名望计算(角色属性, 部位)
