...
def 希洛克融合名望计算(人物设置):
    ...
    希洛克融合名望 = {"残香名望": 残香名望, "希洛克融合名望": 58 * 希洛克件数}
    return 希洛克融合名望


def 奥兹玛融合名望计算(人物设置):
    奥兹玛选择状态 = 人物设置.希洛克选择状态
    奥兹玛件数 = sum(奥兹玛选择状态)
...
def 获取人物名望(角色属性, 人物设置):
    ...
    总名望 = 装备名望 + sum(打造名望) + sum(list(附魔名望.values())) + 称号名望 + 宠物名望 + sum(
        徽章名望.values()) + 时装名望 + 护石名望 + 符文名望 + 勋章名望 + 辟邪玉名望 + 收集箱名望 + sum(list(改造升级名望.values())) + \
          sum(list(黑鸦名望.values())) + sum(list(希洛克融合名望.values())) + 奥兹玛融合名望
    名望详情 = {
        "基础装备名望": 装备名望,
        "增幅/强化/锻造/改造名望": 打造名望,
        "附魔名望": 附魔名望,
