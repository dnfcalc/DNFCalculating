...
增幅系数 = [0, 1, 2, 3, 4, 6, 8, 10, 12, 13, 17, 33, 50, 67, 108, 150, 192, 267, 342, 417, 499.9, 583, 667, 750, 833, 917, 1000, 1083, 1167, 1250, 1333, 1417]
...
def 武器计算(装备等级, 品质, 强化等级, 武器类型, 伤害类型):
    ...
    if 伤害类型 == '固伤':
        ...
        强化系数 = 武器强化系数[强化等级]
    return int((装备等级 + 武器品级加分[品质]) / 8 * 武器成长系数[品质] * 强化系数 * 武器系数 + 补正)

def 锻造计算(装备等级, 品质, 强化等级):
    return 武器计算( 装备等级, 品质, max(强化等级,8) , '' , '固伤' )
