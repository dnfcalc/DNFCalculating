...
特殊成长系数 = {'普通': 0.4, '高级': 0.7, '稀有': 1.0, '神器': 1.25, '传说': 1.35, '史诗': 1.45, '神话': 1.55}
...
def 武器计算(装备等级, 品质, 强化等级, 武器类型, 伤害类型): 
    武器系数 = (武器类型系数[武器类型][0 if 伤害类型 == '物理' else 1])
    return int((装备等级 + 武器品级加分[品质]) / 8 * 武器成长系数[品质] * 武器强化系数[强化等级] * 武器系数)

def 锻造计算(装备等级, 品质, 锻造等级): 
    return int(round((装备等级 + 武器品级加分[品质]) / 8 * 武器成长系数[品质] * 武器锻造系数[锻造等级]))
...
def 基础属性输入(属性):
    ...
    属性.基础力量 = int(角色数据[4] + 角色数据[0] * min(14, 当前等级) + 职业数据[0] * max(当前等级 - 15, 0) + 275 + 2.1 * min(71, 当前等级))
    属性.基础体力 = int(角色数据[5] + 角色数据[1] * min(14, 当前等级) + 职业数据[1] * max(当前等级 - 15, 0) + 275 + 2.0 * min(71, 当前等级))
    属性.基础智力 = int(角色数据[6] + 角色数据[2] * min(14, 当前等级) + 职业数据[2] * max(当前等级 - 15, 0) + 275 + 2.1 * min(71, 当前等级))
    属性.基础精神 = int(角色数据[7] + 角色数据[3] * min(14, 当前等级) + 职业数据[3] * max(当前等级 - 15, 0) + 275 + 2.0 * min(71, 当前等级))
