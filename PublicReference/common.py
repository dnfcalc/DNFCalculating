...
class 窗口(QWidget):
    ...
    def 界面(self):
        ...
        self.main_frame.setGeometry(0, 28, self.width(), self.height() - self.frame_tool.height())
            ...
            for j in range(len(装备列表)):
                if 装备列表[j].品质 == '神话':
                    if self.装备选择状态[j] == 1 or 装备列表[j].名称 in 自选装备名称:
                        self.神话属性图片[num].move(int(self.width() / 7 * (count1 % 7+ 0.42)), int(self.height() / 5.2 * (count2 + 0.05))- 3)
                        for i in range(4):
                            self.神话属性选项[num * 4 + i].move(int(self.width() / 7 * (count1 % 7) + 6), int(self.height() / 5.2 * (count2 + 0.05)) - 3+ i * 22 + 32)
                        count1 += 1
                        if count1 % 7 == 0:
                            count2 += 1
                    else:
        ...
        for j in range(len(装备列表)):
            if 装备列表[j].所属套装 == '智慧产物' :
                if self.装备选择状态[j] == 1 and x==1:
                    self.改造产物图片[num].move(int(self.width() / 7 * (count1 % 5+ 0.42+2))-10, int(self.height() / 5.2 * (count2 + 0.05)))
                    for i in range(4):
                        self.改造产物选项[num * 4 + i].move(int(self.width() / 7 * (count1 % 5+2) + 2), int(self.height() / 5.2 * (count2 + 0.05)) + i * 22 + 32)
                    count1 += 1
                    if count1 % 5 == 0:
                        count2 += 1
                else:
    ...
    def 排行界面(self, 筛选 = {}, 显示序号 = [x for x in range(100)]):
        ...
        if 输出数据 == 1 and len(筛选) == 0:
            setfile = open('./数据记录/{}-{}.csv'.format(self.角色属性A.实际名称, time.strftime('%m-%d-%H-%M-%S')), 'w', encoding='gbk')
    ...
    def __init__(self, father, data, 统计详情, excel, 实际技能等级, 技能等效CD, 显示模式, count):
        ...
        for i in range(0,len(data.角色属性B.技能栏)):
            ...
            if 统计详情[i*4 + 1] != 0:
                ...
                CD恢复 = ''
                try:
                    tempstr='<font face="宋体"><font color="#FF6666">'+data.角色属性B.技能栏[i].名称+data.角色属性B.技能栏[i].备注+ '</font><br>'
                    百分比 = int(data.角色属性B.技能栏[i].等效百分比(data.角色属性B.武器类型) / data.角色属性B.技能栏[i].倍率)
                    ...
                    CD显示 = str(round(data.角色属性B.技能栏[i].等效CD(data.角色属性B.武器类型,data.角色属性B.类型) * data.角色属性B.技能栏[i].恢复,2))
                ...
                #总伤害
                if 显示模式 == 1:
                    每行详情[4].setText(data.百分比输出(统计详情[i*4+1], data.基准值[1][i*4+1]))
                else:
                ...
                #平均伤害
                if 显示模式 == 1:
                    每行详情[5].setText(data.百分比输出(统计详情[i*4+2], data.基准值[1][i*4+2]))
                else:
                ...
                try:
                    self.data[j].append(str(int(百分比*被动倍率/100*其他倍率/100))+'%')
                except:
                    self.data[j].append('-')
