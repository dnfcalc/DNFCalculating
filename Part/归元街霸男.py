...
while i >= 0:
    try:
        exec('归元街霸男技能列表.append(归元街霸男技能' + str(i) + '())')
        i += 1
    except:
        i = -1

归元街霸男技能序号 = dict()
for i in range(len(归元街霸男技能列表)):
    归元街霸男技能序号[归元街霸男技能列表[i].名称] = i
...
for i in 归元街霸男技能列表:
    if i.所在等级 == 50:
        归元街霸男一觉序号 = 归元街霸男技能序号[i.名称]
    if i.所在等级 == 85:
        归元街霸男二觉序号 = 归元街霸男技能序号[i.名称]
    if i.所在等级 == 100:
        归元街霸男三觉序号 = 归元街霸男技能序号[i.名称]

归元街霸男护石选项 = ['无']
for i in 归元街霸男技能列表:
    if i.是否有伤害 == 1 and i.是否有护石 == 1:
        归元街霸男护石选项.append(i.名称)

归元街霸男符文选项 = ['无']
for i in 归元街霸男技能列表:
    if i.所在等级 >= 20 and i.所在等级 <= 80 and i.所在等级 != 50 and i.是否有伤害 == 1:
        归元街霸男符文选项.append(i.名称)


class 归元街霸男角色属性(角色属性):
    职业名称 = '归元街霸男'
    ...
    def __init__(self):
        self.技能栏 = deepcopy(归元街霸男技能列表)
        self.技能序号 = deepcopy(归元街霸男技能序号)

    def 被动倍率计算(self):
        super().被动倍率计算()
        if self.技能栏[self.技能序号['狂·霸王拳']].等级 != 0:
            self.技能栏[self.技能序号['伏虎霸王拳']].基础 += (4562 + 194 * self.技能栏[self.技能序号['狂·霸王拳']].等级) / self.技能栏[self.技能序号['狂·霸王拳']].加成倍率(self.武器类型)


class 归元街霸男(角色窗口):
    def 窗口属性输入(self):
        self.初始属性 = 归元街霸男角色属性()
        self.角色属性A = 归元街霸男角色属性()
        self.角色属性B = 归元街霸男角色属性()
        self.一觉序号 = 归元街霸男一觉序号
        self.二觉序号 = 归元街霸男二觉序号
        self.三觉序号 = 归元街霸男三觉序号
        self.护石选项 = deepcopy(归元街霸男护石选项)
        self.符文选项 = deepcopy(归元街霸男符文选项)

    def 界面(self):
        super().界面()
        self.毒瓶强化 = QCheckBox('强化毒瓶投掷', self.main_frame2)
        self.毒瓶强化.resize(100, 20)
        self.毒瓶强化.move(325, 450)
    ...
    def 输入属性(self, 属性, x=0):
        ...
        if self.毒针强化.isChecked():
            属性.强化毒针 = 1
            属性.技能栏[属性.技能序号['毒针投掷']].基础 = 1276
            属性.技能栏[属性.技能序号['毒针投掷']].成长 = 144
            属性.技能栏[属性.技能序号['毒针投掷']].出血基础 = 2003
            属性.技能栏[属性.技能序号['毒针投掷']].出血成长 = 216
        ...
        if self.爆狱强化.isChecked():
            ...
            属性.技能栏[属性.技能序号['逆道·爆狱']].灼伤基础 = 1889
            属性.技能栏[属性.技能序号['逆道·爆狱']].灼伤成长 = 214
