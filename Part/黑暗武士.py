...
class 黑暗武士技能36(被动技能):
    ...
    def 加成倍率(self, 武器类型):
        if self.等级 == 0 or 武器类型 != '光剑':
            ...
            return round(1.01279 + self.等级 * 0.01321, 5)
...
class 黑暗武士技能39(被动技能):
    ...
    def 加成倍率(self, 武器类型):
        if self.等级 == 0:
            return 1.0
        return round(1.000 + 0.11 * self.等级, 5)

    def 物理攻击力倍率(self, 武器类型):
        if self.等级 == 0:
            return 1.0
        else:
            return round(1.00 + self.等级 * 0.11, 5)
...
class 黑暗武士角色属性(角色属性):
    ...
    def 伤害指数计算(self):
        ...
        面板1 = (self.面板力量()/250+1) * (self.物理攻击力 + self.进图物理攻击力) * (1 + self.百分比三攻)
        面板2 = (self.面板智力()/250+1) * (self.魔法攻击力 + self.进图魔法攻击力) * (1 + self.百分比三攻)
