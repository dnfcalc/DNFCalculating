...
class 真·召唤师技能0(被动技能):
    ...
    等级上限=30
    基础等级=20
    关联技能=['契约召唤：桑德尔', '契约召唤：袄索', '精灵召唤：亡魂默克尔', '精灵召唤：极光格雷林', '精灵召唤：火焰赫瑞克', '精灵召唤：冰影阿奎利斯', '契约召唤：路易斯', '精灵召唤：伊伽贝拉', '契约召唤：牛头王库鲁塔', '契约召唤：征服者卡西利亚斯', '咒令：愤怒咆哮', '传说召唤：逆月者拉莫斯', '咒令：逆月之蚀', '千鬼杀', '至圣精灵王', '鞭挞' ]
    def 加成倍率(self, 武器类型):
        if self.等级==0:
            return 1.0
        else:
            return round((1+0.02*self.等级)*1.045,3)
...
class 真·召唤师技能30(真·召唤师主动技能):
    ...
    等级上限=40
    基础等级=2  
    基础 = 40372 + 94203 + 134574
    成长 = 12188 + 28438 + 40627
    CD=290
    演出时间 = 5.7
    关联技能 = ['无']
