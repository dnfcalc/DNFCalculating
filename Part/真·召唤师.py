...
class 真·召唤师技能0(被动技能):
    ...
    等级上限=30
    基础等级=20
    关联技能=['契约召唤：桑德尔', '契约召唤：袄索', '精灵召唤：亡魂默克尔', '精灵召唤：极光格雷林', '精灵召唤：火焰赫瑞克', '精灵召唤：冰影阿奎利斯', '契约召唤：路易斯', '精灵召唤：伊伽贝拉', '契约召唤：牛头王库鲁塔', '契约召唤：征服者卡西利亚斯', '咒令：愤怒咆哮', '传说召唤：逆月者拉莫斯', '咒令：逆月之蚀', '必杀剑千鬼杀', '至圣精灵王', '传说召唤：月蚀之影', '黑月之噬', '月蚀迪拉里昂：捕食模式' ]
    def 加成倍率(self, 武器类型):
        if self.等级==0:
            return 1.0
        else:
...
class 真·召唤师技能13(被动技能):
    名称='强击光环'
    所在等级=48
    等级上限=40
    基础等级=20
    关联技能=['精灵召唤：亡魂默克尔', '精灵召唤：极光格雷林', '精灵召唤：火焰赫瑞克', '精灵召唤：冰影阿奎利斯', '至圣精灵王']
...
class 真·召唤师技能31(被动技能):
    名称 = '绝对支配'
    所在等级=10
    等级上限=1
    基础等级=1
    关联技能=['契约召唤：桑德尔', '契约召唤：袄索', '精灵召唤：亡魂默克尔', '精灵召唤：极光格雷林', '精灵召唤：火焰赫瑞克', '精灵召唤：冰影阿奎利斯', '契约召唤：路易斯', '精灵召唤：伊伽贝拉', '契约召唤：牛头王库鲁塔', '契约召唤：征服者卡西利亚斯', '咒令：愤怒咆哮', '传说召唤：逆月者拉莫斯', '咒令：逆月之蚀', '必杀剑千鬼杀', '至圣精灵王' ]
...
while i >= 0:
    try:
        exec('真·召唤师技能列表.append(真·召唤师技能'+str(i)+'())')
        i += 1
    except:
        i = -1

真·召唤师技能序号 = dict()
for i in range(len(真·召唤师技能列表)):
    真·召唤师技能序号[真·召唤师技能列表[i].名称] = i
...
for i in 真·召唤师技能列表:
    if i.所在等级 == 50:
        真·召唤师一觉序号 = 真·召唤师技能序号[i.名称]
    if i.所在等级 == 85:
        真·召唤师二觉序号 = 真·召唤师技能序号[i.名称]
    if i.所在等级 == 100:
        真·召唤师三觉序号 = 真·召唤师技能序号[i.名称]

真·召唤师护石选项 = ['无']
for i in 真·召唤师技能列表:
    if i.是否有伤害 == 1 and i.是否有护石 == 1:
        真·召唤师护石选项.append(i.名称)

真·召唤师符文选项 = ['无']
for i in 真·召唤师技能列表:
    if i.所在等级 >= 20 and i.所在等级 <= 80 and i.所在等级 != 50 and i.是否有伤害 == 1:
        真·召唤师符文选项.append(i.名称)

class 真·召唤师角色属性(角色属性):
    ...
    def __init__(self):
        基础属性输入(self)
        self.技能栏= deepcopy(真·召唤师技能列表)
        self.技能序号= deepcopy(真·召唤师技能序号)
    ...
    def 数据计算(self, x = 0, y = -1):
        ...
        if self.觉醒选择状态 == 2:
            ...
            self.技能栏[self.技能序号['契约召唤：征服者卡西利亚斯']].演出时间 += 3.47
        if self.觉醒选择状态 == 1:
            self.技能栏[self.技能序号['契约召唤：征服者卡西利亚斯']].演出时间 += 1.47
...
class 真·召唤师(角色窗口):
    def 窗口属性输入(self):
        self.初始属性 = 真·召唤师角色属性()
        self.角色属性A = 真·召唤师角色属性()
        self.角色属性B = 真·召唤师角色属性()
        self.一觉序号 = 真·召唤师一觉序号
        self.二觉序号 = 真·召唤师二觉序号
        self.三觉序号 = 真·召唤师三觉序号
        self.护石选项 = deepcopy(真·召唤师护石选项)
        self.符文选项 = deepcopy(真·召唤师符文选项)
