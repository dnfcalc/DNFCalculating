...
class 混沌行者技能20(混沌行者主动技能):
    ...
    等级上限 = 40
    基础等级 = 16
    数据1 = [0, 7127, 7850, 8573, 9297, 10020, 10743, 11466, 12189, 12912, 13635, 14358, 15081, 15804, 16528, 17251,
    ...
    攻击次数1 = 4
    # 强化
    数据2 = [0, 4751, 5233, 5715, 6198, 6680, 7162, 7644, 8126, 8608, 9090, 9572, 10054, 10536, 11018, 11500, 11982,
...
class 混沌行者角色属性(角色属性):
    ...
    def 被动倍率计算(self):
        ...
        if self.聚爆形态 == 0:
            self.技能栏[self.技能序号['次元思维聚爆']].攻击次数1 = 7 # 不知道对不对
            self.技能栏[self.技能序号['次元思维聚爆']].攻击次数2 = 1
        if self.聚爆形态 == 1:
            self.技能栏[self.技能序号['次元思维聚爆']].攻击次数1 = 1
            self.技能栏[self.技能序号['次元思维聚爆']].攻击次数2 = 1

        if self.奇点形态 == 0:
            self.技能栏[self.技能序号['次元奇点']].攻击次数1 = 4
