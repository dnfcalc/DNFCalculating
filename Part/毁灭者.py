...
class 毁灭者技能0(主动技能):
    ...
    学习间隔 = 2
    等级精通 = 50
    基础等级 = min(int((毁灭者等级 - 所在等级) / 学习间隔 + 1), 等级精通) + 1
    数据 = [0, 274, 302, 329, 357, 385, 413, 441, 469, 496, 525, 552, 580, 608, 635, 664, 691, 719, 
    747, 775, 803, 831, 858, 886, 914, 942, 970, 997, 1025, 1053, 1081, 1109, 1136, 1165, 1192, 1220, 1248, 1275, 1304, 1332, 1359, 1387, 1415, 1443, 1471, 1498, 1526, 1554, 1582, 1610, 1637, 1665, 1694, 1721, 1749, 1776, 1805, 1833, 1860, 1888, 1915, 1944, 1972, 1999, 2027, 2055, 2083, 2111, 2138, 2166, 2195]
    攻击次数 = 7
    CD = 7.0
    TP成长 = 0.08
    TP上限 = 5
...
class 毁灭者技能13(主动技能):
    ...
    学习间隔 = 5
    等级精通 = 30
    基础等级 = min(int((毁灭者等级 - 所在等级) / 学习间隔 + 1), 等级精通)
    ...
    CD = 159.5
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2) * self.倍率 * 1.1
