...
class 真龙星君技能14(被动技能):
    ...
    等级上限 = 40
    基础等级 = 10
    关联技能 = ['真龙焚天','式神封灵阵','逆龙七杀','逆鳞震','灭魂符','苍龙逐日','式神：殇','疾空旋风破','落雷符','狂乱锤击','疾风打','朱雀符','黑暗切割','升天阵','落凤锤','破魔符']
...
class 真龙星君角色属性(角色属性):
    ...
    def 被动倍率计算(self):
        ...
        if self.反身空斩打 == 1:
            self.技能栏[self.技能序号['空斩打']].倍率 *= 1 + 2 * (潜龙.空斩打剑气[潜龙.等级] / 100)
        else:
            self.技能栏[self.技能序号['空斩打']].倍率 *= 1 + (潜龙.空斩打剑气[潜龙.等级] / 100)
