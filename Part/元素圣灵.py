...
class 元素圣灵技能16(主动技能):
    ...
    是否主动 = 1
    关联技能 = ['属性精通']
    属性精通加成 = [0, 131, 144, 156, 169, 181, 194, 206, 219, 231, 244, 256, 269, 281, 294, 306, 319, 331, 344, 356, 369]
    冷却关联技能 = ['魔法秀']
    魔法秀加成 = [0, 160, 174, 188, 201, 216, 229, 244, 257, 272, 285, 299, 313, 327, 341, 355, 369, 383, 397, 411, 424]
    
    def 加成倍率(self, 武器类型):
        if self.等级 == 0:
            return 1.0
        else:
            return round(1 + 0.001*self.属性精通加成[self.等级] , 3)
            
    def CD缩减倍率(self, 武器类型):
        if self.等级 == 0:
            return 0.0
        else:
            return round(0.001*self.魔法秀加成[self.等级] , 3)
...
class 元素圣灵技能18(主动技能):
    ...
    def CD缩减倍率(self, 武器类型):
        ...
        if 武器类型 == '法杖':
            return round(1.1*(1 - 0.001*self.魔法秀数值[self.等级]), 3)
    ...
    def CD倍率计算(self):
        for i in self.技能栏:
            if i.冷却关联技能 !=['无']:
                ...
                if i.名称 =='魔法秀':
                      for l in i.冷却关联技能:
                        self.技能栏[self.技能序号[l]].CD *= self.魔法秀加成()
                elif i.名称 !='圣灵符文':
                    for k in i.冷却关联技能:
