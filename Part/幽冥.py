...
class 幽冥技能0(主动技能):
    ...
    学习间隔 = 2
    等级精通 = 50
    基础等级 = min(int((幽冥等级 - 所在等级) / 学习间隔) + 1, 等级精通)
...
class 幽冥技能8(主动技能):
    ...
    def 装备护石(self, x):
        if x == 0:
            self.攻击次数1 = 18 * 2 * 0.78
            self.攻击次数2 = 1
        elif x == 1:
            self.攻击次数1 = 18 * 2 * (0.78 + 0.14)
            self.攻击次数2 *= 1.26
...
while i >= 0:
    try:
        exec('幽冥技能列表.append(幽冥技能' + str(i) + '())')
        i += 1
    except:
        i = -1

幽冥技能序号 = dict()
for i in range(len(幽冥技能列表)):
    幽冥技能序号[幽冥技能列表[i].名称] = i
...
class 幽冥角色属性(角色属性):
    ...
    def __init__(self):
        基础属性输入(self)
        self.技能栏 = deepcopy(幽冥技能列表)
        self.技能序号 = deepcopy(幽冥技能序号)
...
class 幽冥(角色窗口):
    def 窗口属性输入(self):
        ...
        self.二觉序号 = 幽冥二觉序号
        self.三觉序号 = 幽冥三觉序号
        self.护石选项 = deepcopy(幽冥护石选项)
        self.符文选项 = deepcopy(幽冥符文选项)
