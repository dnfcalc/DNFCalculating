...
class 亡魂主宰技能0(亡魂主宰主动技能):
    ...
    学习间隔 = 2
    等级精通 = 50
    基础等级 = min(int((亡魂主宰等级 - 所在等级) / 学习间隔), 等级精通) 
...
class 亡魂主宰技能2(亡魂主宰主动技能):
    名称 = '诅咒之剑'
    所在等级 = 15
    等级上限 = 60
    学习间隔 = 2
    等级精通 = 50
    基础等级 = min(int((亡魂主宰等级 - 所在等级) / 学习间隔) + 1, 等级精通)
...
class 亡魂主宰技能12(被动技能):
    ...
    def 技能描述(self, 武器类型):
        temp = ''
        temp += '百鬼夜行 +%.1f%%' % (22 + 3 * self.等级) + '<br>'
        temp += '驱使僵尸 +%.1f%%' % (24 + 6* self.等级) + '<br>'
        temp += '死灵之怨 +%.1f%%' % (24 + 1 * self.等级) + '<br>'
        temp += '降临：僵尸莱迪娅 +%.1f%%' % (8 + 2 * self.等级) + '<br>'
        temp += '死灵之缚 +%.1f%%' % (21 + 1 * (self.等级 - 2))
        return temp

class 亡魂主宰技能13(亡魂主宰主动技能):
    名称 = '降临：暴君巴拉克（一轮平x）'
    所在等级 = 35
    等级上限 = 60
    基础等级 = 36
...
class 亡魂主宰技能24(亡魂主宰主动技能):
    ...
    def 技能描述(self, 武器类型):
        temp = ''
        temp += '降临：尼古拉斯（蜘蛛团）, 降临：尼古拉斯（艾克洛索） Lv+4<br>'
        temp += '降临：暴君巴拉克（一轮平x）, 降临：暴君巴拉克（2下平x+黑手）, 降临：暴君巴拉克（暗击拳爆炸）, 降临：暴君巴拉克（强击）, 降临：暴君巴拉克（杀戮乱舞）Lv+5'
        return temp

    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2 + self.数据3[self.等级] * self.攻击次数3) * (1 + self.TP成长 * self.TP等级) * self.倍率

class 亡魂主宰技能25(亡魂主宰主动技能):
    名称 = '死灵之缚'
    所在等级 = 60
    等级上限 = 40
    学习间隔 = 2
...
while i >= 0:
    try:
        exec('亡魂主宰技能列表.append(亡魂主宰技能' + str(i) + '())')
        i += 1
    except:
        i = -1

亡魂主宰技能序号 = dict()
for i in range(len(亡魂主宰技能列表)):
    亡魂主宰技能序号[亡魂主宰技能列表[i].名称] = i
...
for i in 亡魂主宰技能列表:
    if i.所在等级 == 50:
        亡魂主宰一觉序号 = 亡魂主宰技能序号[i.名称]
    if i.所在等级 == 85:
        亡魂主宰二觉序号 = 亡魂主宰技能序号[i.名称]
    if i.所在等级 == 100:
        亡魂主宰三觉序号 = 亡魂主宰技能序号[i.名称]

亡魂主宰护石选项 = ['无']
for i in 亡魂主宰技能列表:
    if i.是否有伤害 == 1 and i.是否有护石 == 1:
        亡魂主宰护石选项.append(i.名称)

亡魂主宰符文选项 = ['无']
for i in 亡魂主宰技能列表:
    if i.所在等级 >= 20 and i.所在等级 <= 80 and i.所在等级 != 50 and i.是否有伤害 == 1:
        亡魂主宰符文选项.append(i.名称)
...
class 亡魂主宰角色属性(角色属性):
    ...
    def __init__(self):
        基础属性输入(self)
        self.技能栏 = deepcopy(亡魂主宰技能列表)
        self.技能序号 = deepcopy(亡魂主宰技能序号)

    def 被动倍率计算(self):
        if self.一觉Buff开关 == 1:
            self.一觉提升技能1 = [ '降临：尼古拉斯（蜘蛛团）', '降临：尼古拉斯（艾克洛索）']
            self.一觉提升技能2 = [ '降临：暴君巴拉克（一轮平x）', '降临：暴君巴拉克（2下平x+黑手）', '降临：暴君巴拉克（暗击拳爆炸）', '降临：暴君巴拉克（强击）', '降临：暴君巴拉克（杀戮乱舞）']
