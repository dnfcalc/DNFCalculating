...
class 月影星劫技能1(被动技能):
    ...
    等级上限 = 30
    基础等级 = 20
    data1 = [0, 36.0, 53.0, 70.0, 87.0, 104.0, 121.0, 138.0, 155.0, 172.0, 189.0, 206.0, 223.0, 240.0, 257.0, 274.0, 291.0, 308.0, 325.0, 342.0, 359.0, 387.0, 415.0, 443.0, 471.0, 499.0, 527.0, 555.0, 583.0, 611.0, 639.0, 667.0, 695.0, 723.0, 751.0, 779.0, 807.0, 835.0, 863.0, 891.0, 919.0, 947.0, 975.0, 1003.0, 1031.0, 1059.0, 1087.0, 1115.0, 1143.0, 1171.0, 1199.0, 1227.0, 1255.0, 1283.0, 1311.0, 1339.0, 1367.0, 1395.0, 1423.0, 1451.0, 1479.0, 1507.0, 1535.0, 1563.0, 1591.0, 1619.0, 1647.0, 1675.0, 1703.0, 1731.0, 1759.0]
    data2 = [0, 10.0, 13.0, 16.0, 19.0, 22.0, 25.0, 28.0, 31.0, 34.0, 37.0, 40.0, 43.0, 46.0, 49.0, 52.0, 55.0, 58.0, 61.0, 64.0, 67.0, 70.0, 73.0, 76.0, 79.0, 82.0, 85.0, 88.0, 91.0, 94.0, 97.0, 100.0, 103.0, 106.0, 109.0, 112.0, 115.0, 118.0, 121.0, 124.0, 127.0, 130.0, 133.0, 136.0, 139.0, 142.0, 145.0, 148.0, 151.0, 154.0, 157.0, 160.0, 163.0, 166.0, 169.0, 172.0, 175.0, 178.0, 181.0, 184.0, 187.0, 190.0, 193.0, 196.0, 199.0, 202.0, 205.0, 208.0, 211.0, 214.0, 217.0]
    def 加成倍率(self, 武器类型):
        if 武器类型 != '匕首':
            return 1.0
        else:
...
class 月影星劫技能2(被动技能):
    ...
    基础等级 = 20
    data1 = [0, 36.0, 53.0, 70.0, 87.0, 104.0, 121.0, 138.0, 155.0, 172.0, 189.0, 206.0, 223.0, 240.0, 257.0, 274.0, 291.0, 308.0, 325.0, 342.0, 359.0, 387.0, 415.0, 443.0, 471.0, 499.0, 527.0, 555.0, 583.0, 611.0, 639.0, 667.0, 695.0, 723.0, 751.0, 779.0, 807.0, 835.0, 863.0, 891.0, 919.0, 947.0, 975.0, 1003.0, 1031.0, 1059.0, 1087.0, 1115.0, 1143.0, 1171.0, 1199.0, 1227.0, 1255.0, 1283.0, 1311.0, 1339.0, 1367.0, 1395.0, 1423.0, 1451.0, 1479.0, 1507.0, 1535.0, 1563.0, 1591.0, 1619.0, 1647.0, 1675.0, 1703.0, 1731.0, 1759.0]
    data2 = [0, 22.0, 23.0, 25.0, 26.0, 27.0, 28.0, 29.0, 31.0, 32.0, 33.0, 34.0, 36.0, 37.0, 38.0, 39.0, 40.0, 42.0, 43.0, 44.0, 45.0, 46.0, 48.0, 49.0, 50.0, 51.0, 52.0, 54.0, 55.0, 56.0, 57.0, 59.0, 60.0, 61.0, 62.0, 63.0, 65.0, 66.0, 67.0, 68.0, 69.0, 71.0, 72.0, 73.0, 74.0, 75.0, 77.0, 78.0, 79.0, 80.0, 82.0, 83.0, 84.0, 85.0, 86.0, 88.0, 89.0, 90.0, 91.0, 92.0, 94.0, 95.0, 96.0, 97.0, 98.0, 100.0, 101.0, 102.0, 103.0, 104.0, 106.0]
...
class 月影星劫技能5(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级]+self.数据1[self.等级]+self.数据2[self.等级]) * self.攻击次数 + self.数据3[self.等级] * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能6(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据[self.等级] * self.攻击次数 + (self.数据1[self.等级] + self.数据2[self.等级] * 3) * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能7(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据[self.等级] * self.攻击次数 + self.数据1[self.等级] * self.攻击次数3) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能8(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级] * 3 + self.数据1[self.等级] * 3 +self.数据2[self.等级] * 3) * self.攻击次数 + self.数据3[self.等级] * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能9(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据[self.等级] * self.旋转次数 * self.攻击次数 + self.数据1[self.等级] * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能10(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级] * self.旋转次数 * self.旋转倍率 + self.数据1[self.等级]) * self.攻击次数 + self.数据2[self.等级] * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能12(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级]*self.乱舞数 + self.数据1[self.等级] * self.分身乱舞数 + self.数据2[self.等级]) * self.攻击次数 + self.数据3[self.等级] * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能13(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级] + self.数据1[self.等级] + self.数据2[self.等级] + self.数据3[self.等级]) * self.攻击次数 + self.数据4[self.等级] * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能15(月影星劫主动技能):
    ...
    def 装备护石(self):
        self.攻击次数 *= 1.129
        self.攻击次数2 *= 0.2
        self.攻击次数2 += 0.857 * self.攻击次数
        self.CD *= 0.89

    def 等效百分比(self, 武器类型):
        return (self.数据[self.等级] * 30 * self.攻击次数 + self.数据1[self.等级] * 5 * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能16(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级] * 3 * self.倍率1 + self.数据1[self.等级] * self.倍率2) * self.攻击次数 + self.数据2[self.等级] * self.攻击次数2) * self.倍率 * (1 + self.TP成长 * self.TP等级)
...
class 月影星劫技能17(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级] * 6 + self.数据1[self.等级]) * self.攻击次数 + self.数据2[self.等级] * self.攻击次数2) * self.倍率
...
class 月影星劫技能18(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据[self.等级] * 10 * self.攻击次数 + self.数据1[self.等级] * self.攻击次数2) * self.倍率
...
class 月影星劫技能19(月影星劫主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return ((self.数据[self.等级] * 8 + self.数据1[self.等级]) * self.攻击次数 + self.数据2[self.等级] * self.攻击次数2) * self.倍率
...
class 月影星劫技能20(被动技能):
    ...
    匕首数量 = 30
    def 额外百分比(self):
        return self.数据[self.等级] * self.匕首数量
...
class 月影星劫(角色窗口):
    ...
    def 输入属性(self, 属性, x = 0):
        ...
        属性.技能栏[属性.技能序号['旋舞斩']].攻击次数3 = self.旋舞斩收招次数.currentIndex() + 1
        属性.技能栏[属性.技能序号['剑刃风暴']].旋转次数 = self.剑刃风暴旋转次数.currentIndex() + 1
        属性.技能栏[属性.技能序号['螺旋穿刺']].旋转次数 = self.螺旋穿刺旋转次数.currentIndex() + 1
