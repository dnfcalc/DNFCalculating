...
class 极诣剑魂技能6(极诣剑魂主动技能):
    ...
    hit数 = 4
    def 额外刺伤层数(self, 武器类型):
        return (0.01 * self.等级 + 0.17) * int(self.等级 / 20)

    def 穿云刺数量(self, 武器类型):
        return 2


class 极诣剑魂技能7(极诣剑魂主动技能):
    名称 = '流心跃'
    备注 = '(地面释放)'
    所在等级 = 25
    等级上限 = 60
    ...
    hit数 = 2
    def 额外刺伤层数(self, 武器类型):
        return (0.01 * self.等级 + 0.34) * int(self.等级 / 15)

    def 穿云刺数量(self, 武器类型):
        return 2

class 极诣剑魂技能8(极诣剑魂主动技能):
    名称 = '流心升'
    所在等级 = 30
    等级上限 = 60
    基础等级 = 38
...
class 极诣剑魂技能13(极诣剑魂主动技能):
    ...
    hit数 = 24 + 3 + 4 + 3
    def 额外刺伤层数(self, 武器类型):
        return min((0.005 * self.等级 + 0.005), 1) * 2 * self.hit数
...
class 极诣剑魂技能17(极诣剑魂主动技能):
    ...
    hit数 = 40 + 3
    def 额外刺伤层数(self, 武器类型):
        return min((0.03 * self.等级 + 0.26), 1) * 1
...
class 极诣剑魂技能25(被动技能):
    ...
    等级上限 = 40
    基础等级 = 11
    关联技能 = ['拔刀斩','破军斩龙击','极鬼剑术暴风式','破空拔刀斩','极神剑术瞬斩','万剑归宗(终结)','万剑归宗(穿云刺)','极拔剑术无形斩','万剑极诣开天斩']
    def 加成倍率(self, 武器类型):
        if self.等级 == 0:
            return 1.0
        else:
    ...
    #破军升龙击     上斩百分比 36 + 4 * self.等级
    #猛龙断空斩     突进攻击力 19 + 2 * self.等级  8段
    #幻影剑舞       斩击攻击力 50 + 5 * self.等级  4段 （不计算武器倍率）
    #幻影剑舞       剑气攻击力 50 + 5 * self.等级  3段 / 短剑4段 （不计算武器倍率）
    #幻影剑舞       钝器捶地   36 + 5 * self.等级  4.2倍剑气
    #极神剑术破空斩 爆炸攻击力  54 + 6 * self.等级  
    #极神剑术流星落 额外百分比  2864.5 + 260.5 * self.等级  3段
    #              巨剑倍率 1.10 + 0.01 *self.等级
    #              钝器额外冲击波 1772.7 + 163.3 *self.等级  3段
    #              光剑额外冲击波 1098.8 + 197.2 *self.等级  3段
    #              太刀刺伤层数  (0.61 + 0.03 * self.等级) * int((self.等级 - 1) / 3 + 3) 
...
class 极诣剑魂技能26(被动技能):
    ...
    #短剑  流心刺 2 + 1       + 0.27 + 0.03 * self.等级 
    #     流心跃 2 * 0.5      + 0.18 + 0.02 * self.等级
    #太刀  流心跃 2 * 0.5        + (0.035 + 0.005 * self.等级) *5段 
    #     流心升 3 * 0.8      + (0.11 + 0.01 * self.等级) *4段
    #钝器  流心刺 2 * 1.25      + 0.45 + 0.05 * self.等级 
    #     流心跃 1      + 0.18 + 0.02 * self.等级
    #巨剑  流心刺 (2 + 1) * (1.09 + 0.01 * self.等级)  + (0.07 + 0.02 * self.等级) * 3段 
    #     流心升 (1 + 1.1) * (0.18 + 0.02 * self.等级)
    #光剑  流心跃 1 * (0.18 + 0.02 * self.等级) 
    #     流心升 3 * 0.8 + (0.44 + 0.04 * self.等级)
...
class 极诣剑魂角色属性(角色属性):
    ...
    def 被动倍率计算(self):
        ...
        if self.武器类型 == '太刀':
            ...
            self.技能栏[self.技能序号['流心跃']].被动倍率 *= 1 + (0.035 + 0.005 * 无形剑意等级) * 5 
            
            self.技能栏[self.技能序号['流心刺']].hit数 +=4
            self.技能栏[self.技能序号['流心刺']].被动倍率 *= 1 + 4 * (0.11 + 0.01 * 无形剑意等级) / (3 * 0.8)
        ...
        if self.武器类型 == '光剑':
            ...
            self.技能栏[self.技能序号['流心升']].被动倍率 *= 1 + (0.44 + 0.04 * 无形剑意等级) / (3 * 0.8)
    ...
    def 伤害计算(self, x = 0):
        ...
        if self.武器类型 == '光剑':
            temp += 技能总伤害[self.技能序号['极神剑术流星落']] / self.技能栏[self.技能序号['极神剑术流星落']].等效百分比(self.武器类型) * 3 * (1098.8 + 197.2 * self.技能栏[极神剑术序号].等级)             
