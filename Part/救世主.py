...
# 多段技能，基础段数为9段；带护石15段，减伤26.56%，护石倍率15/9*0.7344=1.224
class 救世主技能11(主动技能):
    名称 = '杀戮战镰'
    所在等级 = 40
    等级上限 = 60
...
# 二段技能，第一段为无伤害判定，带护石增伤19%，护石倍率1+0.19=1.19
class 救世主技能15(主动技能):
    名称 = '嫉妒之吻'
    所在等级 = 60
    等级上限 = 40
...
# 多段技能，吸收为8段攻击，释放为4段；带护石吸收段数减半伤害+200%（换算为伤害不变，段数12段（0.5*3*8=12）），释放伤害+8%（换算为伤害不变，段数4.32段（1.08*4=4.32））
class 救世主技能16(主动技能):
    名称 = '暴食之噬'
    所在等级 = 70
    等级上限 = 40
...
# 单段技能，由75被动拆分，固定冷却时间2s，此处添加演出时间锁定技能冷却
class 救世主技能18(主动技能):
    名称 = '原罪结晶'
    所在等级 = 75
    等级上限 = 40
...
# 多段技能，刺击为单段伤害，散开为三段伤害。散开伤害根据罪业层数每层+3%，罪业层数最高为7，即散开为3*1.21段伤害
class 救世主技能20(主动技能):
    名称 = '肋骨重塑：原罪战矛'
    所在等级 = 80
    等级上限 = 40
