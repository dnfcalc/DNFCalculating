...
class 真·魔道学者技能4(主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2) * (1 + self.TP成长 * self.TP等级) * self.倍率*1.304/1.13
        


class 真·魔道学者技能5(主动技能):
    名称 = '电鳗碰撞机'
    所在等级 = 35
    等级上限 = 60
    基础等级 = 36
...
class 真·魔道学者技能11(主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2 *1.1) * (1 + self.TP成长 * self.TP等级) * self.倍率*1.18/1.13
    
    
   
class 真·魔道学者技能12(主动技能):
    名称 = '光电兔'
    所在等级 = 70
    等级上限 = 40
    基础等级 = 18
...
class 真·魔道学者技能13(主动技能):
    ...
    护石选项 = ['圣痕']
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2 + self.数据3[self.等级] * self.攻击次数3) * (1 + self.TP成长 * self.TP等级) * self.倍率*1.131/1.13
    def 装备护石(self):
        self.倍率 *= 1.3

class 真·魔道学者技能14(被动技能):
    名称 = '贤者之石'
    所在等级 = 75
    等级上限 = 40
    基础等级 = 11
...
class 真·魔道学者技能15(被动技能):
    名称 = '魔道学助手'
    所在等级 = 75
    等级上限 = 40
    基础等级 = 11
    关联技能 = ['变异苍蝇拍','超级苍蝇拍','改良舒露露','技艺融合','超级棒棒糖','光电兔','雪人刨冰','乌洛波洛斯之环','志高技艺','万圣节南瓜']
    def 加成倍率(self, 武器类型):
        if self.等级 == 0:
            return 1.0
        else:
...
class 真·魔道学者技能17(主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2) * (1 + self.TP成长 * self.TP等级) * self.倍率*1.143/1.13
...
class 真·魔道学者技能18(主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2+self.数据3[self.等级] * self.攻击次数3) * self.倍率*1.155/1.13

    CD = 180.0


class 真·魔道学者技能19(主动技能):
    名称 = '魔法秀'
    所在等级 = 20
    等级上限 = 15
    基础等级 = 10
    是否有伤害 = 0
    冷却关联技能 = ['改良舒露露','熔岩药瓶失败','魔道酸云雨','电鳗碰撞机','暴炎加热炉失败','冰霜钻孔车失败','反重力装置','熔岩药瓶成功','光电兔','雪人刨冰','万圣节南瓜']
...
class 真·魔道学者技能21(主动技能):
    名称 = '万圣节南瓜'
    所在等级 = 95
    等级上限 = 40
    基础等级 = 6
    #数据 = [5463,6017,6571,7125,7679,8233,8787,9341,9895,10449]
    攻击次数 = 26
    基础 = 4909
    成长 = 554
    CD = 60.0
...
while i >= 0:
    try:
        exec('真·魔道学者技能列表.append(真·魔道学者技能'+str(i)+'())')
        i += 1
    except:
        i = -1

真·魔道学者技能序号 = dict()
for i in range(len(真·魔道学者技能列表)):
    真·魔道学者技能序号[真·魔道学者技能列表[i].名称] = i
...
for i in 真·魔道学者技能列表:
    if i.所在等级 == 50:
        真·魔道学者一觉序号 = 真·魔道学者技能序号[i.名称]
    if i.所在等级 == 85:
        真·魔道学者二觉序号 = 真·魔道学者技能序号[i.名称]
    if i.所在等级 == 100:
        真·魔道学者三觉序号 = 真·魔道学者技能序号[i.名称]

真·魔道学者护石选项 = ['无']
for i in 真·魔道学者技能列表:
    if i.是否有伤害 == 1 and i.是否有护石 == 1:
        真·魔道学者护石选项.append(i.名称)

真·魔道学者符文选项 = ['无']
for i in 真·魔道学者技能列表:
    if i.所在等级 >= 20 and i.所在等级 <= 80 and i.所在等级 != 50 and i.是否有伤害 == 1:
        真·魔道学者符文选项.append(i.名称)

class 真·魔道学者角色属性(角色属性):
    ...
    def __init__(self):
        基础属性输入(self)
        self.技能栏= deepcopy(真·魔道学者技能列表)
        self.技能序号= deepcopy(真·魔道学者技能序号)
    
class 真·魔道学者(角色窗口):
    def 窗口属性输入(self):
        self.初始属性 = 真·魔道学者角色属性()
        self.角色属性A = 真·魔道学者角色属性()
        self.角色属性B = 真·魔道学者角色属性()
        self.一觉序号 = 真·魔道学者一觉序号
        self.二觉序号 = 真·魔道学者二觉序号
        self.三觉序号 = 真·魔道学者三觉序号
        self.护石选项 = deepcopy(真·魔道学者护石选项)
        self.符文选项 = deepcopy(真·魔道学者符文选项)
