...
class 不知火技能0(主动技能):
    ...
    学习间隔 = 2
    等级精通 = 50
    基础等级 = min(int((不知火等级 - 所在等级) / 学习间隔), 等级精通) + 1
...
class 不知火技能1(主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2 * self.攻击倍率2 + self.数据3[self.等级] * self.攻击次数3 * self.攻击倍率3) * (1 + self.TP成长 * self.TP等级) * self.倍率
...
class 不知火技能9(主动技能):
    ...
    攻击次数1 = 2
    数据2 = [0, 5810, 6400, 6991, 7581, 8171, 8761, 9349, 9940, 10529, 11119, 11710, 12300, 12888, 13479, 14067, 14656, 15246, 15835, 16424, 17016, 17605, 18195, 18785, 19373, 19964, 20552, 21142, 21733, 22323, 22911, 23502, 24091, 24682, 25271, 25861, 26451, 27041, 27630, 28221, 28809, 29400, 29988, 30577, 31169, 31758, 32348, 32937, 33527, 34116, 34707, 35295, 35888, 36475, 37065, 37655, 38245, 38834, 39425, 40013, 40604, 41194, 41784, 42374, 42964, 43551, 44141, 44731, 45321, 45912, 46501]
    攻击次数2 = 1
    数据3 = [0, 161, 178, 193, 210, 227, 242, 259, 275, 292, 308, 324, 341, 358, 373, 390, 406, 422, 439, 455, 473, 488,505, 522, 538, 554, 571, 587, 604, 619, 636, 653, 668, 685, 702, 718, 734, 750, 767, 784, 799, 816, 832, 848,865, 881, 898, 914, 930, 947, 962, 979, 997, 1013, 1030, 1046, 1062, 1079, 1094, 1111, 1128, 1144, 1160,1177, 1193, 1210, 1225, 1242, 1259, 1274, 1291]
    攻击次数3 = 1
    CD = 21.0
    TP成长 = 0.10
    TP基础 = 5
...
class 不知火技能10(主动技能):
    ...
    等级上限 = 11
    基础等级 = 1
    关联技能 = ['火遁·豪火球之术', '忍法：幻影手里剑', '火遁·飓风煞', '火遁·螺旋手里剑', '忍法：替身术', '火遁·炎天道','火遁·蟾蜍油炎弹','火遁·炎舞天璇','八岐大蛇','火遁·风魔手里剑','天照']
    CD = 31.5
...
class 不知火技能13(被动技能):
    ...
    等级上限 = 40
    基础等级 = 20
    数据 = [0, 131.9, 152.8, 174.1, 194.95, 215.85, 237.15, 258.05, 278.9, 300.2, 321.1, 341.95, 363.25, 384.15, 405, 426.3, 447.2, 468.1, 489.35, 510.25, 531.15, 552.4, 573.3, 594.2, 615.45, 636.35, 657.25, 678.55, 699.4, 720.3, 741.6, 762.45, 783.35, 804.65, 825.5, 846.4, 867.7, 888.6, 909.45, 930.75, 951.65, 972.5, 993.8, 1014.7, 1035.55, 1056.85, 1077.75, 1099.05, 1119.9, 1140.8, 1162.1, 1182.95, 1203.85, 1225.15, 1246, 1266.9, 1288.2, 1309.1, 1329.95, 1351.25, 1372.15, 1393, 1414.3, 1435.2, 1456.05, 1477.35, 1498.25, 1519.15, 1540.4, 1561.3, 1582.2]
    攻击次数 = 1
    攻击倍率 = 0.675
    CD = 1.0
    CD倍率 = 1.0
...
class 不知火技能14(主动技能):
    ...
    学习间隔 = 5
    等级精通 = 30
    基础等级 = min(int((不知火等级 - 所在等级) / 学习间隔 + 1), 等级精通)
...
class 不知火技能21(主动技能):
    ...
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1 + self.数据2[self.等级] * self.攻击次数2 + self.数据3[self.等级] * self.攻击次数3 * self.攻击倍率3) * (1 + self.TP成长 * self.TP等级) * self.倍率
...
while i >= 0:
    try:
        exec('不知火技能列表.append(不知火技能' + str(i) + '())')
        i += 1
    except:
        i = -1

不知火技能序号 = dict()
for i in range(len(不知火技能列表)):
    不知火技能序号[不知火技能列表[i].名称] = i
...
for i in 不知火技能列表:
    if i.所在等级 == 50:
        不知火一觉序号 = 不知火技能序号[i.名称]
    if i.所在等级 == 85:
        不知火二觉序号 = 不知火技能序号[i.名称]
    if i.所在等级 == 100:
        不知火三觉序号 = 不知火技能序号[i.名称]
...
class 不知火角色属性(角色属性):
    ...
    def __init__(self):
        self.技能栏 = deepcopy(不知火技能列表)
        self.技能序号 = deepcopy(不知火技能序号)
    ...
    def 火属性强化技能(self):
        return self.技能栏[self.技能序号['毕方之印']].属强加成()
...
class 不知火(角色窗口):
    def 窗口属性输入(self):
        ...
        self.二觉序号 = 不知火二觉序号
        self.三觉序号 = 不知火三觉序号
        self.护石选项 = deepcopy(不知火护石选项)
        self.符文选项 = deepcopy(不知火符文选项)
