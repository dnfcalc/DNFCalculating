...
class 技能1(主动技能):
    ...
    #基础 = 1721 - 174.644
    #成长 = 174.644
    data0 = [int(i*1.206) for i in [0, 1721, 1895, 2070, 2245, 2420, 2594, 2769, 2944, 3118, 3293, 3468, 3641, 3816, 3992, 4166, 4340, 4516, 4689, 4865, 5039, 5212, 5389, 5563, 5738, 5911, 6087, 6262, 6437, 6610, 6786, 6961, 7134, 7309, 7484,
...
class 技能2(主动技能):
    ...
    基础 = 83.97+98.96+112.99+129.97+143.97+166.99
...
class 技能3(主动技能):
    ...
    等级上限 = 60
    基础等级 = 46
    data0 = [int(i*1.119) for i in [0, 267, 295, 323, 350, 377, 404, 432, 459, 486, 513, 541, 567, 595, 621, 650, 677, 704, 731, 759, 786, 813, 840, 867, 895, 921, 949, 977, 1004, 1031, 1058, 1085, 1113, 1140, 1167, 1194, 1221, 1249, 1275, 1303, 1331, 1358, 1385, 1412, 1440, 1467, 1494, 1521, 1549, 1575, 1603, 1630, 1658, 1685, 1712, 1739, 1767, 1794, 1821, 1848, 1875, 1903, 1930, 1957, 1985, 2012, 2039, 2066, 2094, 2121, 2148]]
    攻击次数 = 6
    CD = 4.0
    TP成长 = 0.1
    TP上限 = 5
...
class 技能10(主动技能):
    ...
    #成长 = 823.406 * 1.103
    data0 = [int(i*1.211) for i in [0, 8953, 9862, 10770, 11679, 12587, 13495, 14404, 15312, 16220, 17129, 18037, 18946, 19854, 20762, 21671, 22579, 23487, 24396, 25304, 26212, 27121, 28029, 28938, 29846, 30754, 31663, 32571, 33479, 34388, 35296, 36205, 37113, 38021, 38930, 39838, 40746, 41655, 42563, 43472, 44380, 45288, 46197, 47105, 48013, 48922, 49830, 50738, 51647, 52555, 53464, 54372, 55280, 56189, 57097, 58005, 58914, 59822, 60731, 61639, 62547, 63456, 64364, 65272, 66181, 67089, 67998, 68906, 69814, 70723, 71631]]
    CD = 20.0
    TP成长 = 0.1
    TP上限 = 5
...
class 技能11(主动技能):
    ...
    #基础 = 13060 - 1326.25
    #成长 = 1326.25
    data0 = [int(i*1.211) for i in [0, 653, 720, 786, 852, 919, 984, 1051, 1117, 1184, 1250, 1316, 1383, 1449, 1516, 1582, 1648, 1713, 1780, 1846, 1913, 1979, 2046, 2111, 2179, 2244, 2311, 2377, 2443, 2510, 2576, 2643, 2709, 2775,
...
class 技能14(主动技能):
    ...
    #成长 = 10407 * 1.102
    data0 = [int(i*1.21) for i in [0, 2167, 2670, 3173, 3676, 4178, 4681, 5184, 5686, 6189, 6692, 7194, 7697, 8200, 8703, 9205, 9708, 10211, 10713, 11216, 11719, 12222, 12724, 13227, 13730, 14232, 14735, 15238, 15740, 16243, 16746, 17249, 17751, 18254, 18757, 19259, 19762, 20265, 20768, 21270, 21773]]
    攻击次数 = 16
    data1 = [int(i*1.21) for i in [0, 14865, 18312, 21760, 25207, 28654, 32101, 35548, 38995, 42442, 45889, 49337, 52784, 56231, 59678, 63125, 66572, 70019, 73466, 76914, 80361, 83808, 87255, 90702, 94149, 97596, 101044, 104491, 107938, 111385, 114832, 118279, 121726, 125173, 128621, 132068, 135515, 138962, 142409, 145856, 149303]]
    攻击次数1 = 1
    CD = 140
...
class 技能15(主动技能):
    ...
    #成长 = 1354.864 * 1.15
    data0 = [int(i*1.211) for i in [0, 1426, 1571, 1716, 1861, 2005, 2150, 2295, 2440, 2584, 2729, 2874, 3019, 3163, 3308, 3453, 3598, 3742, 3887, 4032, 4177, 4321, 4466, 4611, 4756, 4900, 5045, 5190, 5335, 5479, 5624, 5769, 5914, 6058, 6203, 6348, 6493, 6637, 6782, 6927, 7072, 7216, 7361, 7506, 7651, 7795, 7940, 8085, 8230, 8374, 8519, 8664, 8809, 8953, 9098, 9243, 9388, 9532, 9677, 9822, 9967, 10111, 10256, 10401, 10546, 10690, 10835, 10980, 11125, 11269, 11414]]
    攻击次数 = 7
    data1 = [int(i*1.211) for i in [0, 5378, 5923, 6469, 7014, 7560, 8106, 8651, 9197, 9742, 10288, 10834, 11379, 11925, 12470, 13016, 13562, 14107, 14653, 15198, 15744, 16290, 16835, 17381, 17926, 18472, 19018, 19563, 20109, 20654, 21200, 21746, 22291, 22837, 23382, 23928, 24474, 25019, 25565, 26110, 26656, 27202, 27747, 28293, 28839, 29384, 29930, 30475, 31021, 31567, 32112, 32658, 33203, 33749, 34295, 34840, 35386, 35931, 36477, 37023, 37568, 38114, 38659, 39205, 39751, 40296, 40842, 41387, 41933, 42479, 43024]]
    攻击次数1 = 1
    CD = 30.0
    TP成长 = 0.1
    TP上限 = 5
...
class 技能16(主动技能):
    ...
    #成长 = 2464.235 * 1.052
    data0 = [int(i*1.211) for i in [0, 1826, 2011, 2196, 2381, 2566, 2752, 2937, 3122, 3307, 3493, 3678, 3863, 4048, 4234, 4419, 4604, 4789, 4975, 5160, 5345, 5530, 5716, 5901, 6086, 6271, 6457, 6642, 6827, 7012, 7198, 7383, 7568, 7753, 7939, 8124, 8309, 8494, 8680, 8865, 9050, 9235, 9421, 9606, 9791, 9976, 10162, 10347, 10532, 10717, 10903, 11088, 11273, 11458, 11644, 11829, 12014, 12199, 12385, 12570, 12755, 12940, 13126, 13311, 13496, 13681, 13867, 14052, 14237, 14422, 14608]]
    攻击次数 = 5
    data1 = [int(i*1.211) for i in [0, 16434, 18101, 19768, 21435, 23102, 24770, 26437, 28104, 29771, 31439, 33106, 34773, 36440, 38107, 39775, 41442, 43109, 44776, 46444, 48111, 49778, 51445, 53112, 54780, 56447, 58114, 59781, 61449, 63116, 64783, 66450, 68117, 69785, 71452, 73119, 74786, 76453, 78121, 79788, 81455, 83122, 84790, 86457, 88124, 89791, 91458, 93126, 94793, 96460, 98127, 99795, 101462, 103129, 104796, 106463, 108131, 109798, 111465, 113132, 114800, 116467, 118134, 119801, 121468, 123136, 124803, 126470, 128137, 129805, 131472]]
    攻击次数1 = 1
    CD = 50.0
    TP成长 = 0.1
    TP上限 = 5
...
class 技能18(主动技能):
    ...
    #成长 = 4303.067 * 1.111
    data0 = [int(i*1.211) for i in [0, 471, 518, 566, 614, 662, 710, 757, 805, 853, 901, 949, 996, 1044, 1092, 1140, 1188, 1235, 1283, 1331, 1379, 1427, 1474, 1522, 1570, 1618, 1666, 1713, 1761, 1809, 1857, 1905, 1952, 2000, 2048, 2096, 2144, 2191, 2239, 2287, 2335, 2383, 2430, 2478, 2526, 2574, 2622, 2669, 2717, 2765, 2813, 2860, 2908, 2956, 3004, 3052, 3099, 3147, 3195, 3243, 3291, 3338, 3386, 3434, 3482, 3530, 3577, 3625, 3673, 3721, 3769]]
    攻击次数 = 1
    data1 = [int(i*1.211) for i in [0, 3886, 4281, 4675, 5069, 5464, 5858, 6252, 6647, 7041, 7435, 7830, 8224, 8618, 9013, 9407, 9801, 10196, 10590, 10984, 11379, 11773, 12167, 12562, 12956, 13350, 13745, 14139, 14533, 14928, 15322, 15716, 16110, 16505, 16899, 17293, 17688, 18082, 18476, 18871, 19265, 19659, 20054, 20448, 20842, 21237, 21631, 22025, 22420, 22814, 23208, 23603, 23997, 24391, 24786, 25180, 25574, 25969, 26363, 26757, 27152, 27546, 27940, 28335, 28729, 29123, 29518, 29912, 30306, 30700, 31095]]
    攻击次数1 = 12
    CD = 40.0
    是否有护石 = 1
    护石选项 = ['圣痕']
