...
class 技能5(职业主动技能):
    ...
    等级上限 = 30
    基础等级 = 20
    data1 = [int(i*1.0) for i in [0, 5540, 6423, 7306, 8189, 9072, 9956, 10839, 11722, 12605, 13488, 14371, 15255, 16138, 17021, 17904, 18787, 19671, 20554, 21437, 22320, 23203, 24087, 24970, 25853, 26736, 27619, 28503, 29386, 30269, 31152]]
    #基础 = 4154.954
    #成长 = 788.050
    攻击次数1 = 1
    CD = 15.0
...
class 技能6(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 1
    data1 = [int(i*1.0) for i in [0, 2959, 3259, 3559, 3859, 4160, 4460, 4760, 5060, 5360, 5661, 5961, 6261, 6561, 6861, 7162, 7462, 7762, 8062, 8362, 8663, 8963, 9263, 9563, 9864, 10164, 10464, 10764, 11064, 11365, 11665, 11965, 12265, 12565, 12866, 13166, 13466, 13766, 14066, 14367, 14667, 14967, 15267, 15567, 15868, 16168, 16468, 16768, 17069, 17369, 17669, 17969, 18269, 18570, 18870, 19170, 19470, 19770, 20071, 20371, 20671, 20971, 21271, 21572, 21872, 22172, 22472, 22773, 23073, 23373, 23673]]
    #基础 = 2371.133
    #成长 = 267.834
    攻击次数1 = 1
    TP成长 = 0.10
...
class 技能16(职业主动技能):
    ...
    基础等级 = 23
    # 持续伤害
    data1 = [int(i*1.0) for i in [0, 563, 620, 676, 733, 790, 848, 905, 962, 1019, 1076, 1133, 1190, 1248, 1305, 1362, 1419, 1476, 1533, 1589, 1646, 1704, 1761, 1818, 1875, 1932, 1989, 2046, 2103, 2161, 2218, 2275, 2332, 2389, 2446, 2502, 2559, 2617, 2674, 2731, 2788, 2845, 2902, 2959, 3017, 3074, 3131, 3188, 3245, 3302, 3359, 3415, 3473, 3530, 3587, 3644, 3701, 3758, 3815, 3873, 3930, 3987, 4044, 4101, 4158, 4215, 4273, 4329, 4386, 4443, 4500]]
    #基础 = 451.063
    #成长 = 50.909
    攻击次数1 = 1
    # 爆发伤害
    data2 = [int(i*1.0) for i in [0, 18181, 20025, 21870, 23714, 25558, 27403, 29247, 31092, 32936, 34781, 36625, 38469, 40314, 42159, 44003, 45847, 47691, 49536, 51381, 53225, 55069, 56913, 58759, 60603, 62447, 64291, 66135, 67981, 69825, 71669, 73513, 75359, 77203, 79047, 80891, 82735, 84581, 86425, 88269, 90113, 91958, 93803, 95647, 97491, 99335, 101180, 103025, 104869, 106713, 108558, 110402, 112247, 114091, 115936, 117780, 119624, 121469, 123313, 125158, 127002, 128846, 130691, 132536, 134380, 136224, 138069, 139913, 141758, 143602, 145446]]
    #基础2 = 14580.815
    #成长2 = 1646.227
    攻击次数2 = 1
    CD = 30.
...
class 技能17(职业主动技能):
    ...
    def 普通形态(self):
        self.data1 = [int(i*1.0) for i in [0, 27341, 30115, 32889, 35663, 38437, 41211, 43984, 46758, 49532, 52306, 55080, 57854, 60627, 63401, 66175, 68949, 71723, 74497, 77270, 80044, 82818, 85592, 88366, 91139, 93913, 96687, 99461, 102235, 105009, 107782, 110556, 113330, 116104, 118878, 121652, 124425, 127199, 129973, 132747, 135521]]
        self.data2 = [int(i*1.0) for i in [0, 31598, 34804, 38010, 41215, 44421, 47627, 50832, 54038, 57244, 60449, 63655, 66861, 70066, 73272, 76478, 79683, 82889, 86095, 89300, 92506, 95712, 98917, 102123, 105329, 108534, 111740, 114946, 118151, 121357, 124563, 127768, 130974, 134180, 137386, 140591, 143797, 147003, 150208, 153414, 156620]]
        self.攻击次数1 = 1
        self.攻击次数2 = 1

    def 强化形态(self):
        self.data1 = [int(i*1.0) for i in [0, 7989, 8799, 9610, 10420, 11231, 12041, 12852, 13662, 14473, 15283, 16094, 16904, 17715, 18525, 19336, 20146, 20957, 21767, 22578, 23388, 24199, 25009, 25820, 26630, 27441, 28251, 29062, 29872, 30683, 31493, 32304, 33114, 33925, 34735, 35546, 36356, 37167, 37977, 38788, 39598]]
        self.data2 = [int(i*1.0) for i in [0, 5326, 5866, 6406, 6947, 7487, 8027, 8568, 9108, 9648, 10189, 10729, 11269, 11810, 12350, 12890, 13431, 13971, 14511, 15052, 15592, 16132, 16673, 17213, 17753, 18294, 18834, 19374, 19915, 20455, 20995, 21536, 22076, 22616, 23157, 23697, 24237, 24778, 25318, 25858, 26399]]
        self.攻击次数1 = 4
        self.攻击次数2 = 4
...
class 技能19(职业主动技能):
    ...
    基础等级 = 13
    是否有护石 = 1
    data1 = [int(i*1.0) for i in [0, 40782, 44919, 49056, 53194, 57331, 61468, 65606, 69743, 73880, 78017, 82155, 86292, 90429, 94567, 98704, 102841, 106979, 111116, 115253, 119391, 123528, 127665, 131803, 135940, 140077, 144214, 148352, 152489, 156626, 160764, 164901, 169038, 173176, 177313, 181450, 185588, 189725, 193862, 198000, 202137]]
    #基础 = 32790.618
    #成长 = 3702.340
    攻击次数1 = 1
    data2 = [int(i*1.0) for i in [0, 1359, 1497, 1634, 1772, 1910, 2048, 2186, 2324, 2462, 2600, 2738, 2875, 3013, 3151, 3289, 3427, 3565, 3703, 3841, 3979, 4116, 4254, 4392, 4530, 4668, 4806, 4944, 5082, 5220, 5357, 5495, 5633, 5771, 5909, 6047, 6185, 6323, 6461, 6598, 6736]]
    #基础2 = 1090.657
    #成长2 = 123.333
    攻击次数2 = 24
    CD = 45.
...
class 技能20(职业主动技能):
    ...
    等级上限 = 40
    基础等级 = 5
    迷雾data = [int(i*1.0) for i in [0, 6623, 8158, 9694, 11230, 12766, 14302, 15837, 17373, 18909, 20445, 21980, 23516, 25052, 26588, 28124, 29659, 31195, 32731, 34267, 35803, 37338, 38874, 40410, 41946, 43481, 45017, 46553, 48089, 49625, 51160, 52696, 54232, 55768, 57304, 58839, 60375, 61911, 63447, 64982, 66518]]
    炮击data = [int(i*1.0) for i in [0, 5519, 6799, 8078, 9358, 10638, 11918, 13198, 14477, 15757, 17037, 18317, 19597, 20877, 22156, 23436, 24716, 25996, 27276, 28556, 29835, 31115, 32395, 33675, 34955, 36234, 37514, 38794, 40074, 41354, 42634, 43913, 45193, 46473, 47753, 49033, 50313, 51592, 52872, 54152, 55432]]
    普通炮击data = [int(i*1.0) for i in [0, 3294, 4058, 4822, 5587, 6351, 7115, 7879, 8643, 9407, 10171, 10935, 11699, 12463, 13227, 13991, 14755, 15519, 16283, 17047, 17811, 18575, 19339, 20103, 20867, 21631, 22395, 23159, 23923, 24687, 25451, 26215, 26980, 27744, 28508, 29272, 30036, 30800, 31564, 32328, 33092]]
    强化炮击data = [int(i*1.0) for i in [0, 3088, 3804, 4520, 5236, 5953, 6669, 7385, 8101, 8817, 9533, 10250, 10966, 11682, 12398, 13114, 13830, 14547, 15263, 15979, 16695, 17411, 18127, 18844, 19560, 20276, 20992, 21708, 22424, 23141, 23857, 24573, 25289, 26005, 26722, 27438, 28154, 28870, 29586, 30302, 31019]]
    终结伤害 = [int(i*1.0) for i in [0, 109511, 134905, 160299, 185693, 211087, 236481, 261875, 287269, 312663, 338058, 363452, 388846, 414240, 439634, 465028, 490422, 515816, 541210, 566604, 591998, 617392, 642786, 668180, 693574, 718968, 744362, 769756, 795150, 820544, 845938, 871332, 896726, 922120, 947514, 972908, 998302, 1023696, 1049090, 1074484, 1099878]]
    # 不强化
    # 10次迷雾伤害和13次炮击伤害
    攻击次数1 = 10
    攻击次数2 = 12
