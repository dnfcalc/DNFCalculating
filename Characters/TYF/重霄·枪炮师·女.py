...
class 技能2(主动技能):
    ...
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通) + 1
    #1235 1169
    data0 = [(i*1.0565*1.051) for i in [0, 1062, 1169, 1278, 1385, 1492, 1601, 1708, 1817, 1924, 2031, 2140, 2247, 2356, 2463, 2570, 2679, 2786, 2895, 3002, 3109, 3218, 3325, 3433, 3541, 3648, 3757, 3864, 3972, 4080, 4187, 4295, 4403, 4511, 4619, 4726, 4834, 4942, 5050, 5158, 5265, 5373, 5481, 5589, 5696, 5804, 5912, 6020, 6127, 6235, 6343, 6451, 6558, 6666, 6774, 6882, 6990, 7097, 7205, 7313, 7421, 7529, 7636, 7744, 7852, 7959, 8068, 8175, 8283, 8391, 8498]]
    攻击次数 = 2
    CD = 5.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能5(主动技能):
    ...
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通) + 1
    #3163 2693
    data0 = [(i*1.1745*1.052) for i in [0,2444, 2693, 2940, 3188, 3436, 3684, 3933, 4180, 4429, 4677, 4924, 5173, 5420, 5669, 5917, 6164, 6413, 6660, 6909, 7157, 7405, 7653, 7900, 8149, 8397, 8645, 8893, 9140, 9389, 9637, 9885, 10133, 10380, 10629, 10877, 11125, 11373, 11622, 11869, 12117, 12365, 12613, 12862, 13109, 13358, 13605, 13853, 14102, 14349, 14598, 14845, 15093, 15342, 15589, 15838, 16085, 16334, 16582, 16829, 17078, 17325, 17574, 17822, 18069, 18318, 18566, 18814, 19062, 19309, 19558]]
    CD = 7.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能9(主动技能):
    ...
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通) + 1
    #1398 1190
    data0 = [(i*1.1748*1.052) for i in [0, 1080, 1190, 1299, 1410, 1519, 1628, 1739, 1848, 1957, 2068, 2177, 2286, 2396, 2506, 2615, 2725, 2835, 2945, 3054, 3163, 3274, 3383, 3492, 3603, 3712, 3821, 3932, 4041, 4151, 4260, 4370, 4480, 4589, 4698, 4809, 4918, 5027, 5138, 5247, 5356, 5467, 5576, 5686, 5795, 5905, 6015, 6124, 6234, 6344, 6453, 6562, 6673, 6782, 6891, 7002, 7111, 7221, 7331, 7440, 7550, 7659, 7769, 7879, 7988, 8098, 8208, 8317, 8428, 8537, 8646]]
    攻击次数 = 8
    CD = 20.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能10(主动技能):
    ...
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通) + 1
    #873 743
    data1 = [(i*1.1750*1.085) for i in [0,675, 743, 812, 880, 949, 1016, 1085, 1154, 1222, 1291, 1359, 1428, 1497, 1565, 1634, 1701, 1770, 1838, 1907, 1976, 2044, 2113, 2181, 2250, 2317, 2386, 2455, 2523, 2592, 2660, 2729, 2798, 2866, 2935, 3002, 3071, 3140, 3208, 3277, 3345, 3414, 3482, 3551, 3620, 3687, 3756, 3824, 3893, 3962, 4030, 4099, 4167, 4236, 4305, 4372, 4441, 4509, 4578, 4646, 4715, 4784, 4852, 4921, 4989, 5057, 5125, 5194, 5263, 5331, 5400]]
    攻击次数1 = 1
    #10047 8550
    data2 = [(i*1.1751*1.085) for i in [0, 7762, 8550, 9337, 10125, 10913, 11701, 12487, 13275, 14063, 14850, 15638, 16426, 17212, 18000, 18788, 19576, 20363, 21151, 21938, 22725, 23513, 24301, 25089, 25876, 26663, 27451, 28238, 29026, 29814, 30602, 31388, 32176, 32964, 33751, 34539, 35327, 36113, 36901, 37689, 38477, 39264, 40052, 40839, 41626, 42414, 43202, 43990, 44776, 45564, 46352, 47139, 47927, 48715, 49501, 50289, 51077, 51865, 52652, 53440, 54227, 55014, 55802, 56590, 57378, 58165, 58952, 59740, 60527, 61315, 62103]]
    攻击次数2 = 1

    # 感电
    data3 = [(i*1.085) for i in [0, 10, 10, 10, 10, 10, 10, 11, 12, 12, 13, 14, 14, 15, 16, 16, 17, 18, 19, 19, 20, 21, 21, 22, 23, 23, 24, 25, 25, 26, 27, 27, 28, 29, 30, 30, 31, 32, 32, 33, 34, 34, 35, 36, 36, 37, 38, 39, 39, 40, 41, 41,
...
class 技能15(主动技能):
    ...
    攻击次数 = 8
    #205 186
    data1 =[(i*1.1022*1.1) for i in [0,169, 186, 204, 221, 237, 255, 272, 289, 307, 323, 340, 358, 375, 393, 410, 426, 444, 461, 478, 496, 512, 530, 547, 564, 582, 599, 615, 633, 650, 667, 685, 701, 719, 736, 753, 771, 788, 804, 822, 839, 857, 874, 890, 908, 925, 942, 960, 977, 993, 1011, 1028, 1046, 1063, 1079, 1097, 1114, 1131, 1149, 1166, 1182, 1200, 1217, 1235, 1252, 1268, 1286, 1303, 1320, 1338, 1355]]
    攻击次数2 = 24
    # 灼烧攻击力,在点TP之后删除
    data2 = [(i*1.1 for i in [0,28, 31, 34, 36, 39, 42, 45, 48, 51, 53, 56, 59, 62, 65, 68, 70, 73, 76, 79, 82, 85, 87, 90, 93, 96, 99, 102, 104, 107, 110, 113, 116, 119, 121, 124, 127, 130, 133, 136, 138, 141, 144, 147, 150, 153, 155, 158, 161, 164, 167, 170, 172, 175, 178, 181, 184, 187, 189, 192, 195, 198, 201, 204, 206, 209, 212, 215, 218, 221, 223])]
    攻击次数3 = 0
    CD = 50.0
    TP成长 = 0.10
    TP上限 = 5
