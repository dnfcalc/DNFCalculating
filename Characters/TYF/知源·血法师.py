...
class 技能6(职业主动技能):
    ...
    #基础 = 2329.84763635444
    #成长 = 263.1339020924
    data0 = [ int(i*1.0) for i in [0, 2594, 2857, 3120, 3383, 3646, 3909, 4173, 4436, 4699, 4962, 5225, 5488, 5751, 6015, 6278, 6541, 6804, 7067, 7330, 7594, 7857, 8120, 8383, 8646, 8909, 9173, 9436, 9699, 9962, 10225, 10488, 10752, 11015, 11278, 11541, 11804, 12067, 12331, 12594, 12857, 13120, 13383, 13646, 13910, 14173, 14436, 14699, 14962, 15225, 15488, 15752, 16015, 16278, 16541, 16804, 17067, 17331, 17594, 17857, 18120, 18383, 18646, 18910, 19173, 19436, 19699, 19962, 20225, 20489, 20752]]
    CD = 5
    TP成长 = 0.10
    TP上限 = 5
...
class 技能17(职业主动技能):
    ...
    #成长 = 2852.003846153
    #蝙蝠群冲击波攻击力：<data0>%
    data0 = [ int(i*1.0) for i in [0, 11245, 12386, 13527, 14667, 15808, 16949, 18090, 19231, 20372, 21512, 22653, 23794, 24935, 26076, 27217, 28358, 29498, 30639, 31780, 32921, 34062, 35203, 36343, 37484, 38625, 39766, 40907, 42048, 43188, 44329, 45470, 46611, 47752, 48893, 50034, 51174, 52315, 53456, 54597, 55738]]
    #血气之翼物理攻击力：<data1>%
    data1 = [ int(i*1.0) for i in [0, 16869, 18579, 20290, 22001, 23713, 25424, 27135, 28846, 30558, 32269, 33980, 35691, 37403, 39114, 40825, 42537, 44248, 45959, 47670, 49382, 51093, 52804, 54515, 56227, 57938, 59649, 61360, 63072, 64783, 66494, 68206, 69917, 71628, 73339, 75050, 76762, 78473, 80185, 81895, 83607]]
    攻击次数2 = 1
    CD =50
    TP成长 = 0.10
    TP上限 = 5
...
class 技能19(职业主动技能):
    ...
    #基础 = 60756.9115151488
    #成长 = 6859.7572027936
    data0 = [ int(i*1.0) for i in [0, 67617, 74477, 81337, 88196, 95056, 101916, 108776, 115634, 122494, 129354, 136214, 143074, 149933, 156793, 163653, 170513, 177372, 184232, 191092, 197952, 204811, 211671, 218530, 225391, 232250, 239110, 245969, 252830, 259689, 266549, 273409, 280269, 287129, 293988, 300848, 307707, 314568, 321427, 328287, 335145]]
    CD =50
    是否有护石 = 1
    护石选项 = ['圣痕']
    def 装备护石(self, x):
...
class 技能20(职业主动技能):
    ...
    #成长 = 32796.96
    #多段攻击力：<data0>%
    data0 = [ int(i*1.0) for i in [0, 2052, 2527, 3003, 3479, 3955, 4430, 4906, 5382, 5858, 6334, 6810, 7286, 7760, 8236, 8712, 9188, 9663, 10140, 10615, 11091, 11566, 12043, 12518, 12994, 13470, 13946, 14421, 14897, 15373, 15849, 16324, 16801, 17276, 17752, 18228, 18704, 19180, 19655, 20132, 20607]]
    攻击次数 = 35
    #最终一击攻击力：<data1>%
    data1 = [ int(i*1.0) for i in [0, 69761, 85938, 102115, 118291, 134468, 150644, 166821, 182998, 199175, 215351, 231528, 247705, 263881, 280058, 296234, 312411, 328588, 344764, 360941, 377117, 393294, 409471, 425648, 441823, 458000, 474178, 490354, 506531, 522707, 538884, 555061, 571238, 587414, 603590, 619768, 635944, 652121, 668297, 684473, 700651]]
    攻击次数2 = 1
    CD = 180
...
class 技能21(职业主动技能):
    ...
    等级上限 = 40
    基础等级 = 6
    data0 = [ int(i*1.0) for i in [0, 116057, 127831, 139604, 151378, 163152, 174926, 186700, 198474, 210248, 222022, 233796, 245570, 257344, 269117, 280891, 292665, 304439, 316213, 327987, 339761, 351535, 363309, 375083, 386857, 398630, 410404, 422178, 433952, 445726, 457500, 469274, 481048, 492822, 504596, 516370, 528144, 539917, 551691, 563465, 575239, 587013, 598787, 610561, 622335, 634109, 645883, 657657, 669430, 681204, 692978, 704752, 716526, 728300, 740074, 751848, 763622, 775396, 787170, 798943, 810717, 822491, 834265, 846039, 857813, 869587, 881361, 893135, 904909, 916683, 928456]]
    CD = 60.0
