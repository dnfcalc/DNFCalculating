...
class 技能5(主动技能):
    ...
    等级上限 = 60
    基础等级 = 46
    数据 = [int(i*1.105*1.169)for i in [0, 206, 227, 247, 268, 289, 310, 331, 351, 374, 394, 415, 436, 457, 478, 498, 519, 541, 561, 582, 602, 625, 645, 666, 688, 708, 729, 749, 771, 792, 812, 833, 855, 876, 897, 917,
...
class 技能6(主动技能):
    ...
    等级上限 = 60
    基础等级 = 46
    数据 = [int(i*1.102*1.168) for i in [0, 370, 407, 444, 482, 519, 557, 594, 632, 670, 707, 745, 782, 820, 857, 895, 932, 969, 1006, 1044, 1083, 1120, 1158, 1195, 1232, 1269, 1309, 1346, 1383, 1420, 1458, 1495, 1532, 1571, 1608,
...
class 技能8(主动技能):
    ...
    基础等级 = 43
    # 基础 = 2270.46039*1.168
    # 成长 = 258.1335954*1.168

    # 突刺攻击力：<data0>%
    数据 = [int(i*1.105*1.168) for i in [0, 255, 282, 308, 334, 359, 385, 411, 438, 464, 489, 515, 541, 568, 593, 619, 645, 671, 696, 724, 749, 775, 800, 827, 853, 879, 905, 931, 956, 982, 1009, 1035, 1061, 1087, 1112, 1140,
    ...
    攻击次数 = 4
    # 终结横斩攻击力：<data1>%
    数据2 = [int(i*1.105*1.168) for i in [0, 773, 850, 928, 1006, 1085, 1164, 1243, 1321, 1398, 1476, 1556, 1634, 1713, 1791, 1870, 1948, 2026, 2104, 2183, 2261, 2340, 2418, 2497, 2575, 2652, 2731, 2809, 2888, 2966, 3045, 3123, 3201,
    ...
    攻击次数2 = 1
    # 终结射击攻击力：<data2>%
    数据3 = [int(i*1.105*1.168) for i in [0, 247, 273, 299, 323, 349, 374, 399, 424, 449, 475, 499, 526, 549, 576, 601, 626, 651, 676, 701, 726, 752, 778, 802, 828, 852, 878, 902, 928, 954, 979, 1004, 1029, 1055, 1079, 1105,
...
class 技能9(主动技能):
    ...
    等级上限 = 60
    基础等级 = 41
    数据 = [int(i*1.107*1.168) for i in [0, 201, 222, 242, 263, 283, 304, 325, 345, 364, 385, 405, 426, 446, 467, 488, 508, 529, 549, 569, 590, 610, 631, 651, 672, 693, 713, 734, 753, 774, 795, 815, 836, 856, 877, 898,
...
class 技能10(主动技能):
    ...
    # 左右斩击攻击力：<data0>%
    数据 = [int(i*1.16*1.168) for i in [0, 380, 419, 457, 495, 535, 573, 611, 650, 689, 728, 765, 804, 842, 882, 919, 959, 997, 1037, 1074, 1113, 1151, 1191, 1228, 1268, 1306, 1344, 1383, 1421, 1460, 1499, 1537, 1576, 1615, 1653,
    ...
    攻击次数 = 2
    # 旋转斩击攻击力：<data1>%
    数据2 = [int(i*1.16*1.168) for i in [0, 213, 234, 255, 277, 299, 321, 342, 363, 385, 407, 429, 450, 471, 493, 515, 537, 559, 580, 601, 623, 645, 667, 688, 709, 732, 753, 775, 796, 817, 840, 861, 883, 903, 926, 948,
    ...
    攻击次数2 = 5
    # 最终旋斩攻击力：<data2>%
    数据3 = [int(i*1.16*1.168) for i in [0, 1216, 1340, 1464, 1587, 1711, 1834, 1958, 2082, 2204, 2328, 2451, 2575, 2699, 2822, 2946, 3069, 3193, 3316, 3439, 3563, 3686, 3810, 3934, 4057, 4181, 4304, 4426, 4550, 4673, 4797, 4920, 5044, 5168,
...
class 技能11(主动技能):
    ...
    等级上限 = 60
    基础等级 = 38
    数据 = [int(i*1.104*1.169) for i in [0, 134, 148, 162, 176, 189, 203, 217, 231, 244, 258, 272, 285, 299, 313, 327, 340, 354, 368, 382, 395, 408, 423, 437, 449, 464, 478, 490, 505, 519, 532, 546, 560, 573,
...
class 技能12(主动技能):
    ...
    # 周围射击攻击力：<data0>%
    数据 = [int(i*1.104*1.168) for i in [0, 282, 312, 340, 368, 398, 426, 454, 483, 512, 541, 569, 598, 627, 655, 685, 713, 742, 771, 799, 828, 856, 886, 914, 943, 972, 1001, 1029, 1058, 1087, 1115, 1144, 1173, 1202, 1229, 1260,
    ...
    攻击次数 = 15
    # 最终射击攻击力：<data1>%
    数据2 = [int(i*1.104*1.168) for i in [0, 471, 519, 567, 614, 663, 711, 758, 806, 853, 901, 951, 998, 1046, 1093, 1141, 1190, 1237, 1285, 1332, 1380, 1428, 1476, 1524, 1572, 1620, 1668, 1715, 1764, 1812, 1859, 1907, 1954, 2002, 2051,
...
class 技能13(主动技能):
    ...
    等级上限 = 60
    基础等级 = 36
    基础 = 7642.573878*1.162
    成长 = 863.4943577*1.162
    CD = 20
    TP成长 = 0.10
    TP上限 = 5
    ###
...
class 技能14(主动技能):
    ...
    # 斩击攻击力：<data0>%
    数据 = [int(i*1.121*1.168) for i in [0, 318, 350, 384, 415, 448, 481, 512, 544, 577, 610, 642, 673, 707, 739, 771, 803, 836, 868, 900, 934, 965, 997, 1030, 1062, 1095, 1126, 1159, 1192, 1223, 1257, 1289, 1321, 1353, 1385,
    ...
    攻击次数 = 10
    # 射击攻击力：<data1>%
    数据2 = [int(i*1.121*1.168) for i in [0, 219, 242, 265, 285, 309, 333, 353, 376, 399, 421, 443, 466, 488, 510, 533, 555, 577, 600, 622, 644, 667, 689, 712, 734, 755, 779, 800, 823, 846, 868, 890, 913, 935, 956, 980,
    ...
    攻击次数2 = 15
    # 最终横斩攻击力：<data2>%
    数据3 = [int(i*1.121*1.168) for i in [0, 4160, 4582, 5005, 5427, 5849, 6271, 6694, 7115, 7537, 7959, 8381, 8804, 9226, 9649, 10070, 10493, 10915, 11338, 11760, 12182, 12603, 13026, 13448, 13871, 14292, 14715, 15137, 15558, 15981, 16403, 16825, 17247, 17670, 18092,
...
class 技能15(主动技能):
    ...
    数据 = [int(i*1.105*1.168) for i in [0, 16133, 17770, 19406, 21043, 22680, 24317, 25953, 27590, 29226, 30864, 32500, 34137, 35773, 37411, 39048, 40684, 42320, 43957, 45595, 47231, 48867, 50504, 52142, 53778, 55414, 57052, 58688, 60325, 61961, 63599, 65235, 66872, 68508, 70146,
...
class 技能16(主动技能):
    ...
    # 剑术攻击力：<data0>%
    数据 = [int(i*1.103*1.168) for i in [0, 1398, 1723, 2046, 2370, 2695, 3019, 3344, 3668, 3992, 4317, 4642, 4965, 5289, 5614, 5938, 6262, 6587, 6912, 7236, 7559, 7883, 8208, 8532, 8857, 9181, 9505, 9830, 10153, 10478, 10802, 11127, 11451, 11775, 12100, 12425,
    ...
    攻击次数 = 13
    # 射击攻击力：<data1>%
    数据2 = [int(i*1.103*1.168) for i in [0, 1703, 2098, 2494, 2889, 3284, 3680, 4075, 4470, 4866, 5261, 5656, 6051, 6447, 6842, 7237, 7633, 8027, 8422, 8818, 9213, 9608, 10004, 10399, 10794, 11190, 11585, 11980, 12376, 12771, 13166, 13562, 13957, 14351, 14747,
...
class 技能17(主动技能):
    ...
    # 刺击攻击力：<data0>%
    数据 = [int(i*1.105*1.168) for i in [0, 5885, 6482, 7080, 7676, 8273, 8871, 9468, 10066, 10662, 11259, 11857, 12454, 13051, 13648, 14245, 14843, 15440, 16036, 16633, 17230, 17828, 18425, 19022, 19619, 20216, 20814, 21410, 22008, 22605, 23202, 23800, 24395, 24993, 25590,
    ...
    # 射击攻击力：<data1>%
    数据2 = [int(i*1.105*1.168) for i in [0, 440, 486, 531, 576, 621, 664, 709, 754, 799, 844, 889, 934, 979, 1023, 1067, 1112, 1158, 1203, 1247, 1291, 1336, 1381, 1426, 1470, 1516, 1561, 1606, 1651, 1694, 1739, 1784, 1829, 1874, 1919,
...
class 技能18(主动技能):
    ...
    基础 = (795.9230769 + 832.6695157)*1.1307*1.168
    成长 = (90.02136752 + 94.03418803)*1.1307*1.168
    攻击次数 = 10

    基础2 = 6902.951567*1.1298*1.168
    成长2 = 779.4108669*1.1298*1.168
    攻击次数2 = 1
    CD = 50
    TP成长 = 0.10
    TP上限 = 5
...
class 技能19(主动技能):
    ...
    # 移动射击攻击力：<data0>%
    数据 = [int(i*1.117*1.167) for i in [0, 1702, 1876, 2048, 2221, 2394, 2567, 2740, 2912, 3085, 3258, 3431, 3604, 3777, 3948, 4121, 4296, 4467, 4640, 4813, 4986, 5159, 5331, 5504, 5677, 5850, 6023, 6196, 6368, 6541, 6715, 6887, 7059, 7232, 7406,
    ...
    # 后方射击攻击力：<data2>%
    数据2 = [int(i*1.117*1.167) for i in [0, 1705, 1878, 2050, 2224, 2396, 2570, 2743, 2915, 3089, 3262, 3433, 3608, 3781, 3952, 4127, 4299, 4471, 4646, 4817, 4990, 5164, 5336, 5509, 5683, 5855, 6028, 6202, 6374, 6548, 6721, 6893, 7067, 7239, 7412,
    ...
    # 原地范围射击攻击力：<data1>%
    数据3 = [int(i*1.117*1.167) for i in [0, 2554, 2813, 3073, 3332, 3591, 3851, 4110, 4368, 4628, 4887, 5146, 5405, 5665, 5924, 6183, 6443, 6702, 6960, 7221, 7479, 7738, 7998, 8257, 8516, 8776, 9035, 9294, 9553, 9812, 10070, 10331, 10589, 10848, 11108, 11367,
...
class 技能20(主动技能):
    ...
    # 成长 = 1708.524812
    CD = 50
    数据 = [int(i*1.10416*1.168) for i in [0, 16841, 18551, 20259, 21967, 23676, 25384, 27093, 28802, 30510, 32218, 33927, 35635, 37344, 39053, 40761, 42469, 44178, 45887, 47595, 49304, 51013, 52722, 54430, 56139, 57848, 59556, 61264, 62973, 64681, 66390, 68099, 69807, 71515, 73224, 74933,
...
class 技能21(主动技能):
    ...
    CD = 180
    # 剑术第1~4击攻击力：<data0>%
    数据 = [int(i*1.10633*1.168) for i in [0, 7958, 9803, 11648, 13494, 15339, 17184, 19030, 20876, 22721, 24567, 26412, 28256, 30103, 31948, 33793, 35639, 37484, 39329, 41175, 43021, 44866, 46712, 48557, 50401, 52248, 54093, 55938, 57784, 59629, 61474, 63320, 65166, 67011, 68857, 70702,
    ...
    攻击次数 = 4
    # 射击攻击力：<data1>%
    数据2 = [int(i*1.10633*1.168) for i in [0, 1929, 2376, 2823, 3270, 3719, 4165, 4613, 5060, 5508, 5954, 6403, 6850, 7297, 7745, 8192, 8639, 9088, 9534, 9982, 10429, 10876, 11323, 11770, 12219, 12665, 13113, 13560, 14008, 14454, 14903, 15350, 15797, 16245, 16693,
    ...
    攻击次数2 = 11
    # 最终齐射攻击力：<data2>%
    数据3 = [int(i*1.10633*1.168) for i in [0, 2122, 2613, 3106, 3597, 4091, 4582, 5074, 5566, 6059, 6551, 7042, 7535, 8026, 8520, 9011, 9503, 9995, 10488, 10980, 11471, 11964, 12455, 12949, 13440, 13932, 14424, 14917, 15409, 15900, 16393, 16884, 17378, 17869, 18361,
    ...
    攻击次数3 = 20
    # 最终齐射爆炸攻击力：<data3>%
    数据4 = [int(i*1.10633*1.168) for i in [0, 10610, 13071, 15531, 17992, 20453, 22914, 25374, 27835, 30295, 32755, 35216, 37676, 40137, 42598, 45059, 47519, 49980, 52440, 54900, 57361, 59821, 62282, 64742, 67204, 69664, 72125, 74585, 77045, 79506, 81966, 84427, 86887, 89349, 91809, 94270,
