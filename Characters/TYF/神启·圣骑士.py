...
class 神启·圣骑士技能1(主动技能):
    ...
    BUFF魔攻 = 0
    BUFF独立 = 0
    三攻 = [0, 38, 40, 42, 43, 44, 46, 48, 49, 51, 52, 53, 55, 57, 58, 60, 61, 62, 64, 66, 68, 69, 70, 72, 74, 75, 77, 78, 79, 81, 83, 84, 86, 87, 88, 90, 92, 93, 95, 96, 98, 100, 101, 103, 104, 105, 107, 109, 110, 112, 113, 114, 116, 118, 119, 121, 122, 123, 126, 127, 129, 130, 131, 133, 135, 136, 138, 139, 140, 143, 144]
    力智 = [0, 148, 158, 169, 179, 189, 198, 208, 218, 228, 239, 249, 259, 269, 279, 290, 299, 309, 319, 329, 339, 349, 360, 370, 380, 390, 399, 409, 420, 430, 440, 450, 460, 470, 481, 491, 500, 510, 520, 530, 541, 551, 561, 571, 581, 592, 601, 611, 621, 631, 641, 651, 662, 672, 682, 692, 701, 711, 722, 732, 742, 752, 762, 772, 783, 793, 802, 812, 822, 832, 843]
    ...
    def 技能面板(self):
        temp = []
        temp.append(self.名称)
        temp.append(int(round(((self.力智[self.等级]) + self.BUFF力量) * self.BUFF力量per)))
        temp.append(int(round(((self.力智[self.等级]) + self.BUFF智力) * self.BUFF智力per)))
        temp.append(int(round(((self.三攻[self.等级]) + self.BUFF物攻) * self.BUFF物攻per)))
        temp.append(int(round(((self.三攻[self.等级]) + self.BUFF魔攻) * self.BUFF魔攻per)))
        temp.append(int(round(((self.三攻[self.等级]) + self.BUFF独立) * self.BUFF独立per)))
        return temp
