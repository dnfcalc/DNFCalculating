...
class 技能7(主动技能):
    ...
    基础等级 = 1
    # 7.23职业平衡
    基础 = [int(i*1.1878) for i in [0, 1257, 1257, 1257, 1257,
                                  1257, 1257, 1257, 1257, 1257, 1257, 1257]]
    涂毒基础 = [int(i*1.189) for i in [0, 127, 127, 127,
...
class 技能16(主动技能):
    ...
    CD = 30.0
    # 7.23职业平衡
    基础 = [int(i*1.1863*0.94) for i in [0, 10088, 11115, 12137, 13160, 14184, 15208, 16232, 17256, 18279, 19302, 20327, 21352, 22374, 23400, 24422, 25445, 26469, 27493, 28516, 29540, 30565, 31587, 32610, 33636, 34657, 35682, 36705, 37730, 38751, 39776, 40801, 41824, 42846, 43871,
    ...
    直伤倍率 = 1.6  # 异常增伤
    中毒基础 = [int(i*1.1862*0.94) for i in [0, 1596, 1758, 1920, 2083, 2245, 2406, 2568, 2731, 2894, 3055, 3216, 3380, 3541, 3703, 3865, 4029, 4189, 4351, 4514, 4675, 4836, 4999, 5160, 5323, 5485, 5646, 5808, 5971, 6134, 6295, 6457, 6620, 6781, 6943, 7105, 7269,
    ...
    中毒倍率 = 1.6
    涂毒基础 = [int(i*1.1864*0.94) for i in [0, 1167, 1286, 1404, 1524, 1643, 1760, 1880, 1998, 2113, 2234, 2355, 2471, 2592, 2710, 2828, 2947, 3065, 3182, 3304, 3421, 3539, 3660, 3776, 3895, 4014, 4131, 4251, 4369, 4488, 4607, 4728, 4842, 4961, 5081,
...
class 技能27(主动技能):
    ...
    # 7.23职业平衡
    # 直伤1(命中敌人)
    基础1 = [int(i*1.1994) for i in [0, 28772, 31691, 34610, 37529, 40448, 43367, 46286, 49205, 52124, 55043, 57962, 60881, 63800, 66719, 69638, 72557, 75476, 78395, 81314,
    ...
    直伤倍率1 = 1
    # 直伤2(爆炸*5)
    基础2 = [int(i*1.1994) for i in [0, 6739, 7422, 8106, 8790, 9474, 10157, 10841, 11525, 12208, 12892, 13576, 14259, 14943, 15627, 16311, 16994, 17678, 18362,
    ...
    直伤倍率2 = 5
    # 出血(命中敌人)
    出血基础 = [int(i*1.1993) for i in [0, 9590, 10563, 11536, 12509, 13482, 14455, 15428, 16401, 17374, 18347, 19320, 20293, 21266, 22239, 23212, 24185, 25158, 26131,
    ...
    出血倍率 = 1
    # 中毒(爆炸*5)
    中毒基础 = [int(i*1.1993) for i in [0, 1684, 1855, 2026, 2197, 2368, 2539, 2710, 2881, 3052, 3223, 3394, 3564, 3735, 3906, 4077, 4248, 4419,
                                    4590, 4761, 4932, 5103, 5274, 5445, 5616, 5786, 5957, 6128, 6299, 6470, 6641, 6812, 6983, 7154, 7325, 7496, 7667, 7838, 8008, 8179, 8350]]
    中毒倍率 = 5
    # 灼伤(爆炸*5)
    灼伤基础 = [int(i*1.1993) for i in [0, 1684, 1855, 2026, 2197, 2368, 2539, 2710, 2881, 3052, 3223, 3394, 3564, 3735, 3906, 4077, 4248, 4419,
                                    4590, 4761, 4932, 5103, 5274, 5445, 5616, 5786, 5957, 6128, 6299, 6470, 6641, 6812, 6983, 7154, 7325, 7496, 7667, 7838, 8008, 8179, 8350]]
    灼伤倍率 = 5
    # 涂毒1(命中敌人)
    涂毒基础1 = [int(i*1.1994) for i in [0, 3836, 4225, 4614, 5003, 5393, 5782, 6171, 6560, 6949, 7339, 7728, 8117, 8506, 8895, 9285, 9674, 10063, 10452, 10841,
    ...
    涂毒倍率1 = 1
    # 涂毒2(爆炸*5)
    涂毒基础2 = [int(i*1.1996) for i in [0, 1010, 1113, 1216, 1318, 1421, 1523, 1626, 1728, 1831, 1933, 2036, 2138, 2241, 2344, 2446, 2549, 2651,
...
class 技能28(主动技能):
    ...
    # 7.23职业平衡
    # 直伤(解除束缚)
    基础1 = [int(i*1.2054) for i in [0, 7125, 8777, 10429, 12081, 13734, 15386, 17038, 18690, 20343, 21995, 23647, 25299, 26951, 28604, 30256, 31908, 33560, 35213,
    ...
    直伤倍率1 = 1
    # 中毒(毒气)
    中毒基础 = [int(i*1.2054) for i in [0, 21375, 26332, 31288, 36245, 41202, 46159, 51115, 56072, 61029, 65985, 70942, 75899, 80855, 85812, 90769, 95725, 100682, 105639, 110595,
    ...
    中毒倍率 = 1
    # 直伤2(膝踢)
    基础2 = [int(i*1.2054) for i in [0, 28500, 35109, 41718, 48327, 54936, 61545, 68154, 74763, 81372, 87980, 94589, 101198, 107807, 114416, 121025, 127634, 134243, 140852, 147460,
    ...
    直伤倍率2 = 1
    # 直伤3(下劈)
    基础3 = [int(i*1.2054) for i in [0, 42751, 52664, 62577, 72491, 82404, 92318, 102231, 112144, 122058, 131971, 141884, 151798, 161711, 171624, 181538, 191451, 201364, 211278, 221191,
    ...
    直伤倍率3 = 1
    # 直伤4(毒蛇群)
    基础4 = [int(i*1.2054) for i in [0, 8906, 10971, 13037, 15102, 17167, 19232, 21298, 23363, 25428, 27494, 29559, 31624, 33689, 35755, 37820, 39885, 41950, 44016,
    ...
    直伤倍率4 = 8
    # 直伤5(最后一击)
    基础5 = [int(i*1.2054) for i in [0, 114003, 140439, 166874, 193310, 219745, 246181, 272616, 299052, 325488, 351923, 378359, 404794, 431230, 457666, 484101, 510537, 536972, 563408, 589843,
    ...
    直伤倍率5 = 1
    # 涂毒1(膝踢)
    涂毒基础1 = [int(i*1.2055) for i in [0, 4275, 5266, 6257, 7249, 8240, 9231, 10223, 11214, 12205, 13197, 14188, 15179, 16171, 17162, 18153, 19145, 20136, 21127,
    ...
    涂毒倍率1 = 1
    # 涂毒2(下劈)
    涂毒基础2 = [int(i*1.2054) for i in [0, 5700, 7021, 8343, 9665, 10987, 12309, 13630, 14952, 16274, 17596, 18917, 20239, 21561, 22883, 24205, 25526, 26848, 28170,
    ...
    涂毒倍率2 = 1
    # 涂毒3(毒蛇群)
    涂毒基础3 = [int(i*1.2051) for i in [0, 890, 1097, 1303, 1510, 1716, 1923, 2129, 2336, 2542, 2749, 2955, 3162, 3368, 3575, 3782, 3988, 4195,
                                     4401, 4608, 4814, 5021, 5227, 5434, 5640, 5847, 6053, 6260, 6466, 6673, 6879, 7086, 7293, 7499, 7706, 7912, 8119, 8325, 8532, 8738, 8945]]
    涂毒倍率3 = 8
    # 涂毒4(最后一击)
    涂毒基础4 = [int(i*1.2054) for i in [0, 11400, 14043, 16687, 19331, 21974, 24618, 27261, 29905, 32548, 35192, 37835, 40479, 43123, 45766, 48410, 51053, 53697, 56340,
