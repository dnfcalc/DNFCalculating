...
class 技能9(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 43
    data0 = [(i*1.0) for i in [0, 318, 350, 383, 415, 447, 480, 512, 545, 577, 610, 642, 674, 707, 739, 772, 804, 836, 869, 901, 934, 966, 997, 1031, 1062, 1096, 1127, 1159, 1192, 1224, 1257, 1289, 1322, 1354, 1386, 1419, 1451, 1484, 1516, 1548, 1581, 1613, 1646, 1678, 1710, 1743, 1775, 1808, 1840, 1872, 1905, 1937, 1970, 2002, 2035, 2067, 2098, 2132, 2163, 2196, 2228, 2260, 2293, 2325, 2358, 2390, 2422, 2455, 2487, 2520, 2552]]
    攻击次数 = 6
    # 基础 = 1466.4
    # 成长 = 165.6
    CD = 6
...
class 技能13(职业主动技能):
    ...
    # 基础 = 2420.6
    # 成长 = 273.4
    data0 = [(i*1.0) for i in [0, 3313, 3650, 3986, 4323, 4659, 4996, 5332, 5667, 6004, 6340, 6677, 7013, 7349, 7686, 8022, 8359, 8694, 9030, 9367, 9703, 10040, 10376, 10712, 11049, 11384, 11721, 12057, 12393, 12730, 13066, 13403, 13739, 14074, 14411, 14747, 15084, 15420, 15757, 16093, 16429, 16766, 17101, 17438, 17774, 18110, 18447, 18783, 19120, 19456, 19791, 20128, 20464, 20801, 21137, 21473, 21810, 22146, 22483, 22818, 23154, 23491, 23827, 24164, 24500, 24836, 25173, 25508, 25845, 26181, 26517]]
    CD = 10
    TP成长 = 0.10
    TP上限 = 5
    # 攻击倍率 = 1.23
...
class 技能16(职业主动技能):
    ...
    # 成长 = 395.6
    # 战意攻击力： <data0>%
    data0 = [(i*1.0) for i in [0, 1718, 1894, 2068, 2243, 2417, 2591, 2766, 2940, 3114, 3289, 3463, 3639, 3813, 3987, 4162, 4336, 4510, 4685, 4859, 5035, 5209, 5383, 5558, 5732, 5907, 6081, 6255, 6430, 6604, 6778, 6954, 7128, 7303, 7477, 7651, 7826, 8000, 8174, 8349, 8523, 8699, 8873, 9047, 9222, 9396, 9570, 9745, 9919, 10095, 10269, 10443, 10618, 10792, 10967, 11141, 11315, 11490, 11664, 11838, 12014, 12188, 12363, 12537, 12711, 12886, 13060, 13234, 13410, 13584, 13759]]
    # 敌人断开时伤害：<data1>%
    data1 = [(i*1.0) for i in [0, 2927, 3225, 3522, 3819, 4115, 4413, 4710, 5007, 5304, 5601, 5899, 6196, 6492, 6789, 7087, 7384, 7681, 7978, 8276, 8572, 8869, 9166, 9464, 9761, 10058, 10354, 10652, 10949, 11246, 11543, 11841, 12138, 12434, 12731, 13029, 13326, 13623, 13920, 14218, 14514, 14811, 15108, 15405, 15703, 16000, 16297, 16593, 16891, 17188, 17485, 17782, 18080, 18377, 18673, 18970, 19268, 19565, 19862, 20159, 20457, 20753, 21050, 21347, 21645, 21942, 22239, 22535, 22833, 23130, 23427]]
    攻击次数2 = 1
    CD = 15
    TP成长 = 0.10
    TP上限 = 5
...
class 技能17(职业主动技能):
    ...
    # 成长 = 803.7
    # 刺击攻击力：<data0>%
    data0 = [(i*1.0) for i in [0, 2854, 3143, 3432, 3722, 4011, 4301, 4590, 4880, 5170, 5460, 5749, 6038, 6328, 6617, 6907, 7196, 7487, 7776, 8065, 8355, 8644, 8934, 9223, 9513, 9803, 10092, 10382, 10671, 10961, 11250, 11540, 11829, 12118, 12409, 12698, 12988, 13277, 13567, 13856, 14145, 14436, 14725, 15015, 15304, 15594, 15883, 16172, 16462, 16751, 17042, 17331, 17621, 17910, 18200, 18489, 18778, 19069, 19358, 19648, 19937, 20227, 20516, 20805, 21095, 21385, 21675, 21964, 22254, 22543, 22832]]
    攻击次数 = 2
    # 旋转攻击力：<data1>%
    data1 = [(i*1.0) for i in [0, 306, 338, 368, 400, 431, 462, 493, 525, 555, 587, 618, 649, 681, 712, 743, 774, 806, 836, 868, 898, 930, 961, 992, 1023, 1055, 1086, 1117, 1149, 1179, 1211, 1241, 1273, 1304, 1335, 1366, 1398, 1428, 1460, 1492, 1522, 1554, 1585, 1616, 1647, 1679, 1709, 1741, 1771, 1803, 1835, 1865, 1897, 1928, 1959, 1990, 2022, 2052, 2084, 2115, 2146, 2177, 2209, 2240, 2271, 2303, 2333, 2365, 2395, 2427, 2458]]
    攻击次数2 = 10
    CD = 25
    演出时间 = 2
    TP成长 = 0.10
...
class 技能22(职业主动技能):
    ...
    # 成长 = 2516.4
    # [剑盾猛攻]连锁伤害额外增加量：+5%
    data0 = [(i*1.0) for i in [0, 29988, 33031, 36072, 39115, 42157, 45199, 48242, 51284, 54327, 57369, 60412, 63454, 66496, 69539, 72581, 75624, 78666, 81709, 84750, 87792, 90835, 93877, 96920, 99962, 103004, 106047, 109089, 112132, 115174, 118217, 121259, 124300, 127343, 130385, 133428, 136470, 139513, 142555, 145597, 148640]]
    CD = 40
    # 攻击倍率 = 1.209
    是否有护石 = 1
