...
class 技能5(职业主动技能):
    ...
    # 成长 = 195.9
    # 第1次射击攻击力：<data0>%
    data0 = [int(i*1.114*1.189) for i in [0, 771, 850, 928, 1006, 1085, 1163, 1240, 1319, 1397, 1476, 1554, 1632, 1711, 1789, 1867, 1946, 2024, 2103, 2181, 2259, 2338, 2416, 2493, 2572, 2650, 2728, 2807, 2885, 2963, 3042, 3120, 3199, 3277,
    ...
    # 第2次射击攻击力：<data1>%
    data1 = [int(i*1.114*1.189) for i in [0, 1158, 1274, 1392, 1509, 1627, 1744, 1862, 1979, 2097, 2215, 2332, 2448, 2566, 2684, 2801, 2919, 3036, 3154, 3271, 3389, 3506, 3624, 3740, 3858, 3976, 4093, 4211, 4328, 4446, 4563, 4681, 4798,
...
class 技能7(职业主动技能):
    ...
    成长 = 451
    # 击退攻击力：<data0>%
    data0 = [int(i*1.124*1.189) for i in [0, 445, 491, 536, 581, 626, 672, 717, 762, 807, 852, 897, 942, 989, 1034, 1079, 1124, 1169, 1214, 1259, 1304, 1351, 1396, 1441, 1486, 1531, 1576, 1621, 1667, 1712, 1757, 1803, 1848, 1893,
    ...
    # 爆炸攻击力：<data1>%
    data1 = [int(i*1.124*1.189) for i in [0, 1263, 1391, 1519, 1648, 1775, 1904, 2032, 2160, 2288, 2417, 2544, 2673, 2802, 2929, 3058, 3185, 3314, 3441, 3570, 3698, 3826, 3955, 4083, 4211, 4339, 4468, 4595, 4724, 4851, 4980, 5109, 5236,
    ...
    攻击次数2 = 3
    # 特殊灼伤攻击力：<data2>%
    data2 = [int(i*1.124*1.189) for i in [0, 31, 35, 37, 41, 44, 48, 50, 54, 57, 61, 63, 67, 70, 73, 76, 80, 82, 86, 89, 93, 95, 99, 102, 106, 108, 112, 115, 118, 122, 125, 129, 131, 135, 138,
...
class 技能8(职业主动技能):
    ...
    # 基础 = 3084
    # 成长 = 351
    data0 = [int(i*1.114*1.189) for i in [0, 3454, 3804, 4155, 4506, 4857, 5207, 5558, 5908, 6258, 6609, 6959, 7310, 7660, 8010, 8361, 8711, 9062, 9412, 9763, 10113, 10465, 10815, 11165, 11516, 11866, 12217, 12567, 12918, 13268, 13618, 13969, 14319, 14670, 15020,
...
class 技能9(职业主动技能):
    ...
    # 成长 = 392
    # 爆炸攻击力：<data0>%
    data0 = [int(i*1.166*1.189) for i in [0, 3690, 4065, 4439, 4814, 5189, 5563, 5938, 6311, 6686, 7061, 7435, 7810, 8185, 8559, 8933, 9307, 9682, 10057, 10431, 10806, 11179, 11554, 11929, 12303, 12678, 13053, 13427, 13801, 14175, 14550, 14925, 15299, 15674, 16049,
    ...
    # 特殊灼伤攻击力：<data1>%
    data1 = [int(i*1.166*1.189) for i in [0, 27, 30, 32, 36, 39, 41, 44, 46, 50, 53, 55, 58, 61, 63, 67, 70, 72, 75, 77, 81, 84, 86, 89, 91, 95, 98, 100, 103, 106, 108, 112, 115, 117, 120,
...
class 技能10(职业主动技能):
    ...
    # 成长 = 757
    # 第一次爆炸攻击力：<data0>%
    data0 = [int(i*1.1*1.182) for i in [0, 1492, 1644, 1795, 1947, 2099, 2249, 2401, 2552, 2704, 2856, 3006, 3158, 3309, 3461, 3613, 3763, 3915, 4066, 4218, 4370, 4520, 4672, 4823, 4975, 5127, 5277, 5429, 5580, 5733, 5885, 6036, 6188, 6338, 6490,
    ...
    # 第二次爆炸攻击力：<data1>%
    data1 = [int(i*1.1*1.182) for i in [0, 2239, 2466, 2693, 2920, 3147, 3375, 3601, 3829, 4056, 4282, 4510, 4737, 4965, 5191, 5418, 5647, 5874, 6101, 6328, 6555, 6783, 7009, 7237, 7464, 7690, 7918, 8145, 8373, 8599, 8826, 9054, 9280, 9508, 9735, 9961,
    ...
    攻击次数2 = 1
    # 第三次爆炸攻击力：<data2>%
    data2 = [int(i*1.1*1.182) for i in [0, 3731, 4110, 4488, 4867, 5245, 5624, 6004, 6382, 6761, 7139, 7518, 7896, 8275, 8653, 9032, 9410, 9789, 10167, 10546, 10924, 11304, 11683, 12061, 12440, 12818, 13197, 13575, 13954, 14332, 14711, 15090, 15468, 15847, 16225,
...
class 技能11(职业主动技能):
    ...
    # 射击攻击力：<data0>%
    data0 = [int(i*1.1*1.179) for i in  [0, 478, 524, 573, 622, 671, 720, 767, 815, 864, 913, 962, 1009, 1058, 1106, 1155, 1204, 1251, 1300, 1349, 1397, 1445, 1494, 1542, 1591, 1639, 1687, 1736, 1784, 1833, 1882, 1929, 1978, 2027, 2075, 2124,
    ...
    # 爆炸攻击力：<data1>%
    data1 = [int(i*1.1*1.179) for i in  [0, 1432, 1576, 1723, 1867, 2012, 2158, 2303, 2449, 2594, 2739, 2885, 3029, 3175, 3320, 3466, 3611, 3757, 3901, 4048, 4192, 4338, 4483, 4629, 4774, 4918, 5064, 5209, 5355, 5499, 5646, 5790, 5937, 6081, 6226, 6372,
    ...
    攻击次数2 = 6
    # 特殊灼伤攻击力：<data2>%
    data2 = [int(i*1.1*1.179) for i in  [0, 68, 75, 81, 88, 95, 103, 109, 116, 124, 130, 136, 143, 151, 157, 165, 171, 179, 185, 192, 198, 206, 212, 220, 227, 234, 241, 247, 254, 261, 268, 274, 282, 290, 296,
...
class 技能12(职业主动技能):
    ...
    # 爆炸攻击力：<data0>%
    data0 = [int(i*1.158*1.189) for i in [0, 10610, 11687, 12763, 13839, 14917, 15993, 17070, 18146, 19222, 20299, 21375, 22451, 23528, 24605, 25682, 26758, 27834, 28911, 29987, 31064, 32140, 33216, 34293, 35370, 36447, 37523, 38599, 39676, 40752, 41828, 42905, 43981, 45059,
...
class 技能13(职业主动技能):
    ...
    # 成长 = 1935
    # 爆炸攻击力：<data0>%
    data0 = [int(i*1.124*1.189) for i in [0, 3176, 3498, 3821, 4143, 4465, 4788, 5110, 5432, 5755, 6077, 6399, 6722, 7044, 7366, 7689, 8011, 8333, 8656, 8978, 9300, 9623, 9945, 10267, 10590, 10912, 11233, 11557, 11878, 12200, 12524, 12845, 13167, 13490, 13812,
...
class 技能14(职业主动技能):
    ...
    # 首次爆炸攻击力：<data0>%
    data0 = [int(i*1.114*1.189) for i in [0, 4525, 5575, 6624, 7675, 8724, 9773, 10823, 11872, 12922, 13971, 15021, 16070, 17120, 18169, 19218, 20269, 21318, 22368, 23417, 24467, 25516, 26566, 27615, 28664, 29714, 30763, 31814, 32863, 33913, 34962, 36012, 37061, 38110,
    ...
    # 特殊灼伤攻击力：<data1>%
    data1 = [int(i*1.114*1.189) for i in [0, 322, 397, 472, 547, 622, 697, 773, 848, 923, 998, 1073, 1148, 1223, 1298, 1373, 1448, 1523, 1598, 1673, 1748, 1822, 1897, 1972, 2047, 2122, 2197, 2272, 2347, 2422, 2497, 2572, 2647, 2722,
    ...
    攻击次数2 = 7
    # 霰弹枪射击攻击力：<data2>%
    data2 = [int(i*1.114*1.189) for i in [0, 3960, 4878, 5796, 6715, 7633, 8551, 9469, 10388, 11306, 12225, 13144, 14062, 14980, 15898, 16817, 17735, 18653, 19571, 20490, 21408, 22326, 23244, 24164, 25082, 26000, 26918, 27837, 28755, 29673, 30592, 31510, 32428, 33346,
    ...
    攻击次数3 = 4
    # 最终爆炸攻击力：<data3>%
    data3 = [int(i*1.114*1.189) for i in [0, 22630, 27877, 33126, 38373, 43621, 48868, 54115, 59364, 64611, 69859, 75106, 80355, 85602, 90849, 96097, 101344, 106593, 111840, 117087, 122335, 127583, 132831, 138078, 143326, 148573, 153822, 159069, 164316, 169564, 174812, 180060, 185307, 190554, 195802,
...
class 技能15(职业主动技能):
    ...
    # 旋转爆炸攻击力：<data0>%
    data0 = [int(i*1.155*1.189) for i in [0, 3976, 4379, 4783, 5186, 5589, 5993, 6396, 6799, 7203, 7606, 8009, 8412, 8817, 9220, 9623, 10027, 10430, 10833, 11237, 11640, 12043, 12446, 12850, 13253, 13656, 14061, 14464, 14867, 15271, 15674, 16077, 16481, 16884, 17287,
    ...
    # 最终爆炸攻击力：<data1>%
    data1 = [int(i*1.155*1.189) for i in [0, 9278, 10218, 11160, 12101, 13042, 13983, 14925, 15865, 16807, 17748, 18690, 19630, 20572, 21514, 22454, 23396, 24337, 25279, 26219, 27161, 28102, 29043, 29984, 30926, 31867, 32808, 33749, 34691, 35631, 36573, 37514, 38456, 39396,
...
class 技能16(职业主动技能):
    ...
    # 第一次射击攻击力：<data0>%
    data0 = [int(i*1.1466*1.189) for i in [0, 5647, 6220, 6794, 7367, 7940, 8512, 9085, 9658, 10232, 10805, 11377, 11950, 12523, 13097, 13670, 14242, 14815, 15388, 15962, 16535, 17108, 17680, 18253, 18827, 19400, 19973, 20545, 21118, 21692, 22265, 22838, 23410, 23983,
    ...
    # 第二次射击攻击力：<data1>%
    data1 = [int(i*1.1466*1.189) for i in [0, 8472, 9332, 10190, 11050, 11910, 12769, 13628, 14488, 15348, 16207, 17067, 17926, 18785, 19645, 20505, 21364, 22223, 23083, 23943, 24802, 25662, 26521, 27380, 28240, 29100, 29960, 30818, 31678, 32538, 33397, 34256, 35116, 35975,
    ...
    攻击次数2 = 1
    # 最终射击攻击力：<data2>%
    data2 = [int(i*1.1466*1.189) for i in [0, 3530, 3887, 4245, 4604, 4962, 5320, 5678, 6037, 6395, 6752, 7110, 7469, 7827, 8185, 8543, 8902, 9260, 9617, 9975, 10334, 10692, 11050, 11408, 11767, 12125, 12483, 12840, 13199, 13557, 13915, 14273, 14632, 14990, 15348,
...
class 技能17(职业主动技能):
    ...
    # 攻击次数2 = 5
    # 第一次爆炸攻击力：<data0>%
    data0 = [int(i*1.1*1.184) for i in [0, 6085, 6703, 7320, 7938, 8555, 9171, 9789, 10407, 11024, 11641, 12258, 12877, 13494, 14111, 14728, 15346, 15964, 16581, 17199, 17816, 18433, 19051, 19669, 20285, 20902, 21519, 22138, 22755, 23372, 23989, 24607, 25225, 25842,
    ...
    攻击次数 = 3
    # 第二次爆炸攻击力：<data1>%
    data1 = [int(i*1.1*1.184) for i in [0, 5477, 6032, 6588, 7144, 7698, 8255, 8811, 9366, 9922, 10478, 11033, 11589, 12145, 12700, 13256, 13811, 14367, 14923, 15478, 16034, 16590, 17145, 17701, 18257, 18812, 19368, 19923, 20479, 21036, 21591, 22146, 22703, 23258, 23813,
...
class 技能18(职业主动技能):
    ...
    # 成长 = 5693.7
    # 射击攻击力：<data0>%
    data0 = [int(i*1.114*1.189) for i in [0, 56124, 61817, 67511, 73205, 78898, 84592, 90286, 95979, 101673, 107367, 113060, 118755, 124449, 130143, 135836, 141530, 147224, 152917, 158611, 164305, 169998, 175692, 181386, 187079, 192773, 198468, 204161, 209855, 215549, 221242, 226936, 232630, 238323, 244017,
...
class 技能19(职业主动技能):
    ...
    # 成长 = 26597.5
    # CTF地雷爆炸攻击力：<data0>%
    data0 = [int(i*1.114*1.189) for i in [0, 11470, 14130, 16790, 19450, 22110, 24770, 27430, 30091, 32750, 35410, 38070, 40730, 43390, 46050, 48710, 51369, 54029, 56689, 59349, 62009, 64669, 67330, 69990, 72649, 75309, 77969, 80629, 83289, 85949, 88609, 91268, 93928, 96588, 99248, 101908,
    ...
    攻击次数 = 4
    # 最后爆炸攻击力：<data1>%
    data1 = [int(i*1.114*1.189) for i in [0, 5293, 6521, 7750, 8977, 10205, 11432, 12660, 13887, 15115, 16342, 17570, 18798, 20026, 21254, 22481, 23709, 24936, 26164, 27391, 28619, 29846, 31075, 32303, 33530, 34758, 35985, 37213, 38440, 39668, 40895, 42124, 43352, 44579,
