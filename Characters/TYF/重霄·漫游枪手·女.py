...
class 技能1(职业主动技能):
    ...
    基础等级 = 46
    # 物理攻击力：<data0>%
    data0 = [int(i*1.0) for i in [0, 2321, 2556, 2793, 3027, 3262, 3499, 3733, 3970, 4205, 4439, 4676, 4912, 5149, 5383, 5618, 5855, 6089, 6326, 6561, 6795, 7032, 7267, 7504, 7738, 7973, 8210, 8444, 8681, 8916, 9154, 9389, 9623, 9860, 10095, 10331, 10566, 10801, 11037, 11272, 11508, 11744, 11978, 12215, 12450, 12685, 12921, 13156, 13391, 13627, 13863, 14099, 14333, 14569, 14805, 15040, 15275, 15511, 15746, 15982, 16218, 16452, 16688, 16924, 17160, 17395, 17630, 17867, 18103, 18338, 18574]]
    CD = 4.4
    TP成长 = 0.08
    TP上限 = 5
...
class 技能10(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 33
    data0 = [int(i*1.0) for i in [0, 2080, 2291, 2501, 2713, 2923, 3135, 3346, 3556, 3768, 3978, 4190, 4401, 4611, 4823, 5034, 5245, 5456, 5666, 5878, 6089, 6300, 6511, 6722, 6933, 7144, 7356, 7566, 7777, 7988, 8199, 8411, 8621, 8832, 9044, 9254, 9466, 9676, 9887, 10099, 10309, 10521, 10731, 10942, 11154, 11364, 11576, 11787, 11997, 12209, 12419, 12631, 12842, 13052, 13264, 13475, 13686, 13897, 14107, 14319, 14530, 14741, 14952, 15163, 15374, 15585, 15797, 16007, 16218, 16429, 16640]]
    攻击次数 = 5
    CD = 19.8
    TP成长 = 0.10
    TP上限 = 5
...
class 技能13(职业主动技能):
    ...
    基础等级 = 12
    # 向前斩击物理攻击力：<data0>%
    data0 = [int(i*1.0) for i in [0, 2029, 2500, 2970, 3440, 3911, 4382, 4852, 5323, 5793, 6264, 6735, 7205, 7675, 8146, 8617, 9087, 9558, 10028, 10499, 10969, 11440, 11912, 12383, 12853, 13323, 13794, 14265, 14736, 15205, 15676, 16147, 16618, 17087, 17558, 18029, 18500, 18971, 19440, 19911, 20382]]
    # 该次数存疑
    攻击次数 = 24
    # 最后一击攻击力：<data1>%
    data1 = [int(i*1.0) for i in [0, 6019, 7415, 8811, 10207, 11603, 12998, 14395, 15790, 17187, 18583, 19977, 21374, 22770, 24167, 25562, 26958, 28353, 29749, 31146, 32542, 33938, 35334, 36728, 38125, 39521, 40918, 42313, 43708, 45105, 46500, 47897, 49293, 50689, 52084, 53480, 54876, 56272, 57669, 59063, 60459]]
    攻击次数2 = 1
    CD = 145
...
class 技能15(职业主动技能):
    ...
    # 成长2 = 314.83
    # 前方乱射攻击力：<data0>%
    data0 = [int(i*1.0) for i in [0, 1638, 1803, 1969, 2136, 2301, 2469, 2636, 2801, 2968, 3134, 3300, 3466, 3632, 3798, 3965, 4130, 4297, 4462, 4629, 4796, 4962, 5128, 5295, 5460, 5627, 5793, 5959, 6125, 6291, 6457, 6624, 6789, 6956, 7121, 7289, 7456, 7621, 7788, 7954, 8120]]
    # 最后射击攻击力：<data1>%
    data1 = [int(i*1.0) for i in [0, 3641, 4009, 4379, 4747, 5118, 5487, 5856, 6225, 6595, 6964, 7334, 7702, 8073, 8441, 8810, 9181, 9549, 9919, 10287, 10658, 11027, 11396, 11766, 12135, 12504, 12874, 13243, 13613, 13981, 14350, 14721, 15089, 15459, 15827, 16198, 16567, 16936, 17306, 17675, 18044]]
    攻击次数 = 20
    攻击次数2 = 1
    CD = 50.0
    TP成长 = 0.10
...
class 技能17(职业主动技能):
    ...
    基础等级 = 11
    # 快速射击攻击力：<data0>%
    data0 = [int(i*1.0) for i in [0, 5314, 6161, 7009, 7855, 8703, 9551, 10398, 11246, 12093, 12941, 13787, 14634, 15481, 16329, 17176, 18024, 18870, 19718, 20565, 21413, 22260, 23108, 23956, 24801, 25648, 26496, 27343, 28191, 29039, 29886, 30734, 31580, 32428, 33275, 34123, 34969, 35817, 36663, 37511, 38358]]
    攻击次数 = 5
    # 最终射击攻击力：<data1>%
    data1 = [int(i*1.0) for i in [0, 15944, 18487, 21028, 23570, 26111, 28654, 31195, 33737, 36278, 38821, 41364, 43904, 46447, 48988, 51531, 54071, 56614, 59157, 61698, 64240, 66781, 69324, 71866, 74407, 76949, 79491, 82034, 84574, 87117, 89658, 92201, 94741, 97284, 99827, 102368, 104910, 107451, 109994, 112536, 115077]]
    攻击次数2 = 1
    CD = 40.0
    是否有护石 = 1
    护石选项 = ['圣痕']
...
class 技能18(职业主动技能):
    ...
    基础等级 = 13
    # 第1击攻击力：<data0>%
    data0 = [int(i*1.0) for i in [0, 14483, 15953, 17423, 18892, 20362, 21830, 23300, 24770, 26239, 27708, 29178, 30647, 32117, 33587, 35055, 36525, 37994, 39464, 40934, 42402, 43872, 45342, 46811, 48280, 49750, 51219, 52689, 54159, 55627, 57097, 58566, 60036, 61505, 62974, 64444, 65914, 67382, 68852, 70322, 71791]]
    攻击次数 = 1
    # 第2击攻击力：<data1>%
    data1 = [int(i*1.0) for i in [0, 14483, 15953, 17423, 18892, 20362, 21830, 23300, 24770, 26239, 27708, 29178, 30647, 32117, 33587, 35055, 36525, 37994, 39464, 40934, 42402, 43872, 45342, 46811, 48280, 49750, 51219, 52689, 54159, 55627, 57097, 58566, 60036, 61505, 62974, 64444, 65914, 67382, 68852, 70322, 71791]]
    攻击次数2 = 1
    # 第3击攻击力：<data2>%
    data2 = [int(i*1.0) for i in [0, 19311, 21271, 23231, 25189, 27149, 29108, 31068, 33026, 34986, 36945, 38904, 40863, 42823, 44781, 46741, 48700, 50660, 52618, 54578, 56537, 58496, 60455, 62415, 64373, 66333, 68292, 70252, 72211, 74170, 76130, 78088, 80048, 82007, 83967, 85925, 87885, 89844, 91803, 93762, 95722]]
    攻击次数3 = 1
    CD = 45.0
    是否有护石 = 1
    护石选项 = ['圣痕']
...
class 技能22(职业主动技能):
    ...
    基础等级 = 2
    CD = 290.0
    data0 = [int(i*1.00) for i in [0,3571, 4400, 5228, 6056, 6884, 7713, 8541, 9369, 10197, 11026, 11854, 12682, 13510, 14339, 15167, 15995, 16823, 17652, 18480, 19308, 20136, 20965, 21793, 22621, 23449, 24278, 25106, 25934, 26762, 27591, 28419, 29247, 30075, 30904, 31732, 32560, 33388, 34217, 35045, 35873]]
    攻击次数 = 5
    data1 = [int(i*1.00) for i in [0,4464, 5500, 6535, 7570, 8606, 9641, 10676, 11711, 12747, 13782, 14817, 15853, 16888, 17923, 18959, 19994, 21029, 22065, 23100, 24135, 25171, 26206, 27241, 28277, 29312, 30347, 31382, 32418, 33453, 34488, 35524, 36559, 37594, 38630, 39665, 40700, 41736, 42771, 43806, 44842]]
    攻击次数2 = 6
    data2 = [int(i*1.00) for i in [0,7653, 9428, 11203, 12978, 14753, 16528, 18302, 20077, 21852, 23627, 25402, 27177, 28951, 30726, 32501, 34276, 36051, 37825, 39600, 41375, 43150, 44925, 46700, 48474, 50249, 52024, 53799, 55574, 57349, 59123, 60898, 62673, 64448, 66223, 67997, 69772, 71547, 73322, 75097, 76872]]
    攻击次数3 = 7
    data3 = [int(i*1.00) for i in [0,4464, 5500, 6535, 7570, 8606, 9641, 10676, 11711, 12747, 13782, 14817, 15853, 16888, 17923, 18959, 19994, 21029, 22065, 23100, 24135, 25171, 26206, 27241, 28277, 29312, 30347, 31382, 32418, 33453, 34488, 35524, 36559, 37594, 38630, 39665, 40700, 41736, 42771, 43806, 44842]]
    攻击次数4 = 6
    data4 = [int(i*1.00) for i in [0,6868, 8461, 10054, 11647, 13240, 14832, 16425, 18018, 19611, 21204, 22796, 24389, 25982, 27575, 29167, 30760, 32353, 33946, 35539, 37131, 38724, 40317, 41910, 43503, 45095, 46688, 48281, 49874, 51467, 53059, 54652, 56245, 57838, 59431, 61023, 62616, 64209, 65802, 67394, 68987]]
    攻击次数5 = 13
    data5 = [int(i*1.00) for i in [0,41671, 51334, 60997, 70660, 80323, 89986, 99649, 109311, 118974, 128637, 138300, 147963, 157626, 167289, 176952, 186615, 196278, 205941, 215604, 225267, 234930, 244593, 254255, 263918, 273581, 283244, 292907, 302570, 312233, 321896, 331559, 341222, 350885, 360548, 370211, 379874, 389537, 399199, 408862, 418525]]
    攻击次数6 = 3
    data6 = [int(i*1.00) for i in [0,2976, 3666, 4356, 5047, 5737, 6427, 7117, 7807, 8498, 9188, 9878, 10568, 11259, 11949, 12639, 13329, 14019, 14710, 15400, 16090, 16780, 17470, 18161, 18851, 19541, 20231, 20921, 21612, 22302, 22992, 23682, 24373, 25063, 25753, 26443, 27133, 27824, 28514, 29204, 29894]]
    攻击次数7 = 6
