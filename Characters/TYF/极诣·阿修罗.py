...
class 技能3(主动技能):
    ...
    TP成长 = 0.10
    TP上限 = 5
    data0 = [int(i*1.00*1.833) for i in [0, 111, 121, 133, 144, 156, 166, 178, 189, 201, 211, 223, 234, 246, 256, 268, 279, 291, 301, 313, 324, 335, 346, 358, 370, 380, 391, 403, 415, 425, 436, 448, 460, 470, 482, 493, 505, 515, 527, 538, 550, 560, 572, 583, 595, 605, 617, 628, 639, 650, 662, 673, 684, 695, 707, 718, 729, 740, 752, 763, 774, 785, 797, 809, 819, 830, 842, 854, 864, 876, 887]]
    攻击次数 = 18
    data1 = [int(i*1.00*1.833) for i in [0, 498, 550, 600, 650, 702, 752, 802, 854, 904, 954, 1006, 1056, 1106, 1157, 1208, 1258, 1309, 1360, 1410, 1461, 1512, 1562, 1613, 1663, 1714, 1765, 1815, 1866, 1917, 1967, 2018, 2069, 2119, 2169, 2221, 2271, 2321, 2373, 2423, 2473, 2525, 2575, 2625, 2677, 2727, 2777, 2829, 2879, 2929, 2981, 3031, 3081, 3132, 3183, 3233, 3284, 3335, 3385, 3436, 3487, 3537, 3588, 3638, 3689, 3740, 3790, 3841, 3892, 3942, 3993]]
    攻击次数2 = 1
    # 波动刻印增幅 5印
    倍率 = 1+0.2
    def 等效百分比(self, 武器类型):
...
class 技能9(主动技能):
    ...
    TP成长 = 0.10
    TP上限 = 5
    data0 = [int(i*1.055*1.1) for i in [0, 295, 304, 314, 323, 333, 342, 352, 362, 371, 381, 390, 400, 409, 419, 428, 438, 448, 457, 467, 476]]
    攻击次数 = 2
    def 等效百分比(self, 武器类型):
        return self.data0[self.等级] * self.攻击次数 * (1 + self.TP成长 * self.TP等级) * self.倍率
...
class 技能12(主动技能):
    ...
    TP上限 = 5
    是否有护石 = 1
    data0 = [int(i*1.169*1.102) for i in [0, 1450, 1597, 1744, 1891, 2038, 2185, 2332, 2479, 2626, 2774, 2921, 3068, 3215, 3362, 3509, 3656, 3803, 3950, 4098, 4245, 4392, 4539, 4686, 4833, 4980, 5127, 5274, 5422, 5569, 5716, 5863, 6010, 6157, 6304, 6451, 6598, 6746, 6893, 7040, 7187, 7334, 7481, 7628, 7775, 7922, 8070, 8217, 8364, 8511, 8658, 8805, 8952, 9099, 9246, 9394, 9541, 9688, 9835, 9982, 10129, 10276, 10423, 10570, 10718, 10865, 11012, 11159, 11306, 11453, 11600]]
    攻击次数 = 1
    data1 = [int(i*1.169*1.102) for i in [0, 1325, 1460, 1594, 1729, 1863, 1998, 2132, 2267, 2401, 2536, 2670, 2805, 2939, 3074, 3208, 3343, 3477, 3612, 3746, 3881, 4015, 4150, 4284, 4419, 4553, 4688, 4822, 4957, 5091, 5226, 5360, 5495, 5629, 5764, 5898, 6033, 6167, 6302, 6436, 6571, 6705, 6840, 6974, 7109, 7243, 7378, 7512, 7647, 7781, 7916, 8050, 8185, 8319, 8454, 8588, 8723, 8857, 8992, 9126, 9261, 9395, 9530, 9664, 9799, 9933, 10068, 10202, 10337, 10471, 10606]]
    攻击次数2 = 7
    data3 = [int(i*1.169*1.102) for i in [0, 3770, 4152, 4535, 4917, 5300, 5682, 6065, 6447, 6830, 7212, 7595, 7977, 8360, 8742, 9125, 9507, 9890, 10272, 10654, 11037, 11419, 11802, 12184, 12567, 12949, 13332, 13714, 14097, 14479, 14862, 15244, 15627, 16009, 16392, 16774, 17157, 17539, 17922, 18304, 18687, 19069, 19452, 19834, 20217, 20599, 20982, 21364, 21747, 22129, 22512, 22894, 23277, 23659, 24042, 24424, 24806, 25189, 25571, 25954, 26336, 26719, 27101, 27484, 27866, 28249, 28631, 29014, 29396, 29779, 30161]]
    攻击次数3 = 1
    def 等效百分比(self, 武器类型):
...
class 技能18(主动技能):
    ...
    TP成长 = 0.1
    CD = 4.9
    data0 = [int(i*1.1966*1.1) for i in [0, 739, 910, 1082, 1253, 1424, 1597, 1768, 1939, 2110, 2281, 2453, 2625, 2796, 2968, 3139, 3310, 3481, 3654, 3825, 3996, 4168, 4339, 4510, 4683, 4854, 5025, 5196, 5368, 5539, 5711, 5883, 6054, 6225, 6396, 6568, 6740, 6911, 7083, 7254, 7425, 7596, 7769, 7940, 8111, 8282, 8454, 8625, 8797, 8969, 9140, 9311, 9482, 9655, 9826, 9997, 10169, 10340, 10511, 10684, 10855, 11026, 11197, 11369, 11540, 11712, 11884, 12055, 12226, 12397, 12569]]
    攻击次数 = 7
    def 等效百分比(self, 武器类型):
        return self.data0[self.等级] * self.攻击次数 * (1 + self.TP成长 * self.TP等级) * self.倍率
...
class 技能20(主动技能):
    ...
    TP上限 = 5
    是否有护石 = 1
    data0 = [int(i*1.05*1.101) for i in [0, 1835, 2021, 2208, 2393, 2580, 2766, 2954, 3138, 3324, 3512, 3696, 3884, 4070, 4254, 4442, 4628, 4815, 5000, 5187, 5373, 5558, 5745, 5931, 6119, 6303, 6491, 6677, 6861, 7049, 7235, 7422, 7607, 7794, 7980, 8166, 8352, 8538, 8726, 8912, 9096, 9284, 9470, 9657, 9842, 10029, 10215, 10400, 10587, 10773, 10961]]
    攻击次数 = 3
    data1 = [int(i*1.05*1.101) for i in [0, 2723, 2999, 3276, 3552, 3828, 4104, 4380, 4656, 4935, 5211, 5487, 5763, 6039, 6315, 6591, 6867, 7143, 7421, 7697, 7973, 8249, 8526, 8802, 9080, 9356, 9632, 9908, 10184, 10460, 10736, 11012, 11288, 11565, 11843, 12119, 12395, 12671, 12947, 13224, 13500, 13776, 14052, 14328, 14604, 14880, 15156, 15435, 15711, 15987, 16263]]
    攻击次数2 = 5
    def 等效百分比(self, 武器类型):
        return (self.data0[self.等级] * self.攻击次数 + self.data1[self.等级] * self.攻击次数2) * (1 + self.TP成长 * self.TP等级) * self.倍率
...
class 技能23(主动技能):
    ...
    CD = 2.5
    关联技能 = ['所有']
    data0 = [int(i*1.055*1.1) for i in [0, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 11, 12, 12, 13, 14, 14, 15, 15, 16, 17, 17, 18, 19, 19, 20, 21, 21, 22, 22, 23, 24, 24, 25, 26, 26, 27, 28, 28, 29, 29, 30, 31, 31, 32, 33, 33, 34, 35, 35]]
    攻击次数 = 1
...
class 技能24(主动技能):
    ...
    是否有护石 = 1
    # 终结攻击发动时的小型金刚杵
    data0 = [int(i*1.0563*1.1) for i in [0, 887, 978, 1068, 1159, 1248, 1339, 1428, 1519, 1608, 1698, 1789, 1879, 1968, 2058, 2149, 2239, 2329, 2418, 2509, 2599, 2689, 2779, 2870, 2960, 3050, 3140, 3231, 3321, 3410, 3500, 3591, 3681, 3770, 3861, 3950, 4041, 4131, 4221, 4310, 4401, 4491, 4582, 4671, 4762, 4852, 4942, 5031, 5122, 5212, 5302, 5392, 5482, 5572, 5662, 5752, 5842, 5933, 6023, 6113, 6204, 6294, 6384, 6473, 6563, 6654, 6744, 6833, 6923, 7014, 7104]]
    攻击次数 = 8*2
    data1 = [int(i*1.0563*1.389) for i in [0, 8885, 9786, 10688, 11589, 12489, 13392, 14294, 15195, 16095, 16997, 17900, 18801, 19703, 20603, 21504, 22407, 23309, 24209, 25110, 26012, 26915, 27815, 28716, 29618, 30519, 31419, 32322, 33224, 34125, 35025, 35927, 36830, 37731, 38631, 39533, 40434, 41337, 42239, 43139, 44040, 44942, 45845, 46745, 47646, 48548, 49449, 50349, 51252, 52154, 53055, 53955, 54857, 55760, 56661, 57561, 58463, 59364, 60267, 61167, 62069, 62970, 63872, 64775, 65675, 66576, 67478, 68381, 69279, 70182, 71084]]
    攻击次数2 = 1
    data3 = [int(i*1.0563*1.389) for i in [0, 24846, 27366, 29886, 32409, 34929, 37449, 39969, 42492, 45012, 47532, 50052, 52572, 55095, 57615, 60135, 62655, 65177, 67698, 70218, 72738, 75260, 77780, 80301, 82821, 85343, 87863, 90383, 92904, 95426, 97946, 100466, 102986, 105509, 108029, 110549, 113069, 115589, 118112, 120632, 123152, 125672, 128193, 130715, 133235, 135756, 138276, 140796, 143318, 145839, 148359, 150879, 153399, 155922, 158442, 160962, 163482, 166005, 168525, 171045, 173565, 176085, 178608, 181128, 183648, 186168, 188690, 191211, 193731, 196251, 198773]]
    攻击次数3 = 1
    # 感电部分待定
    data4 = [int(i*1.0563*1.1) for i in [0, 38, 42, 46, 49, 53, 57, 61, 65, 69, 72, 76, 80, 84, 88, 92, 96, 99, 103, 107, 111, 115, 119, 122, 126, 130, 134, 138, 142, 145, 149, 153, 157, 161, 165, 169, 172, 176, 180, 184, 188, 192, 195, 199, 203, 207, 211, 215, 218, 222, 226, 230, 234, 238, 242, 245, 249, 253, 257, 261, 265, 268, 272, 276, 280, 284, 288, 291, 295, 299, 303]]
    攻击次数4 = 18
    # 护石加成 = 1
    def 等效百分比(self, 武器类型):
...
class 技能25(主动技能):
    ...
    攻击次数 = 6
    # 感电部分待定
    data1 = [int(i*1.0563*1.1) for i in [0, 38, 42, 46, 49, 53, 57, 61, 65, 69, 72, 76, 80, 84, 88, 92, 96, 99, 103, 107, 111, 115, 119, 122, 126, 130, 134, 138, 142, 145, 149, 153, 157, 161, 165, 169, 172, 176, 180, 184, 188, 192, 195, 199, 203, 207, 211, 215, 218, 222, 226, 230, 234, 238, 242, 245, 249, 253, 257, 261, 265, 268, 272, 276, 280, 284, 288, 291, 295, 299, 303]]
    攻击次数2 = 6
    def 等效百分比(self, 武器类型):
        return (self.data0[self.等级] * self.攻击次数 + self.data1[self.等级] * self.攻击次数2) * (1 + self.TP成长 * self.TP等级) * self.倍率
...
class 技能28(主动技能):
    ...
    #成长 = 13667.14
    CD = 60.0
    data0 = [int(i*1.1686*1.101) for i in [0, 6735, 7419, 8102, 8785, 9469, 10152, 10835, 11519, 12202, 12885, 13569, 14252, 14935, 15619, 16302, 16985, 17669, 18352, 19035, 19719, 20402, 21085, 21769, 22452, 23135, 23819, 24502, 25185, 25869, 26552, 27235, 27919, 28602, 29285, 29969, 30652, 31335, 32019, 32702, 33385]]
    攻击次数 = 5
    data1 = [int(i*1.1686*1.101) for i in [0, 101035, 111286, 121536, 131786, 142036, 152286, 162536, 172786, 183036, 193286, 203536, 213786, 224036, 234286, 244536, 254786, 265036, 275286, 285536, 295786, 306036, 316286, 326536, 336786, 347036, 357286, 367536, 377786, 388036, 398286, 408536, 418786, 429036, 439286, 449536, 459786, 470036, 480286, 490537, 500787]]
    攻击次数2 = 1
    def 等效百分比(self, 武器类型):
        return (self.data0[self.等级] * self.攻击次数 + self.data1[self.等级] * self.攻击次数2) * (1 + self.TP成长 * self.TP等级) * self.倍率
