...
class 技能5(职业主动技能):
    ...
    基础等级 = 1
    是否主动 = 0
    data0 = [(i*1.0) for i in [0, 2578, 2890, 3202, 3514, 3825, 4137, 4449, 4761, 5072, 5384, 5696, 6008, 6319, 6631, 6943, 7255, 7566, 7878, 8190, 8501, 8813, 9125, 9437, 9748, 10060, 10372, 10684, 10995, 11307, 11619]]
    攻击次数 = 1
    # 基础 = 2281
    # 成长 = 311
    CD = 0.5
...
class 技能8(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 38
    data0 = [(i*4765/4764) for i in [0, 4764, 5247, 5730, 6214, 6697, 7180, 7664, 8147, 8630, 9114, 9597, 10080, 10563, 11047, 11530, 12013, 12497, 12980, 13463, 13947, 14430, 14913, 15397, 15880, 16363, 16847, 17330, 17813, 18297, 18780, 19263, 19747, 20230, 20713, 21197, 21680, 22163, 22646, 23130, 23613, 24096, 24580, 25063, 25546, 26030, 26513, 26996, 27480, 27963, 28446, 28930, 29413, 29896, 30380, 30863, 31346, 31830, 32313, 32796, 33279, 33763, 34246, 34729, 35213, 35696, 36179, 36663, 37146, 37629, 38113]]
    攻击次数 = 1
    # 基础 = 4292
    # 成长 = 483
    CD = 7
...
class 技能15(职业主动技能):
    ...
    # 成长 = 4948
    #旋转斩击攻击力：<data0>%
    data0 = [(i*4878/4877) for i in [0, 4877, 5371, 5866, 6361, 6856, 7350, 7845, 8340, 8835, 9329, 9824, 10319, 10814, 11309, 11803, 12298, 12793, 13288, 13782, 14277, 14772, 15267, 15761, 16256, 16751, 17246, 17741, 18235, 18730, 19225, 19720, 20214, 20709, 21204, 21699, 22193, 22688, 23183, 23678, 24173, 24667, 25162, 25657, 26152, 26646, 27141, 27636, 28131, 28626, 29120, 29615, 30110, 30605, 31099, 31594, 32089, 32584, 33078, 33573, 34068, 34563, 35058, 35552, 36047, 36542, 37037, 37531, 38026, 38521, 39016]]
    攻击次数 = 2
    #最后一击攻击力：<data1>%
    data1 = [(i*39031/39016) for i in [0, 39016, 42974, 46932, 50890, 54848, 58806, 62765, 66723, 70681, 74639, 78597, 82555, 86514, 90472, 94430, 98388, 102346, 106304, 110263, 114221, 118179, 122137, 126095, 130053, 134012, 137970, 141928, 145886, 149844, 153802, 157761, 161719, 165677, 169635, 173593, 177551, 181510, 185468, 189426, 193384, 197342, 201300, 205259, 209217, 213175, 217133, 221091, 225049, 229008, 232966, 236924, 240882, 244840, 248798, 252756, 256715, 260673, 264631, 268589, 272547, 276505, 280464, 284422, 288380, 292338, 296296, 300254, 304213, 308171, 312129]]
    攻击次数2 = 1
    CD = 40
    是否有护石 = 1
    护石选项 = ['圣痕']
