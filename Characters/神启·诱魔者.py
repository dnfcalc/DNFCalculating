...
class 神启·诱魔者技能20(主动技能):
    ...
    等级上限 = 40
    基础等级 = 13
    数据1 = [0, 29112, 32065, 35017, 37971, 40924, 43878, 46831, 49785, 52738, 55692, 58645, 61598, 64552, 67505, 70458, 73413, 76365, 79319, 82271, 85224, 88178, 91131, 94086, 97039, 99993, 102946, 105898, 108852, 111805, 114760, 117712, 120665, 123619, 126572, 129525, 132478, 135433, 138386, 141339, 144293]
    攻击次数1 = 1
    数据2 = [0, 9704, 10687, 11672, 12656, 13641, 14626, 15610, 16595, 17579, 18563, 19548, 20532, 21517, 22502, 23486, 24470, 25455, 26439, 27424, 28407, 29393, 30378, 31362, 32346, 33331, 34315, 35299, 36284, 37269, 38254, 39236, 40221, 41206, 42189, 43175, 44159, 45144, 46128, 47112, 48097]
    攻击次数2 = 3*1.21
    CD = 45.0
    是否有护石 = 1
    def 等效百分比(self, 武器类型):
...
class 神启·诱魔者(角色窗口):
    ...
    def 载入配置(self, path='set'):
        ...
        try:
           ...
           self.手搓冷却.setChecked(True if int(setfile[1].replace('\n', '')) == 1 else False)
           self.净化之花冷却Buff.setChecked(True if int(setfile[2].replace('\n', '')) == 1 else False)
