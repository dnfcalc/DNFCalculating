...
class 技能1(主动技能):
    ...
    等级上限 = 60
    基础等级 = 43
    数据 = [int(i*1.0) for i in [0, 162, 179, 195, 212, 229, 245, 262, 278, 295, 311, 328, 344, 361, 377, 394, 410, 427, 443, 460, 476, 493, 509, 526, 543, 559, 576, 592, 609, 625, 642, 658, 675, 691, 708, 724, 741, 757, 774, 790, 807, 824, 840, 857, 873, 890, 906, 923, 939, 956, 972, 989, 1005, 1022, 1038, 1055, 1071, 1088, 1104, 1121, 1138, 1154, 1171, 1187, 1204, 1220, 1237, 1253, 1270, 1286, 1303]]
    CD = 1.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能8(主动技能):
    ...
    等级上限 = 60
    基础等级 = 38
    数据1 = [int(i*1) for i in [0, 4808, 5296, 5784, 6272, 6760, 7248, 7735, 8223, 8711, 9199, 9687, 10175, 10663, 11150, 11638, 12126, 12614, 13102, 13590, 14078, 14565, 15053, 15541, 16029, 16517, 17005, 17493, 17980, 18468, 18956, 19444, 19932, 20420, 20908, 21395, 21883, 22371, 22859, 23347, 23835, 24323, 24810, 25298, 25786, 26274, 26762, 27250, 27738, 28225, 28713, 29201, 29689, 30177, 30665, 31153, 31640, 32128, 32616, 33104, 33592, 34080, 34568, 35055, 35543, 36031, 36519, 37007, 37495, 37983, 38470]]
    攻击次数1 = 1
    CD = 12.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能10(主动技能):
    ...
    等级上限 = 60
    基础等级 = 33
    数据1 = [int(i*1) for i in [0, 704, 776, 847, 919, 990, 1062, 1133, 1205, 1276, 1348, 1419, 1490, 1562, 1633, 1705, 1776, 1848, 1919, 1991, 2062, 2134, 2205, 2277, 2348, 2420, 2491, 2563, 2634, 2706, 2777, 2849, 2920, 2992, 3063, 3135, 3206, 3278, 3349, 3421, 3492, 3564, 3635, 3707, 3778, 3850, 3921, 3992, 4064, 4135, 4207, 4278, 4350, 4421, 4493, 4564, 4636, 4707, 4779, 4850, 4922, 4993, 5065, 5136, 5208, 5279, 5351, 5422, 5494, 5565, 5637]]
    攻击次数1 = 20
    CD = 35
    TP成长 = 0.10
    TP上限 = 5
...
class 技能15(主动技能):
    ...
    等级上限 = 40
    基础等级 = 23
    数据1 = [int(i*1) for i in [0, 7672, 8451, 9229, 10008, 10786, 11564, 12343, 13121, 13900, 14678, 15457, 16235, 17013, 17792, 18570, 19349, 20127, 20905, 21684, 22462, 23241, 24019, 24797, 25576, 26354, 27133, 27911, 28690, 29468, 30246, 31025, 31803, 32582, 33360, 34138, 34917, 35695, 36474, 37252, 38030, 38809, 39587, 40366, 41144, 41922, 42701, 43479, 44258, 45036, 45815]]
    攻击次数1 = 1
    数据2 = [int(i*1) for i in [0, 2557, 2817, 3076, 3336, 3595, 3854, 4114, 4373, 4633, 4892, 5152, 5411, 5671, 5930, 6190, 6449, 6709, 6968, 7228, 7487, 7747, 8006, 8265, 8525, 8784, 9044, 9303, 9563, 9822, 10082, 10341, 10601, 10860, 11120, 11379, 11639, 11898, 12158, 12417, 12676, 12936, 13195, 13455, 13714, 13974, 14233, 14493, 14752, 15012, 15271]]
    攻击次数2 = 3
    CD = 30
    TP成长 = 0.10
    TP上限 = 5
...
class 技能16(主动技能):
    ...
    等级上限 = 40
    基础等级 = 18
    数据1 = [int(i*1) for i in [0, 383, 422, 461, 499, 538, 577, 616, 655, 694, 733, 772, 811, 849, 888, 927, 966, 1005, 1044, 1083, 1122, 1161, 1199, 1238, 1277, 1316, 1355, 1394, 1433, 1472, 1510, 1549, 1588, 1627, 1666, 1705, 1744, 1783, 1822, 1860, 1899, 1938, 1977, 2016, 2055, 2094, 2133, 2172, 2210, 2249, 2288]]
    攻击次数1 = 40
    数据2 = [int(i*1) for i in [0, 1542, 1698, 1855, 2011, 2168, 2324, 2481, 2637, 2794, 2950, 3107, 3263, 3419, 3576, 3732, 3889, 4045, 4202, 4358, 4515, 4671, 4828, 4984, 5141, 5297, 5454, 5610, 5766, 5923, 6079, 6236, 6392, 6549, 6705, 6862, 7018, 7175, 7331, 7488, 7644, 7801, 7957, 8114, 8270, 8426, 8583, 8739, 8896, 9052, 9209]]
    攻击次数2 = 19
    CD = 50
    TP成长 = 0.10
    TP上限 = 5
...
class 技能18(主动技能):
    ...
    等级上限 = 40
    基础等级 = 16
    数据1 = [int(i*1.097) for i in [0, 2616, 2882, 3147, 3413, 3678, 3944, 4209, 4474, 4739, 5005, 5270, 5535, 5801, 6066, 6332, 6597, 6863, 7128, 7394, 7659, 7925, 8190, 8455, 8721, 8986, 9252, 9517, 9783, 10048, 10314, 10579, 10845, 11110, 11375, 11641, 11906, 12172, 12437, 12703, 12968, 13234, 13499, 13765, 14030, 14295, 14561, 14826, 15092, 15357, 15623, 15888, 16154, 16419, 16685, 16950, 17215, 17481, 17746, 18012, 18277, 18543, 18808, 19074, 19338, 19604, 19869, 20135, 20400, 20665, 20931]]
    攻击次数1 = 18
    数据2 = [int(i*1) for i in [0, 9121, 10047, 10972, 11898, 12823, 13748, 14674, 15599, 16525, 17450, 18375, 19301, 20226, 21152, 22077, 23002, 23928, 24853, 25779, 26704, 27630, 28555, 29480, 30406, 31331, 32257, 33182, 34107, 35033, 35958, 36884, 37809, 38734, 39660, 40585, 41511, 42436, 43361, 44287, 45212, 46138, 47063, 47989, 48914, 49839, 50765, 51690, 52616, 53541, 54466, 55392, 56317, 57243, 58168, 59093, 60019, 60944, 61870, 62795, 63720, 64646, 65571, 66497, 67422, 68347, 69273, 70198, 71124, 72049, 72975]]
    攻击次数2 = 1
    CD = 40
    演出时间 = 2
    是否有护石 = 1
