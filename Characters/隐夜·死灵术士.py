...
class 技能16(主动技能):
    ...
    等级精通 = 50
    基础等级 = min(int((等级 - 所在等级) / 学习间隔) + 1, 等级精通)
    data1 = [int(i*1.0) for i in [0, 1327, 1461, 1596, 1731, 1865, 2000, 2134, 2269, 2404, 2538, 2673, 2808, 2942, 3077, 3212, 3346, 3481, 3615, 3750, 3885, 4019, 4154, 4289, 4423, 4558, 4693, 4827, 4962, 5096, 5231, 5366, 5500, 5635, 5770, 5904,
    ...
    攻击次数1 = 2
    data2 = [int(i*1.0) for i in [0, 5977, 6583, 7190, 7796, 8403, 9009, 9615, 10222, 10828, 11435, 12041, 12647, 13254, 13860, 14467, 15073, 15679, 16286, 16892, 17499, 18105, 18712, 19318, 19924, 20531, 21137, 21744, 22350, 22956, 23563, 24169, 24776, 25382, 25988, 26595,
...
class 技能17(主动技能):
    ...
    等级精通 = 50
    基础等级 = min(int((等级 - 所在等级) / 学习间隔) + 1, 等级精通)
    data1 = [int(i*1.0) for i in [0, 2090, 2302, 2514, 2726, 2938, 3150, 3362, 3574, 3786, 3998, 4210, 4422, 4634, 4846, 5058, 5270, 5482, 5694, 5906, 6118, 6330, 6542, 6754, 6966, 7178, 7390, 7602, 7814, 8026, 8238, 8450, 8662, 8874, 9087, 9299, 9511, 9723,
    ...
    攻击次数1 = 1
    data2 = [int(i*1.0) for i in [0, 7389, 8139, 8888, 9638, 10388, 11137, 11887, 12637, 13386, 14136, 14886, 15635, 16385, 17135, 17884, 18634, 19384, 20133, 20883, 21633, 22382, 23132, 23881, 24631, 25381, 26130, 26880, 27630, 28379, 29129, 29879, 30628, 31378, 32128, 32877,
...
class 技能22(主动技能):
    ...
    等级精通 = 30
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通)
    data1 = [int(i*1.0) for i in [0, 625, 770, 916, 1061, 1206, 1351, 1496, 1641, 1786, 1931, 2077, 2222, 2367, 2512, 2657, 2802, 2947, 3092, 3238, 3383,
           3528, 3673, 3818, 3963, 4108, 4253, 4399, 4544, 4689, 4834, 4979, 5124, 5269, 5414, 5560, 5705, 5850, 5995, 6140, 6285]]
    攻击次数1 = 9
    data2 = [int(i*1.0) for i in [0, 1933, 2381, 2829, 3277, 3726, 4174, 4622, 5070, 5519, 5967, 6415, 6863, 7312, 7760, 8208, 8656, 9105, 9553, 10001, 10449, 10898,
           11346, 11794, 12242, 12691, 13139, 13587, 14035, 14484, 14932, 15380, 15828, 16277, 16725, 17173, 17621, 18070, 18518, 18966, 19414]]
    攻击次数2 = 13
    data3 = [int(i*1.0) for i in [0, 12565, 15478, 18392, 21305, 24219, 27133, 30046, 32960, 35874, 38787, 41701, 44614, 47528, 50442, 53355, 56269, 59183, 62096, 65010, 67923,
...
class 技能24(主动技能):
    ...
    等级精通 = 30
    基础等级 = min(int((等级 - 所在等级) / 学习间隔) + 1, 等级精通)
    data1 = [int(i*1.0) for i in [0, 1264, 1392, 1520, 1650, 1777, 1904, 2033, 2162, 2291, 2417, 2547, 2675, 2804, 2932, 3059, 3186, 3314, 3444, 3572, 3700, 3829, 3958, 4084, 4213, 4343, 4471, 4598, 4727, 4853, 4982, 5112, 5239, 5367, 5496,
    ...
    攻击次数1 = 6
    data2 = [int(i*1.0) for i in [0, 10969, 12081, 13194, 14306, 15419, 16530, 17645, 18757, 19871, 20982, 22097, 23208, 24322, 25435, 26547, 27658, 28770, 29884, 30997, 32110, 33223, 34337, 35449, 36561, 37674, 38787, 39898, 41013, 42123, 43236, 44350, 45462, 46576, 47689,
...
class 技能25(主动技能):
    ...
    等级上限 = 40
    基础等级 = 16
    data1 = [int(i*1.0) for i in [0, 5435, 5986, 6537, 7089, 7640, 8192, 8743, 9294, 9846, 10397, 10949, 11500, 12051, 12603, 13154, 13706, 14257, 14808, 15360, 15911,
           16463, 17014, 17565, 18117, 18668, 19219, 19771, 20322, 20874, 21425, 21976, 22528, 23079, 23631, 24182, 24733, 25285, 25836, 26388, 26939]]
    攻击次数1 = 1
    data2 = [int(i*1.0) for i in [0, 7246, 7982, 8717, 9452, 10187, 10922, 11658, 12393, 13128, 13863, 14598, 15333, 16069, 16804, 17539, 18274, 19009, 19745, 20480, 21215,
           21950, 22685, 23421, 24156, 24891, 25626, 26361, 27097, 27832, 28567, 29302, 30037, 30772, 31508, 32243, 32978, 33713, 34448, 35184, 35919]]
    攻击次数2 = 1
    data3 = [int(i*1.0) for i in [0, 905, 997, 1089, 1181, 1273, 1365, 1457, 1549, 1641, 1732, 1824, 1916, 2008, 2100, 2192, 2284, 2376, 2468, 2560,
...
class 技能27(主动技能):
    ...
    等级精通 = 30
    基础等级 = min(int((等级 - 所在等级) / 学习间隔) + 1, 等级精通)
    data = [int(i*1.0) for i in [0, 43448, 47856, 52264, 56671, 61079, 65487, 69895, 74303, 78710, 83118, 87526, 91934, 96342, 100750, 105157, 109565, 113973, 118381, 122789, 127196,
...
class 技能28(主动技能):
    ...
    等级精通 = 30
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通)
    data1 = [int(i*1.0) for i in [0, 18411, 22681, 26949, 31219, 35489, 39758, 44028, 48297, 52566, 56835, 61105, 65376, 69644, 73914, 78182, 82452, 86722, 90990, 95261, 99530, 103799,
    ...
    攻击次数1 = 1
    data2 = [int(i*1.0) for i in [0, 2509, 3091, 3673, 4255, 4837, 5419, 6002, 6584, 7166, 7748, 8329, 8911, 9493, 10075, 10657, 11239, 11821, 12403, 12985, 13567,
           14149, 14731, 15313, 15895, 16477, 17060, 17642, 18224, 18805, 19387, 19969, 20550, 21132, 21714, 22296, 22878, 23460, 24042, 24624, 25206]]
    攻击次数2 = 11
    data3 = [int(i*1.0) for i in [0, 46029, 56704, 67377, 78051, 88725, 99397, 110071, 120745, 131419, 142093, 152766, 163440, 174113, 184787, 195461, 206134, 216808, 227482, 238156, 248829,
...
class 技能31(主动技能):
    ...
    等级精通 = 30
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通)
    data1 = [int(i*1.0) for i in [0, 283901, 349736, 415568, 481400, 547233, 613065, 678899, 744732, 810564, 876396, 942228, 1008063, 1073895, 1139727, 1205560, 1271392, 1337226, 1403059, 1468891, 1534723, 1600555, 1666390, 1732222, 1798054, 1863887, 1929719, 1995553, 2061386, 2127218, 2193050, 2258882, 2324717, 2390549, 2456381, 2522214, 2588046, 2653880, 2719713, 2785545, 2851377, 2917209, 2983044, 3048876, 3114708, 3180541, 3246373, 3312207, 3378040, 3443872, 3509704, 3575536, 3641371, 3707203, 3773035, 3838868, 3904700, 3970534, 4036367, 4102199, 4168031, 4233863, 4299698, 4365530, 4431362, 4497195, 4563027, 4628861, 4694694, 4760526, 4826358]]
    攻击次数 = 1
    CD = 290.0
