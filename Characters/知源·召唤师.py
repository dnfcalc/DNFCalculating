...
class 技能23(知源·召唤师主动技能):
    ...
    等级上限 = 40
    基础等级 = 5
    基础 = 4782.4*1.15*1.0646 + 1912.5*1.15*5*1.0646 + 2733.2*1.15*7*1.0646 + 14346*1.15*1.0646
    成长 = 1443.7*1.15*1.0646 + 577.7*1.15*5 *1.0646+ 824.9*1.15*7*1.0646 + 4332*1.15*1.0646
    演出时间 = 5.25
    CD = 180
