...
class 技能0(主动技能):
    ...
    等级上限 = 60
    基础等级 = 46
    data0 = [int(i*1.00) for i in [0, 300, 331, 361, 391, 422, 453, 483, 514, 545, 574, 605, 636, 666, 697, 727, 758, 788, 819, 850, 879, 910, 941, 971, 1002, 1033, 1062, 1093, 1124, 1155, 1185, 1215, 1246, 1276, 1307, 1338, 1367,
...
class 技能2(主动技能):
    ...
    等级上限 = 60
    基础等级 = 43
    data0 = [int(i*1.00) for i in [0, 85, 94, 103, 111, 120, 129, 138, 145, 155, 164, 173, 180, 189, 198, 206, 215, 224, 233, 241, 250, 259, 268, 276, 285, 294, 303, 310, 319, 328, 337, 345, 354, 363, 372,
    ...
    攻击次数 = 0  # 抓取不了建筑
    data1 = [int(i*1.00) for i in [0, 828, 912, 995, 1079, 1163, 1248, 1331, 1415, 1499, 1584, 1667, 1751, 1835, 1920, 2003, 2087, 2171, 2256, 2339, 2423, 2507, 2592, 2675, 2759, 2843, 2926, 3011, 3095, 3179, 3263, 3348, 3431, 3515, 3599, 3684,
...
class 技能3(主动技能):
    ...
    等级上限 = 60
    基础等级 = 43
    data0 = [int(i*1.00) for i in [0, 1365, 1504, 1642, 1781, 1918, 2057, 2195, 2334, 2473, 2611, 2750, 2888, 3027, 3166, 3304, 3443, 3581, 3720, 3857, 3996, 4135, 4273, 4412, 4550, 4689, 4828, 4966, 5105, 5243, 5382, 5520, 5659, 5798, 5935, 6074,
    ...
    data1 = [int(i*1.00) for i in [0, 273, 300, 328, 355, 384, 411, 439, 466, 494, 521, 550, 577, 605, 632, 660, 688, 716, 743, 771, 798, 827, 854, 882, 909, 937, 964, 993, 1020, 1048, 1075, 1103, 1130, 1159, 1186, 1214, 1241, 1269,
...
class 技能5(主动技能):
    ...
    等级上限 = 70
    基础等级 = 41
    data0 = [int(i*1.00) for i in [0, 3879, 4272, 4666, 5060, 5453, 5847, 6241, 6633, 7027, 7421, 7814, 8208, 8602, 8994, 9388, 9782, 10175, 10569, 10963, 11356, 11750, 12144, 12536, 12930, 13324, 13717, 14111, 14505, 14898, 15292, 15686, 16080, 16472, 16866, 17260, 17653,
    ...
    data1 = [int(i*1.00) for i in [0, 2585, 2848, 3111, 3373, 3635, 3897, 4160, 4423, 4684, 4947, 5209, 5472, 5735, 5996, 6259, 6521, 6784, 7046, 7308, 7571, 7833, 8096, 8357, 8620, 8882, 9145, 9408, 9669, 9932, 10194, 10457, 10720, 10981, 11244, 11506, 11769,
...
class 技能6(主动技能):
    ...
    等级上限 = 70
    基础等级 = 41
    data0 = [int(i*1.00) for i in [0, 501, 551, 603, 653, 704, 754, 806, 856, 908, 958, 1009, 1060, 1111, 1161, 1213, 1263, 1315, 1365, 1416, 1466, 1518, 1568, 1620, 1670, 1721, 1772, 1823, 1873, 1925, 1975, 2027, 2077, 2128, 2178, 2230,
...
class 技能7(主动技能):
    ...
    等级上限 = 70
    基础等级 = 26
    data0 = [int(i*1.00) for i in [0, 599, 694, 791, 886, 981, 1076, 1172, 1268, 1363, 1459, 1554, 1651, 1746, 1841, 1936, 2032, 2128, 2224, 2319, 2414, 2511, 2606, 2701, 2796, 2892, 2988, 3084, 3179, 3274, 3371, 3466, 3561, 3657, 3753, 3848,
...
class 技能8(主动技能):
    ...
    等级上限 = 70
    基础等级 = 38
    data0 = [int(i*1.00) for i in [0, 1444, 1591, 1738, 1885, 2030, 2177, 2324, 2471, 2618, 2765, 2912, 3058, 3205, 3352, 3499, 3645, 3792, 3939, 4085, 4232, 4379, 4526, 4673, 4820, 4965, 5112, 5258, 5404, 5551, 5697, 5844, 5991, 6138, 6285, 6432,
    ...
    data1 = [int(i*1.00) for i in [0, 1543, 1699, 1855, 2012, 2168, 2324, 2480, 2638, 2794, 2950, 3107, 3263, 3419, 3575, 3732, 3888, 4044, 4202, 4359, 4515, 4673, 4829, 4985, 5142, 5298, 5454, 5610, 5767, 5923, 6079, 6237, 6393, 6549, 6705, 6862, 7020,
...
class 技能9(主动技能):
    ...
    成长 = 1043.343
    # 生成攻击力
    data0 = [int(i*1.00) for i in [0, 791, 870, 951, 1031, 1111, 1192, 1272, 1352, 1433, 1513, 1593, 1674, 1754, 1833, 1915, 1994, 2074, 2155, 2235, 2315, 2396, 2476, 2556, 2637, 2717, 2796, 2878, 2957, 3037, 3118, 3198, 3278, 3359, 3439, 3519,
    ...
    # 幻镜攻击力
    data1 = [int(i*1.00) for i in [0, 791, 872, 951, 1031, 1112, 1192, 1272, 1353, 1433, 1513, 1594, 1674, 1754, 1835, 1915, 1994, 2075, 2155, 2236, 2316, 2396, 2477, 2557, 2637, 2718, 2798, 2878, 2959, 3039, 3118, 3199, 3279, 3360, 3440, 3520,
...
class 技能10(主动技能):
    ...
    等级上限 = 70
    基础等级 = 36
    data0 = [int(i*1.00) for i in [0, 14715, 16207, 17701, 19193, 20686, 22178, 23672, 25164, 26658, 28150, 29643, 31136, 32629, 34121, 35615, 37107, 38601, 40093, 41586, 43078, 44572, 46064, 47558, 49050, 50543, 52036, 53529, 55021, 56515, 58007, 59501, 60993, 62486, 63978, 65472,
...
class 技能11(主动技能):
    ...
    等级上限 = 70
    基础等级 = 33
    data0 = [int(i*1.00) for i in [0, 313, 345, 377, 409, 440, 473, 505, 537, 568, 600, 632, 664, 695, 727, 760, 792, 823, 855, 887, 919, 950, 982, 1015, 1047, 1078, 1110, 1142, 1174, 1205, 1237, 1269, 1302, 1333, 1365, 1397, 1429,
    ...
    攻击次数 = 14
    data1 = [int(i*1.00) for i in [0, 9525, 10492, 11457, 12424, 13391, 14357, 15324, 16289, 17256, 18223, 19189, 20156, 21121, 22088, 23055, 24021, 24988, 25953, 26920, 27887, 28853, 29820, 30785, 31752, 32719, 33685, 34652, 35617, 36584, 37551, 38517, 39484, 40449, 41416,
...
class 技能14(主动技能):
    ...
    成长 = 14582 * 1.1
    # 刺击魔法攻击
    data0 = [int(i*1.00) for i in [0, 886, 1091, 1297, 1501, 1707, 1913, 2118, 2324, 2529, 2735, 2941, 3145, 3351, 3556, 3762, 3968, 4173, 4379, 4584, 4790, 4996, 5200, 5406, 5611, 5817, 6022, 6228, 6434, 6638, 6844, 7049, 7255, 7461, 7666, 7872, 8076,
    ...
    攻击次数 = 1
    # 魅影连击
    data1 = [int(i*1.00) for i in [0, 1778, 2190, 2602, 3015, 3427, 3839, 4251, 4665, 5077, 5489, 5902, 6314, 6726, 7138, 7551, 7963, 8375, 8787, 9200, 9612, 10024, 10438, 10850, 11262, 11674, 12087, 12499, 12911, 13324, 13736, 14148, 14560, 14974, 15386, 15798, 16211,
    ...
    攻击次数2 = 5
    # 最终一击
    data2 = [int(i*1.00) for i in [0, 7554, 9306, 11057, 12809, 14560, 16313, 18065, 19816, 21568, 23319, 25071, 26822, 28575, 30327, 32078, 33830, 35581, 37334, 39086, 40837, 42589, 44340, 46093, 47844, 49596, 51348, 53099, 54851, 56602, 58355, 60107, 61858, 63610, 65361, 67114, 68865,
    ...
    攻击次数3 = 5
    # 爆炸
    data3 = [int(i*1.00) for i in [0, 15338, 18895, 22451, 26009, 29565, 33122, 36678, 40236, 43792, 47349, 50906, 54463, 58020, 61576, 65133, 68689, 72247, 75803, 79360, 82918, 86474, 90031, 93587, 97144, 100701, 104258, 107814, 111371, 114929, 118485, 122042, 125598, 129156, 132712, 136269, 139825,
...
class 技能15(主动技能):
    ...
    等级上限 = 40
    基础等级 = 23
    data0 = [int(i*1.00) for i in [0, 5453, 6006, 6560, 7113, 7666, 8219, 8773, 9327, 9879, 10433, 10986, 11539, 12092, 12646, 13199, 13752, 14305, 14859, 15413, 15965, 16519, 17072, 17626, 18178, 18732, 19285, 19838, 20391, 20945, 21499, 22051, 22605, 23158, 23712, 24264,
...
class 技能16(主动技能):
    ...
    等级上限 = 40
    基础等级 = 18
    data0 = [int(i*1.00) for i in [0, 1660, 1827, 1996, 2164, 2333, 2500, 2669, 2838, 3006, 3174, 3342, 3511, 3680, 3848, 4016, 4184, 4353, 4522, 4689, 4858, 5026, 5195, 5362, 5531, 5700, 5868, 6037, 6204, 6373, 6542, 6710, 6878, 7046, 7215, 7384, 7552,
    ...
    攻击次数 = 11
    data1 = [int(i*1.00) for i in [0, 19001, 20928, 22856, 24783, 26710, 28639, 30567, 32494, 34421, 36349, 38277, 40205, 42132, 44060, 45987, 47916, 49843, 51770, 53698, 55625, 57554, 59481, 61409, 63336, 65263, 67191, 69119, 71047, 72974, 74902, 76829, 78758, 80685, 82612, 84540, 86467,
...
class 技能17(主动技能):
    ...
    等级上限 = 40
    基础等级 = 16
    数据 = [int(i*1.00) for i in [0, 52556, 57887, 63219, 68550, 73882, 79215, 84546, 89878, 95209, 100541, 105872, 111205, 116537, 121868, 127200, 132531, 137864, 143196, 148527, 153859, 159190, 164522, 169855, 175186, 180518, 185849, 191181, 196514, 201845, 207177, 212508, 217840, 223171, 228504, 233836,
...
class 技能19(主动技能):
    ...
    等级上限 = 40
    基础等级 = 13
    data0 = [int(i*1.00) for i in [0, 17407, 19172, 20939, 22705, 24470, 26237, 28002, 29768, 31535, 33300, 35066, 36833, 38598, 40364, 42130, 43896, 45662, 47428, 49194, 50959, 52726, 54492, 56257, 58024, 59790, 61555, 63322, 65087, 66853, 68620, 70385, 72152, 73917, 75683, 77450, 79215,
    ...
    data1 = [int(i*1.00) for i in [0, 45626, 50256, 54885, 59513, 64142, 68772, 73400, 78029, 82658, 87287, 91916, 96544, 101173, 105802, 110431, 115060, 119689, 124317, 128947, 133576, 138204, 142833, 147463, 152091, 156720, 161348, 165978, 170607, 175235, 179864, 184494, 189122, 193751, 198379, 203009,
...
class 技能20(主动技能):
    ...
    等级上限 = 40
    基础等级 = 5
    data0 = [int(i*1.00) for i in [0, 9891, 12184, 14477, 16771, 19064, 21358, 23651, 25944, 28239, 30532, 32825, 35119, 37412, 39705, 42000, 44293, 46587, 48880, 51173, 53467, 55760, 58053, 60348, 62641, 64935, 67228, 69521, 71815, 74109, 76402, 78696, 80989, 83283, 85576, 87869, 90164, 92457,
    ...
    攻击次数 = 7
    data1 = [int(i*1.00) for i in [0, 69235, 85289, 101344, 117398, 133453, 149508, 165562, 181617, 197671, 213726, 229781, 245835, 261890, 277944, 293999, 310054, 326108, 342163, 358217, 374272, 390328, 406381, 422436, 438490, 454545, 470601, 486654, 502710, 518763, 534818, 550874, 566927, 582983, 599036, 615092,
...
class 技能22(主动技能):
    ...
    等级上限 = 40
    基础等级 = 6
    data0 = [int(i*1.00) for i in [0, 38764, 42693, 46623, 50553, 54483, 58413, 62343, 66273, 70203, 74133, 78063, 81993, 85923, 89853, 93783, 97713, 101643, 105573, 109502, 113432, 117362, 121292, 125222, 129152, 133082, 137012, 140942, 144872, 148802, 152732, 156662, 160592, 164522, 168452, 172382,
    ...
    data1 = [int(i*1.00) for i in [0, 90448, 99618, 108788, 117958, 127128, 136298, 145467, 154637, 163807, 172977, 182147, 191317, 200487, 209656, 218826, 227996, 237166, 246336, 255506, 264676, 273846, 283015, 292185, 301355, 310525, 319695, 328865, 338035, 347204, 356374, 365544, 374714, 383884, 393054, 402224,
...
class 技能23(主动技能):
    ...
    等级上限 = 40
    基础等级 = 2
    data0 = [int(i*1.00) for i in [0, 11941, 14710, 17479, 20248, 23017, 25786, 28555, 31324, 34093, 36862, 39631, 42400, 45169, 47938, 50707, 53476, 56245, 59014, 61783, 64552, 67321, 70090, 72859, 75628, 78397, 81166, 83935, 86704, 89473, 92242, 95011, 97780, 100549, 103317, 106087, 108856,
    ...
    攻击次数 = 10
    data1 = [int(i*1.00) for i in [0, 278627, 343238, 407846, 472456, 537066, 601674, 666285, 730895, 795503, 860113, 924723, 989331, 1053942, 1118552, 1183162, 1247770, 1312381, 1376991, 1441599, 1506209, 1570819, 1635427, 1700038, 1764648, 1829256, 1893866, 1958476, 2023084, 2087695, 2152305, 2216913, 2281523, 2346134, 2410742, 2475352,
