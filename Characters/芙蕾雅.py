...
class 芙蕾雅角色属性(角色属性):
    ...
    def 技能释放次数计算(self):
            ...
            if i.是否有伤害 == 1:
                if i.名称 == '爆裂弹':
                    ...
                    if self.次数输入[self.技能序号[i.名称]] == '/CD':
                        # 技能无法释放次数 = int(最大不脱手时间/(i.等效CD(self.武器类型,self.类型)+0.5))
                        # 技能释放次数.append(int((self.时间输入 - 反应时间) / (i.等效CD(self.武器类型,self.类型) + i.技能施放时间*释放时间系数 + CD延迟) + 1 + i.基础释放次数)-技能无法释放次数)
                        技能释放次数.append(int((self.时间输入 - 反应时间) / (i.等效CD(self.武器类型,self.类型) + i.技能施放时间*释放时间系数 + CD延迟) + 1 + i.基础释放次数))
                        if i.脱手 == 1:
                            技能消耗时间 += int((self.时间输入 - 反应时间) / (i.等效CD(self.武器类型,self.类型) + i.技能施放时间*释放时间系数 + CD延迟) + 1 + i.基础释放次数) * 0.12 * 释放时间系数
                        else:
        ...
        if self.次数输入[self.技能序号['爆裂弹']] == '/CD':
            ...
            技能释放次数[爆裂弹位置] = round(self.次数输入[self.技能序号['爆裂弹']],2) * 每轮空射次数
        return 技能释放次数
