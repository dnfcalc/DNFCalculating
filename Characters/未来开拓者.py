...
class 未来开拓者角色属性(角色属性):
    ...
    def 数据计算(self, x=0, y=-1):
        ...
        if self.技能栏[self.技能序号['能量禁锢']].是否装备护石 == 1:
            技能单次伤害[self.技能序号['能量禁锢']] += (0.33 * self.能量禁锢爆炸次数) * (
                self.技能栏[self.技能序号['能量禁锢']].基础2 +
                self.技能栏[self.技能序号['能量禁锢']].成长2 *
                self.技能栏[self.技能序号['能量禁锢']].等级) * (
                    1 + self.技能栏[self.技能序号['能量禁锢']].TP成长 *
                    self.技能栏[self.技能序号['能量禁锢']].TP等级
                ) * self.技能栏[self.技能序号['能量禁锢']].倍率 * self.伤害指数 * i.被动倍率
        ...
        if self.技能栏[self.技能序号['引力源光弹']].是否装备护石 == 1:
            if self.宠物次数[self.技能序号['引力源光弹']] < self.引力源光弹充能次数:
                技能总伤害[self.技能序号['CEAB-2超能爆发']] += (
                    技能单次伤害[self.技能序号['引力源光弹']] * 0.3 * self.引力源光弹充能次数 *
                    (1 + self.白兔子技能 * 0.20 + self.年宠技能 * 0.10 *
                     self.宠物次数[self.技能序号['引力源光弹']] / self.引力源光弹充能次数 +
                     self.斗神之吼秘药 * 0.12))
            else:
                技能总伤害[self.技能序号['CEAB-2超能爆发']] += (
                    技能单次伤害[self.技能序号['引力源光弹']] * 0.3 * self.引力源光弹充能次数 *
...
class 未来开拓者(角色窗口):
    ...
    def 引力源光弹充能次数判断(self, 警告, 选项变更):
        ...
        sign = 0
        for x in range(3):
            if self.护石栏[x].currentText(
            ) == '引力源光弹' and 引力源光弹次数 != 0 and 超能爆发次数 != 0:
                sign = 1

        充能次数上限 = 1
        if 引力源光弹次数 != 0 and 超能爆发次数 != 0 and (
                self.次数输入[22].currentText() != '/CD'
                or self.次数输入[15].currentText() != '/CD'):
            if 超能爆发次数 >= 引力源光弹次数:
                充能次数上限 = 引力源光弹次数
            else:
                充能次数上限 = 超能爆发次数
            # self.引力源光弹护石选项.setCurrentIndex(充能次数上限)
        if self.引力源光弹护石选项.currentIndex() > 充能次数上限 and sign == 1 and (
                self.次数输入[15].currentText() != '/CD'
                or self.次数输入[22].currentText() != '/CD') and 警告 == 0:
            self.引力源光弹护石选项.setCurrentIndex(充能次数上限)
            if 选项变更 == 0:
                QMessageBox.information(self, "错误", "输入的充能次数超过上限，已自动修正")
            警告 = 1
        elif self.引力源光弹护石选项.currentIndex() < 充能次数上限 and sign == 1 and (
                self.次数输入[15].currentText() != '/CD'
                or self.次数输入[22].currentText() != '/CD') and 选项变更 == 1:
            self.引力源光弹护石选项.setCurrentIndex(充能次数上限)
            选项变更 = 0
        elif self.次数输入[15].currentText(
        ) == '/CD' and self.次数输入[22].currentText() == '/CD' and 选项变更 == 1:
    ...
    def 能量禁锢爆炸次数判断(self, 警告):
        ...
        sign2 = 0
        for x in range(3):
            if self.护石栏[x].currentText() == '能量禁锢' and 能量禁锢次数 != 0 and (
                    能量飞鱼弹次数 + 源能护盾次数) != 0:
                sign2 = 1
        ...
        if (能量飞鱼弹次数 + 源能护盾次数) < (能量禁锢次数 *
                                 2) and sign2 == 1 and 警告 == False and (
                                     self.次数输入[9].currentText() != '/CD'
                                     and self.次数输入[12].currentText() != '/CD'):
            QMessageBox.information(
                self, "错误", "能量飞鱼弹与源能护盾次数不足以为能量禁锢提供" + str(能量禁锢次数 * 2) +
                "次爆炸，建议修改技能及爆炸次数，若不修改则按输入的数值计算")
            警告 = True
    ...
    def 界面(self):
        ...
        self.次数输入[15].currentIndexChanged.connect(
            lambda state: self.引力源光弹充能次数判断(警告=0, 选项变更=1))
        self.次数输入[22].currentIndexChanged.connect(
            lambda state: self.引力源光弹充能次数判断(警告=0, 选项变更=1))
        ...
        self.次数输入[19].currentIndexChanged.connect(
            lambda state: self.能量禁锢爆炸次数判断(警告=False))
        self.次数输入[12].currentIndexChanged.connect(
            lambda state: self.能量禁锢爆炸次数判断(警告=False))
