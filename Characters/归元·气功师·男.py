...
class 技能11(主动技能):
    ...
    # 基础 = 16854
    # 成长 = 367
    数据 = [0, 17219, 17586, 17954, 18321, 18688,
...
class 技能12(主动技能):
    ...
    等级精通 = 10
    基础等级 = min(int((等级 - 所在等级) / 学习间隔 + 1), 等级精通)
    数据 = [0, 128, 127, 130, 133, 136, 139, 142, 145, 148,
...
class 技能15(主动技能):
    ...
    攻击次数1 = 17
    数据2 = [0, 229, 252, 275, 299, 322, 345, 369, 392, 415, 438, 462, 485, 508, 531, 555, 578, 601, 624, 648, 671, 694, 718, 741, 764, 787, 811, 834, 857, 880, 904, 927, 950,
...
class 技能25(主动技能):
    ...
    攻击次数1 = 1
    数据2 = [0,463,510,557,604,652,699,746,793,840,887,934,981,1028,1075,1122,1169,1216,1263,1310,1358,1405,1452,1499,1546,1593,1640,1687,1734,1781,1828,1875,1922,1969,2016,2063,2110,2158,2205,2252,2299,2346,2393,2440,2487,2534,2581,2628,2675,2722,2769,2816,2863,2910,2958,3005,3052,3099,3146,3193,3240]
    攻击次数2 = 1
    CD = 8.4
    TP成长 = 0.1
    TP上限 = 5
    ...
    # TP加感电
    def 等效百分比(self, 武器类型):
        return (self.数据1[self.等级] * self.攻击次数1) * (1 + self.TP成长 * self.TP等级) * self.倍率+ (self.数据2[self.等级] * self.攻击次数2 ) * (1 + self.TP成长 * self.TP等级* 0.07) * self.倍率
