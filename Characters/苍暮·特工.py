...
class 技能2(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 43
    data0 = [int(x*1.0) for x in [0, 1187, 1307, 1427, 1548, 1668, 1788, 1909, 2030, 2149, 2270, 2391, 2512, 2631, 2752, 2873, 2993, 3113, 3234, 3354, 3474, 3595, 3715, 3836, 3956, 4077, 4197, 4318, 4438, 4558, 4679, 4800, 4919, 5040, 5161, 5280, 5401, 5522, 5642, 5762, 5883, 6004, 6124, 6244, 6365, 6485, 6606, 6726, 6846, 6967, 7088, 7207, 7328, 7449, 7568, 7689, 7810, 7931, 8050, 8171, 8292, 8412, 8532, 8653, 8773, 8894, 9014, 9134, 9255, 9375, 9495]]
    攻击次数1 = 1
    data1 = [int(x*1.0) for x in [0, 1780, 1960, 2141, 2322, 2502, 2683, 2863, 3044, 3225, 3406, 3587, 3767, 3948, 4129, 4309, 4490, 4670, 4851, 5032, 5212, 5393, 5573, 5754, 5935, 6115, 6296, 6476, 6657, 6838, 7018, 7199, 7379, 7560, 7741, 7921, 8102, 8282, 8463, 8644, 8824, 9006, 9186, 9367, 9548, 9728, 9909, 10089, 10270, 10451, 10631, 10812, 10992, 11173, 11354, 11534, 11715, 11895, 12076, 12257, 12437, 12618, 12798, 12979, 13160, 13340, 13521, 13701, 13882, 14063, 14243]]
    攻击次数2 = 1
    CD = 6.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能3(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 41
    数据 = [int(x*1.0) for x in [0, 2029, 2234, 2441, 2647, 2853, 3059, 3264, 3470, 3676, 3882, 4088, 4293, 4499, 4705, 4912, 5118, 5324, 5529, 5735, 5941, 6147, 6353, 6558, 6764, 6970, 7177, 7383, 7588, 7794, 8000, 8206, 8412, 8617, 8823, 9029, 9235, 9441, 9647, 9853, 10059, 10265, 10471, 10676, 10882, 11088, 11294, 11500, 11705, 11912, 12118, 12324, 12530, 12735, 12941, 13147, 13353, 13559, 13764, 13970, 14176, 14383, 14589, 14795, 15000, 15206, 15412, 15618, 15824, 16029, 16235]]
    攻击次数 = 2
    CD = 7.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能6(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 46
    数据 = [int(x*1.0) for x in[0, 2695, 2968, 3241, 3515, 3788, 4062, 4335, 4608, 4883, 5157, 5429, 5703, 5977, 6250, 6523, 6797, 7070, 7344, 7617, 7890, 8164, 8438, 8711, 8984, 9258, 9532, 9804, 10078, 10352, 10624, 10898, 11172, 11445, 11719, 11992, 12265, 12539, 12813, 13085, 13359, 13633, 13905, 14179, 14453, 14726, 14999, 15273, 15546, 15820, 16093, 16366, 16640, 16914, 17187, 17460, 17735, 18009, 18281, 18555, 18829, 19102, 19375, 19649, 19922, 20196, 20469, 20742, 21016, 21290, 21562]]
    攻击次数 = 1
    CD = 5.0
    TP成长 = 0.08
    TP上限 = 5
...
class 技能7(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 36
    数据 = [int(x*1.0) for x in[0, 837, 922, 1007, 1091, 1176, 1261, 1346, 1431, 1516, 1601, 1686, 1771, 1856, 1941, 2026, 2111, 2196, 2281, 2365, 2450, 2535, 2620, 2705, 2790, 2875, 2960, 3045, 3130, 3215, 3300, 3385, 3470, 3554, 3639, 3724,
...
class 技能8(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 33
    数据 = [int(x*1.0) for x in[0, 1210, 1332, 1455, 1578, 1701, 1823, 1946, 2069, 2191, 2314, 2437, 2560, 2683, 2806, 2928, 3051, 3174, 3296, 3419, 3542, 3665, 3787, 3910, 4033, 4156, 4279, 4402, 4524, 4646, 4769, 4892, 5015, 5138, 5261, 5383, 5506, 5629, 5752, 5875, 5997, 6119, 6242, 6365, 6488, 6611, 6734, 6857, 6979, 7102, 7225, 7347, 7470, 7593, 7715, 7838, 7961, 8084, 8207, 8330, 8453, 8575, 8698, 8820, 8943, 9066, 9189, 9311, 9434, 9557, 9680]]
    攻击次数 = 1
    CD = 20.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能10(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 23
    数据 = [int(x*1.0) for x in[0, 5288, 5824, 6361, 6897, 7434, 7971, 8507, 9044, 9580, 10117, 10653, 11190, 11725, 12262, 12798, 13335, 13872, 14408, 14945, 15481, 16018, 16554, 17091, 17627, 18164, 18701, 19237, 19774, 20310, 20847, 21383, 21920, 22456, 22992, 23528, 24065, 24602, 25138, 25675, 26211]]
    攻击次数 = 4
    CD = 30.0
    TP成长 = 0.10
    TP上限 = 5
...
class 技能12(职业主动技能):
    ...
    等级上限 = 40
    基础等级 = 12
    数据 = [int(x*1.0) for x in[0, 29207, 35979, 42753, 49525, 56298, 63070, 69843, 76616, 83389, 90161, 96934, 103706, 110480, 117252, 124025, 130797, 137571, 144344, 151116, 157889, 164661, 171435, 178207, 184980, 191752, 198526, 205298, 212071, 218843, 225616, 232389, 239162, 245934, 252707, 259480, 266253, 273025, 279798, 286570, 293344]]
    攻击次数 = 2
    一觉减CD = 0
    冷却关联技能 = ['所有']
    暗杀目标加成 = 1.1
...
class 技能14(职业主动技能):
    ...
    等级上限 = 40
    基础等级 = 16
    data0 = [int(x*1.0) for x in[0, 4604, 5070, 5538, 6005, 6472, 6939, 7407, 7874, 8340, 8807, 9275, 9742, 10209, 10676, 11144, 11611, 12077, 12544, 13012, 13479, 13946, 14413, 14881, 15347, 15814, 16281, 16749, 17216, 17683, 18150, 18617, 19084, 19551, 20018, 20486, 20953, 21420, 21887, 22353, 22821]]
    攻击次数1 = 9
    data1 = [int(x*1.0) for x in[0, 17759, 19560, 21362, 23164, 24966, 26768, 28569, 30371, 32173, 33975, 35776, 37577, 39379, 41181, 42983, 44785, 46586, 48388, 50190, 51992, 53793, 55594, 57396, 59198, 61000, 62802, 64603, 66405, 68207, 70009, 71810, 73611, 75413, 77215, 79017, 80819, 82620, 84422, 86224, 88026]]
    攻击次数2 = 1
    CD = 40.0
    是否有护石 = 1
...
class 技能17(职业主动技能):
    ...
    等级上限 = 60
    基础等级 = 38
    数据 = [int(x*1.0) for x in[0, 5211, 5740, 6270, 6798, 7327, 7855, 8384, 8912, 9442, 9971, 10499, 11028, 11556, 12086, 12615, 13143, 13672, 14200, 14729, 15258, 15787, 16316, 16844, 17373, 17901, 18431, 18960, 19488, 20017, 20545, 21075, 21603, 22132, 22661, 23189, 23718, 24247, 24776, 25305, 25833, 26362, 26891, 27420, 27948, 28477, 29006, 29534, 30064, 30592, 31121, 31650, 32178, 32708, 33236, 33765, 34293, 34822, 35351, 35880, 36409, 36937, 37466, 37994, 38523, 39053, 39581, 40110, 40638, 41167, 41697]]
    攻击次数 = 1
    CD = 8.0
    TP成长 = 0.10
    TP上限 = 5
