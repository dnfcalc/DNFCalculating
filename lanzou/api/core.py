...
class LanZouCloud(object):
    ...
    def _post(self, url, data, **kwargs):
        for possible_url in self._all_possible_urls(url):
            try:
                kwargs.setdefault('timeout', self._timeout)
                kwargs.setdefault('headers', self._headers)
                return self._session.post(possible_url,
                                          data,
                                          verify=False,
                                          **kwargs)
            except (ConnectionError, requests.RequestException):
    ...
    def delete(self, fid, is_file=True) -> int:
        ...
        if not result:
            return LanZouCloud.NETWORK_ERROR
        return LanZouCloud.SUCCESS if result.json(
        )['zt'] == 1 else LanZouCloud.FAILED
    ...
    def clean_rec(self) -> int:
        ...
        if not html:
            return LanZouCloud.NETWORK_ERROR
        post_data['formhash'] = re.findall(r'name="formhash" value="(.+?)"',
                                           html.text)[0]  # 设置表单 hash
    ...
    def delete_rec(self, fid, is_file=True) -> int:
        ...
        post_data['formhash'] = re.findall(r'name="formhash" value="(\w+?)"',
                                           html.text)[0]  # 设置表单 hash
        html = self._post(self._mydisk_url + '?item=recycle', post_data)
        if not html:
            return LanZouCloud.NETWORK_ERROR
            ...
            for file in resp["text"]:
                file_list.append(
                    File(
                        ...
                        has_pwd=True
                        if int(file['onof']) == 1 else False,  # 是否存在提取码
                        has_des=True
                        if int(file['is_des']) == 1 else False  # 是否存在描述
                    ))
        return file_list
    ...
    def clean_ghost_folders(self):
        ...
        for folder in folder_with_ghost:
            if not real_folders.find_by_id(folder.id):
                logger.debug(
                    f"Delete ghost folder: {folder.name} #{folder.id}")
    ...
    def get_full_path(self, folder_id=-1) -> FolderList:
        ...
        for folder in resp.json()['info']:
            if folder['folderid'] and folder[
                    'name']:  # 有时会返回无效数据, 这两个字段中某个为 None
                path_list.append(
    ...
    def get_file_info_by_url(self, share_url, pwd='') -> FileDetail:
        ...
        if 'id="pwdload"' in first_page or 'id="passwddiv"' in first_page:  # 文件设置了提取码时
            if len(pwd) == 0:
                return FileDetail(LanZouCloud.LACK_PASSWORD,
                                  pwd=pwd,
                                  url=share_url)  # 没给提取码直接退出
            ...
            link_info = self._post(self._host_url + '/ajaxm.php',
                                   post_data)  # 保存了重定向前的链接信息和文件名
            second_page = self._get(
                share_url)  # 再次请求文件分享页面，可以看见文件名，时间，大小等信息(第二页)
            ...
            f_time = re.search(r'class="n_file_infos">(.+?)</span>',
                               second_page)
            f_time = time_format(
                f_time.group(1)) if f_time else time_format('0 小时前')
            f_desc = re.search(r'class="n_box_des">(.*?)</div>', second_page)
            f_desc = f_desc.group(1) if f_desc else ''
        else:  # 文件没有设置提取码时,文件信息都暴露在分享页面上
            para = re.search(r'<iframe.*?src="(.+?)"',
                             first_page).group(1)  # 提取下载页面 URL 的参数
            # 文件名位置变化很多
            ...
            if len(sign) < 20:  # 此时 sign 保存在变量里面, 变量名是 sign 匹配的字符
                sign = re.search(rf"var {sign}\s*=\s*'(.+?)';",
                                 first_page).group(1)
        ...
        fake_url = link_info['dom'] + '/file/' + link_info[
            'url']  # 假直连，存在流量异常检测
    ...
    def get_durl_by_url(self, share_url, pwd='') -> DirectUrlInfo:
        ...
        return DirectUrlInfo(LanZouCloud.SUCCESS, file_info.name,
                             file_info.durl)
    ...
    def get_share_info(self, fid, is_file=True) -> ShareInfo:
        ...
        if ('f_id' in f_info.keys()
                and f_info['f_id'] == 'i') or ('name' in f_info.keys()
                                               and not f_info['name']):
    ...
    def move_folder(self, folder_id, parent_folder_id=-1) -> int:
        ...
        new_folder_id = self.mkdir(parent_folder_id, folder.name,
                                   info.desc)  # 在目标文件夹下创建同名文件夹
    ...
    def _upload_small_file(self,
        ...
        result = self._post('https://pc.woozooo.com/fileup.php',
                            data=monitor,
                            headers=tmp_header,
                            timeout=3600)
        if not result:  # 网络异常
    ...
    def _upload_big_file(self,
        ...
        if not os.path.exists(record_file):  # 初始化记录文件
            ...
            with open(record_file, 'rb') as f:
                ...
                logger.debug(
                    f"Find upload record: {uploaded_size}/{file_size}")
        ...
        while uploaded_size < file_size:
            data_size, data_path = big_file_split(file_path,
                                                  self._max_size,
                                                  start_byte=uploaded_size)
            code = self._upload_small_file(data_path,
                                           dir_id,
                                           callback=_callback,
                                           uploaded_handler=_close_pwd)
            if code == LanZouCloud.SUCCESS:
                ...
                with open(record_file, 'wb') as f:
                    logger.debug(
                        f"Update record file: {uploaded_size}/{file_size}")
                    pickle.dump(info, f)
            else:
        ...
        code = self._upload_small_file(record_file_new,
                                       dir_id,
                                       uploaded_handler=_close_pwd)  # 上传记录文件
        if code != LanZouCloud.SUCCESS:
    ...
    def upload_file(self,
        ...
        return self._upload_big_file(file_path,
                                     dir_id,
                                     callback=callback,
                                     uploaded_handler=uploaded_handler)
    ...
    def upload_dir(self,
        ...
        for filename in os.listdir(dir_path):
            ...
            if not os.path.isfile(file_path):
                continue  # 跳过子文件夹
            code = self.upload_file(file_path,
                                    dir_id,
                                    callback=callback,
                                    uploaded_handler=uploaded_handler)
            if code != LanZouCloud.SUCCESS:
    ...
    def down_file_by_url(self,
        ...
        if os.path.exists(file_path):
            if overwrite:
                logger.debug(
                    f"Overwrite file {file_path}, will remove after download finish"
                )
            else:  # 自动重命名文件
                file_path = auto_rename(file_path)
                logger.debug(
                    f"File has already exists, auto rename to {file_path}")
    ...
    def down_file_by_id(self,
        ...
        if info.code != LanZouCloud.SUCCESS:
            return info.code
        return self.down_file_by_url(info.url,
                                     info.pwd,
                                     save_path,
                                     callback=callback,
                                     overwrite=overwrite,
    ...
    def get_folder_info_by_url(self,
                               share_url,
                               dir_pwd='',
                               get_this_page=0) -> FolderDetail:
        """获取文件夹里所有文件的信息"""
        ...
        if ('id="pwdload"' in html
                or 'id="passwddiv"' in html) and len(dir_pwd) == 0:
        ...
        try:
            ...
            folder_time = re.findall(r'class="rets">([\d\-]+?)<a',
                                     html)  # ['%m-%d'] 或者 None (vip自定义)
            folder_time = folder_time[0] if folder_time else datetime.today(
            ).strftime("%m-%d")  # 没有就设为现在
        ...
        while True:
            ...
            try:
                ...
                resp = self._post(self._host_url + '/filemoreajax.php',
                                  data=post_data,
        ...
        this_folder = FolderInfo(folder_name, folder_id, dir_pwd, folder_time,
                                 folder_desc, share_url)
        return FolderDetail(LanZouCloud.SUCCESS, this_folder, files,
                            sub_folders)
    ...
    def _down_big_file(self,
        ...
        with open(big_file, 'ab') as bf:
            for file in file_list:
                ...
                down_start_byte = file_size_now - info[
                    'last_ending']  # 当前数据块上次下载中断的位置
                headers = {
                    **self._headers, 'Range': 'bytes=%d-' % down_start_byte
                }
                logger.debug(
                    f"Download {file.name}, Range: {down_start_byte}-")
    ...
    def down_dir_by_url(self,
        ...
        if info is not None:
            return self._down_big_file(*info,
                                       save_path,
                                       callback=callback,
                                       overwrite=overwrite,
                                       downloaded_handler=downloaded_handler)
        ...
        for file in folder_detail.files:
            code = self.down_file_by_url(file.url,
                                         dir_pwd,
                                         save_path,
                                         callback=callback,
                                         overwrite=overwrite,
        ...
        if recursive and folder_detail.sub_folders:
            for sub_folder in folder_detail.sub_folders:
                self.down_dir_by_url(sub_folder.url,
                                     dir_pwd,
                                     save_path,
                                     callback=callback,
                                     overwrite=overwrite,
    ...
    def down_dir_by_id(self,
        ...
        for file in file_list:
            code = self.down_file_by_id(file.id,
                                        save_path,
                                        callback=callback,
                                        overwrite=overwrite,
                                        downloaded_handler=downloaded_handler)
        ...
        if recursive:
            ...
            if len(sub_folders) != 0:
                for sub_folder in sub_folders:
                    self.down_dir_by_id(sub_folder.id,
                                        save_path,
                                        callback=callback,
                                        overwrite=overwrite,
                                        failed_callback=failed_callback,
